...
If you look at `./testbed/main.go` you can modify that file and then run 
```sh
haxe testbed/build.hxml -D interp_
```
It will run both Go2hx and Go and allow you to see the difference.
I use this tool daily and almost all problems I run into, I isolate the problem from a bigger piece of Go code, run it in ``testbed``, see the issue, create a unit test, and then make changes to the compiler until it works properly.
...
haxe --run Run build
```
The project relies on git submodules, if you have cloned the project and forgot to add ``--recursive`` you can get the submodules after with:
```sh
...
* ``haxe testbed/build.hxml`` : a test bed that runs ``./testbuild/main.go`` use it for rapid testing of go snippets and comparison with go output.
