...
import stdgo._internal.context.Context;
function testAfterFuncCalledAfterCancel(_t:stdgo.Ref<stdgo._internal.testing.Testing_T_.T_>):Void {
            ...
            while (__select__) {
                if (_donec != null && _donec.__isGet__()) {
                    __select__ = false;
                    {
                        _donec.__get__();
                        {
                            _t.fatalf(("AfterFunc called before context is done" : stdgo.GoString));
                        };
                    };
