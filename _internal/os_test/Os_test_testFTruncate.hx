...
import stdgo._internal.os.Os;
import stdgo._internal.os.Os;
function testFTruncate(_t:stdgo.Ref<stdgo._internal.testing.Testing_T_.T_>):Void {
        var __deferstack__:Array<Void -> Void> = [];
        try {
            _t.parallel();
            var _f = _internal.os_test.Os_test__newFile._newFile(("TestFTruncate" : stdgo.GoString), _t);
            {
                var _a0 = _f.name();
