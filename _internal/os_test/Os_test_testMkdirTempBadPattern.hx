...
import stdgo._internal.os.Os;
import stdgo._internal.os.Os;
function testMkdirTempBadPattern(_t:stdgo.Ref<stdgo._internal.testing.Testing_T_.T_>):Void {
        ...
        try {
            ...
            for (__16 => _tt in _tests) {
                _t.run(_tt._pattern?.__copy__(), function(_t:stdgo.Ref<stdgo._internal.testing.Testing_T_.T_>):Void {
                    var __deferstack__:Array<Void -> Void> = [];
                    try {
                        var __tmp__ = stdgo._internal.os.Os_mkdirTemp.mkdirTemp(_tmpDir?.__copy__(), _tt._pattern?.__copy__()), __25:stdgo.GoString = __tmp__._0, _err:stdgo.Error = __tmp__._1;
                        if (_tt._wantErr) {
                            if (_err == null) {
