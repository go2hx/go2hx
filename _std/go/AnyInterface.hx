...
class AnyInterfaceData {
	...
	public var type:go._internal.internal.reflect.Reflect._Type;
 ...
 * The type information is filled in using ``go.Go.toInterface`` at macro compile time.
 * 
...
abstract AnyInterface(AnyInterfaceData) from AnyInterfaceData {
	...
	@:op(A == B) public static function equals(a:AnyInterface, b:AnyInterface):Bool {
		...
		var gt:go._internal.internal.reflect.GoType = @:privateAccess (a.type : Dynamic)._common();
		var gt2:go._internal.internal.reflect.GoType = @:privateAccess (b.type : Dynamic)._common();
		...
		gt = go._internal.internal.reflect.Reflect.getUnderlying(gt);
		gt2 = go._internal.internal.reflect.Reflect.getUnderlying(gt2);
		// set internal Type
		...
		return switch gt {
			...
			case structType(fields):
				...
				switch gt2 {
					case structType(fields2):
						for (i in 0...fields.length) {
							...
							final type = @:privateAccess new go._internal.internal.reflect.Reflect._Type(go._internal.internal.reflect.Reflect.unroll(gt,
								type));
							final type2 = @:privateAccess new go._internal.internal.reflect.Reflect._Type(go._internal.internal.reflect.Reflect.unroll(gt,
								type2));
				...
				true;
			case invalidType:
				switch @:privateAccess ((b.type : Dynamic)._common() : go._internal.internal.reflect.GoType) {
			...
			case arrayType(_.get() => elem, _):
				...
				var t = new go._internal.internal.reflect.Reflect._Type(elem);
...
inline function errorString(s:go.GoString) {
