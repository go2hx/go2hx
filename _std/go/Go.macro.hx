				...
				case TAbstract(_.get() => at, params):
					...
					switch path {
						...
						case "go.GoMap":
							return expr;
						case "go.Slice":
							value = macro new go.Slice(0, -1);
						case "go.AnyInterface":
							// force cast into
							value = macro new go.AnyInterface(null,
								new go._internal.internal.reflect.Reflect._Type($gt));
						case ".Null":
							t = params[0];
							value = gen(isNull);
						case "go.Chan": // grab params for default value
							final t = Context.toComplexType(params[0]);
							value = macro new go.Chan(-1, null);
						case "go.Pointer":
							final t = Context.toComplexType(params[0]);
							value = macro new go.Pointer(null);
						default:
						...
						case TAnonymous(_.get() => a):
							for (field in a.fields) {
								...
								if (field.name == "__underlying__") {
									final rt = macro new go._internal.internal.reflect.Reflect._Type($gt);
									expr = macro () -> new go.AnyInterface(null, $rt);
								}
