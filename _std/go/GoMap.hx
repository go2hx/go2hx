...
import go._internal.internal.reflect.Reflect._Type;
import go.Pointer;
...
abstract GoMap<K, V>(IMap<K, V>) {
	...
	**/
	...
	// Do not add ObjectMap casting, breaks regexp/syntax GoRefMap cast at go/_internal/regexp/syntax/Syntax_T_parser_static_extension.hx  
}
@:dox(hide)
...
class GoAnyInterfaceMap<V> extends CompareMap<AnyInterface, V> {
	...
	override function remove(key:AnyInterface):Bool {
		...
		switch gt {
			case sliceType(_):
				throw errorString("hash of unhashable type " + new go._internal.internal.reflect.Reflect._Type(gt).string().toString());
			default:
				//trace(gt);
		}
		return super.remove(key);
