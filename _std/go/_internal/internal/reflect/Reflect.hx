...
@:follow private typedef Type = go._internal.reflect.Reflect_type_.Type_;

private typedef ReflectKind = go._internal.reflect.Reflect_kind.Kind;

private typedef ReflectMethod = go._internal.reflect.Reflect_method.Method;

private typedef Kind = go._internal.reflect.Reflect_kind.Kind
private typedef ReflectValue = go._internal.reflect.Reflect_value.Value;

private typedef ReflectStructField = go._internal.reflect.Reflect_structfield.StructField;

private typedef ReflectChanDir = go._internal.reflect.Reflect_chandir.ChanDir;


enum abstract KindType(ReflectKind) from Int 
from go._internal.reflect.Reflect_kind.Kind to go._internal.reflect.Reflect_kind.Kind
{
	final invalid = 0;
	final bool;
	final int;
...
function _directlyAssignable(tgt:GoType, vgt:GoType):Bool {
	...
	return switch tgt {
		...
		case interfaceType(_):
			false;
			/*var bType = new go._internal.internal.reflect.Reflect._Type(vgt);
			final b = new go._internal.internal.reflect.Reflect._Type(tgt).implements_(cast new go._internal.internal.reflect.Reflect._Type_asInterface(new Pointer(() -> bType, value -> bType = value), bType));
			b;*/
		...
		case named(path,_,_,_,_):
			...
			switch typ {
				case TClass(cl):
					...
					if (className == "go.PointerData")
						isPointer = true;
				default:
			}
			return std.Type.createInstance(cl,[isPointer ? value : Go.pointer(value),new go._internal.internal.reflect.Reflect._Type(gt)]);
...
@:structInit
@:using(go._internal.internal.reflect.Reflect._Type_static_extension)
@:named
class _Type {
	@:local
	var gt:GoType;
...
@:keep @:allow(github_com.go2hx.go4hx.rnd.Rnd._Type_asInterface) class _Type_static_extension {
	...
	static public function overflowComplex(t:_Type, _x:go.GoComplex128):Bool
		throw "not implemented overflowComplex";

	static public function overflowFloat(t:_Type, _x:go.GoFloat):Bool
		throw "not implemented overflowFloat";

	static public function overflowInt(t:_Type, _x:go.GoInt64):Bool
	...
	static public function numField(t:_Type):GoInt {
		var type:go._internal.internal.reflect.GoType = @:privateAccess t._common();
	...
	static public function bits(t:_Type):GoInt {
		if (t == null) {
			throw new go.Error.T_errorString_asInterface("reflect: Bits of nil Type").__underlying__();
		}
		final k = t.kind();
		if (k < KindType.int || k > KindType.complex128) {
			throw new go.Error.T_errorString_asInterface("reflect: Bits of non-arithmetic Type " + t.string()).__underlying__();
		}
	...
	static private function formatGoPath(path:String):String {
		if (useHaxePath)
			return path;
		final go = "go._internal";
		var index = path.indexOf(go);
		if (index == 0)
			path = path.substr(go.length);
...
class _Type_asInterface {
	...
	public dynamic function overflowComplex(_x:go.GoComplex128):Bool
		return __self__.value.overflowComplex(_x);

	public dynamic function overflowFloat(_x:go.GoFloat):Bool
		return __self__.value.overflowFloat(_x);

	public dynamic function overflowInt(_x:go.GoInt64):Bool
		return __self__.value.overflowInt(_x);

	public dynamic function overflowUint(_x:go.GoUInt64):Bool
	...
	public dynamic function __underlying__()
		return new AnyInterface((__type__.kind() == go._internal.internal.reflect.Reflect.KindType.pointer
