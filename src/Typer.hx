	...
	for (pkg in data.pkgs) {
		...
		for (file in module.files) {
			...
			for (def in defs) {
				...
				final ct:ComplexType = TPath({name: def.name, pack: []});
								...
								case FFun(fun):
									...
									final field:Field = {
										...
										kind: FFun({
											args: fun.args.copy(),
											ret: fun.ret,
											expr: {expr: fun.expr.expr, pos: null},
										}),
									}
									final wrapperField:Field = {
										name: func.name,
										pos: func.pos,
										meta: func.meta.copy(),
										access: [APublic],
									...
									switch field.kind {
										case FFun(fun):
											final name = fun.args.shift().name;
											switch fun.expr.expr {
												case EBlock(exprs): // that gets added to expr block
													fun.expr = {expr: EBlock([macro var $name = this].concat(exprs.copy())), pos: null};
												default:
											}
										default:
									}

									switch wrapperField.kind {
										case FFun(fun):
											final name = fun.args.shift().name;
											switch fun.expr.expr {
												case EBlock(exprs): // that gets added to expr block
													if (isPointerArg) {
														fun.expr = {expr: EBlock([macro var $name = Go.pointer(__t__)].concat(exprs.copy())), pos: null};
													} else {
														fun.expr = {expr: EBlock([macro var $name = __t__].concat(exprs.copy())), pos: null};
													}
												default:
											}
										default:
									...
									if (isStruct) {
										def.fields.unshift(field);
									}
									staticExtension.fields.unshift(staticField);
									wrapper.fields.unshift(wrapperField);
								default:
							}
						default:
					}
...
private function checkType(e:Expr, ct:ComplexType, fromType:GoType, toType:GoType, info:Info):Expr {
	...
	if (isAnyInterface(toType))
		return e;
	if (isNamed(fromType) && !isStruct(fromType) && isInterface(toType) && !isAnyInterface(toType)) {
		final p = getTypePath(fromType, info);
		p.name += "_wrapper";
		if (!isRefValue(fromType) && !isPointer(fromType))
	...
	if ((isRef(fromType) || isPointer(fromType))
		...
		&& isInterface(toType)) {
		final p = getTypePath(getElem(fromType), info);
		p.name += "_wrapper"; // wrapper name
		e = macro new $p($e);
	}
	return macro($e : $ct);
}
...
private function castTranslate(obj:Ast.Expr, e:Expr, info:Info):{expr:Expr, ok:Bool} {
	return switch obj.id {
		...
		case "IndexExpr":
			...
			var x = typeExpr(obj.x, info);
			final t = getUnderlying(typeof(obj));
			// trace(t);
...
private function typeCallExpr(expr:Ast.CallExpr, info:Info):ExprDef {
	...
	switch expr.fun.id {
		...
		case "SelectorExpr":
			switch typeof(expr.fun.sel) {
				case signature(_, _, _, recv):
					switch recv {
						case _var(_, type):
							final xType = typeof(expr.fun.x);
							if (!isPointer(xType) && isPointer(type)) {
								var sel = switch typeExpr(expr.fun.sel, info).expr {
								...
								genArgs(true);
								return returnExpr(macro Go.pointer($x).$sel($a{args}));
							}
						default:
					}
				default:
			...
			switch expr.fun.sel.name {
				case "String":
					...
					if (typeof(expr.type).match(basic(string_kind))) {
...
private function typeCompositeLit(expr:Ast.CompositeLit, info:Info):ExprDef {
	...
	final e = compositeLit(type, expr, info);
...
function compositeLit(type:GoType, expr:Ast.CompositeLit, info:Info):ExprDef {
	...
	switch underlying {
		...
		case sliceType(elem):
			final p = getTypePath(underlying, info);
...
private function typeSliceExpr(expr:Ast.SliceExpr, info:Info):ExprDef {
	...
	final ct = toComplexType(typeof(expr), info);
...
private function typeIndexExpr(expr:Ast.IndexExpr, info:Info):ExprDef {
	...
	var e = macro($x != null ? $x[$index] : ${defaultValue(typeof(expr), info)});
	return e.expr;
}
...
private function typeFunction(decl:Ast.FuncDecl, data:Info, restricted:Array<String> = null, isNamed:Bool = false, sel:String = ""):TypeDefinition {
	...
	var typeParams:Array<TypeParamDecl> = null;
	var access = [];
	if (decl.type.typeParams != null) {
		...
		final funcArgs = [
			for (arg in args) {
				final ct = macro haxe.macro.TypeTools.toComplexType(haxe.macro.Context.typeof($i{arg.name}));
				macro {
		...
		final extension = [
			for (arg in args)
				macro className += haxe.macro.Context.signature(haxe.macro.TypeTools.toComplexType(haxe.macro.Context.typeof($i{arg.name}))) + "_"
		];
		...
		block = macro {
			...
			try {
				...
				haxe.macro.Context.defineModule(modulePath.join("."), [td], haxe.macro.Context.getLocalImports());
			}
			return @:macro $call;
		};
	}
	...
	switch t {
		case structType(fs):
			...
			return {
				...
				meta: [{name: ":structInit", pos: null}, {name: ":named", pos: null},],
