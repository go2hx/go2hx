					...
					vars = vars.concat([
						for (i in 0...spec.names.length) {
							var expr = typeExpr(spec.values[i], info);
							{
								...
								type: type,
								expr: i < spec.values.length ? expr : type != null ? defaultValue(typeof(spec.type)) : null,
							};
						}
					]);
				default:
		...
		case DEFINE: // var expr = x;
			...
			if (stmt.lhs.length == stmt.rhs.length) {
				...
				for (i in 0...stmt.lhs.length) {
					...
					vars.push({
						name: name,
						type: toComplexType(typeof(stmt.rhs[i])),
						expr: expr,
					});
				}
				return EVars(vars);
				...
				var args = [for (lhs in stmt.lhs) toExpr(EConst(CString(lhs.name)))];
				var func = typeExpr(stmt.rhs[0],info);
				args.push(func);
				return (macro Go.destruct($a{args})).expr;
			} else {
				...
				case "new": // create default value put into pointer
					...
					switch t {
						case TPath(_), TFunction(_,_), TAnonymous(_):
							return (macro Go.pointer(${defaultValue(typeof(expr.args[0]))})).expr; // TODO does not work for non constructed types, such as basic types
						default:
					}
				case "make":
				...
				switch t {
					case TPath(p):
						if ((p.name == "Pointer" || p.name == "PointerWrapper") && p.params != null) {
							switch p.params[0] {
								case TPType(t2):
									var value = defaultValue(typeof(e.x));
									return (macro Go.pointer($value)).expr;
								default:
							}
						}
						return (macro ${defaultValue(typeof(e.x))}).expr;
					default:
						var ex = typeExpr(expr.args[0], info);
						switch ex.expr {
							case EConst(c):
								switch c {
									case CIdent(s):
										if (s == "null") {
											var e = defaultValue(typeof(e.x));
											return e.expr;
										}
									default:
								}
						...
						//return checkType(e,typeof(expr.args[0]),type);
				}
			}
		case "MapType":
			// set assert type
			...
			switch e.expr {
				case EConst(c):
					switch c {
						case CIdent(s):
							if (s == "null") {
								var e = defaultValue(typeof(expr.fun));
								return e.expr;
							}
						default:
					}
		...
		case struct(fields):
			TAnonymous([
				for (field in fields) {
					name: field.name,
					pos: null,
					kind: FVar(toComplexType(field.type))
				}
			]);
		case signature(variadic, params, results, recv):
			var args = [];
								...
								switch c {
									case CInt(v):
										...
										if (dif > 0) {
											...
											for (i in 0...dif)
												params.push(defaultValue(typeof(a.elt)));
										}
						...
						case TPType(t):
							switch typeof(expr.type) {
								case map(_,valueType):
									params.push(defaultValue(valueType));
	...
	switch e.expr {
		case EConst(c):
			switch c {
				case CIdent(s):
					if (s == "null") {
						var e = defaultValue(typeof(expr.type));
						return e.expr;
					}
				default:
			}
		...
		case named(path, underlying):
			switch underlying {
				...
				default:
					...
					var name = className(title(path.substr(index)));
					var pack = StringTools.replace(path.substr(0,index - 1),"-","_").split(".");
					var t:TypePath = {pack: pack,name: name};
					macro new $t();
			}
		case basic(kind):
			switch kind {
			...
			switch type {
				case TPath(p):
					...
					fields.push({
						...
						kind: FVar(type, defaultBool ? defaultValue(typeof(field.type)) : null)
					});
				default:
			}
		}
...
private function typeValue(value:Ast.ValueSpec, info:Info):Array<TypeDefinition> {
	...
	for (i in 0...value.names.length) {
		...
		if (value.values[i] == null) {
			if (type != null) {
				expr = defaultValue(typeof(value.type));
			} else {
				// last expr use
