		...
		if (setVar != "") {
			switch block.expr {
				case EBlock(exprs):
					...
					var defValue = defaultValue(assignType,info);
					var set = macro $assign == null ? $defValue : $assign.__underlying__();
					if (types.length == 1) {
						type = toComplexType(types[0], info);
						defValue = defaultValue(types[0],info);
...
private function toReflectType(t:GoType, info:Info, paths:Array<String>):Expr {
	return switch t {
		case typeParam(name, params):
			...
			final params = macro [];
			macro stdgo.internal.reflect.Reflect.GoType.typeParam($name, params);
		case refType(elem):
			final elem = toReflectType(elem, info, paths.copy());
			macro stdgo.internal.reflect.Reflect.GoType.refType($elem);
		case mapType(_.get() => key, _.get() => value):
...
function compositeLit(type:GoType, ct:ComplexType, expr:Ast.CompositeLit, info:Info):ExprDef {
	...
	switch underlying {
		...
		case mapType(_.get() => var keyType, _.get() => valueType):
			...
			if (params.length == 0) {
				return (macro new stdgo.GoMap.GoObjectMap<$keyType, $valueType>(new stdgo.internal.reflect.Reflect._Type($t))).expr;
			}
			return (macro {
