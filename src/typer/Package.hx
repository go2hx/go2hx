...
function typePackageEmit(pkg:typer.Package.IntermediatePackageType):HaxeAst.Module {
	...
	for (file in module.files) {
		...
		for (def in defs) {
			...
			var restrictedNames = [
				for (func in local) typer.exprs.Ident.nameIdent(func.func.name.name, false, false, info)
						...
						switch kind {
							case FFun(fun):
								...
								if (codegen.Patch.funcInline.indexOf(patchName) != -1 && access.indexOf(AInline) == -1)
									access.push(AInline);
								// recv func named
								HaxeAst.addLocalMethod(func.name, func.pos, func.meta, func.doc, access, fun, staticExtension, wrapper, true, def.params != null
									&& def.params.length > 0);
							default:
						}
					default:
