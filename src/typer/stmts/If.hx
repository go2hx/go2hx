...
function typeIfStmt(stmt:GoAst.IfStmt, info:Info):ExprDef {
	...
	var ifStmt:Expr = toExpr(EIf(typer.exprs.Expr.typeExpr(stmt.cond, info), typer.stmts.Stmt.typeStmt(stmt.body, info),
		typer.stmts.Stmt.typeStmt(stmt.elseStmt, info)));
	if (init != null)
