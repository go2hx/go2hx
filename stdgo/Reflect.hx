...
class _Type implements StructType implements Type {
	...
	public function elem():Type {
		...
		switch (gt) {
			...
			default:
				throw stdgo.Errors.new_("reflect.Type.Elem not implemented for " + toString());
		}
	}
	...
	public function len():GoInt {
		switch (gt) {
			case arrayType(_, len):
				return (len : GoInt);
			default:
				throw stdgo.Errors.new_("reflect.Type.Len() not implemented for " + toString());
		}
	}
	...
	public function numMethod():GoInt {
		switch (gt) {
			...
			default:
				throw stdgo.Errors.new_("reflect.NumMethod not implemented for " + toString());
		}
		return 0;
	}
	...
	public function numField():GoInt {
		switch (gt) {
			...
			case structType(fields):
				return fields.length;
			default:
				throw stdgo.Errors.new_("reflect.NumField not implemented for " + toString());
		}
		return 0;
	}
...
@:named class StructTag implements StructType {
	...
	public function lookup(key:GoString):{value:GoString, ok:Bool} {
		...
		while (tag != (("" : GoString))) {
			...
			tag = tag.slice(i + ((1 : GoInt64)));
			if (key == name) {
				var __tmp__ = stdgo.Strconv.unquote(qvalue),
					value = __tmp__.v0,
					err = __tmp__.v1;
				if (err != null) {
					break;
