...
function testBlocking(_t:stdgo.Ref<stdgo._internal.testing.Testing_T_.T_>):Void {
        for (__4 => _enc in stdgo._internal.encoding.json.Json__blockingTests._blockingTests) {
            ...
            var _val__pointer__ = (stdgo.Go.setRef(_val) : stdgo.Ref<stdgo.AnyInterface>);
            var _val__pointer__ = (stdgo.Go.setRef(_val) : stdgo.Ref<stdgo.AnyInterface>);
            var _val__pointer__ = (stdgo.Go.setRef(_val) : stdgo.Ref<stdgo.AnyInterface>);
            var _val__pointer__ = (stdgo.Go.setRef(_val) : stdgo.Ref<stdgo.AnyInterface>);
            {
                var _err = (stdgo._internal.encoding.json.Json_newDecoder.newDecoder(_r).decode(stdgo.Go.toInterface(_val__pointer__)) : stdgo.Error);
                if (_err != null) {
