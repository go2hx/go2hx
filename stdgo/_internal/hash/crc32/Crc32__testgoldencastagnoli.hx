        ...
        for (__4 => _g in stdgo._internal.hash.crc32.Crc32__golden._golden) {
            {
                ...
                if (_crc != (_g._castagnoli)) {
                    @:check2r _t.errorf(("Castagnoli(%s) = 0x%x want 0x%x" : stdgo.GoString), stdgo.Go.toInterface(_g._in), stdgo.Go.toInterface(_crc), stdgo.Go.toInterface(_g._castagnoli));
                };
            };
        };
    }
