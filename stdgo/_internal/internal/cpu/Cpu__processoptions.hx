package stdgo._internal.internal.cpu;
function _processOptions(_env:stdgo.GoString):Void {
        var _o_8:stdgo._internal.internal.cpu.Cpu_t_option.T_option = ({} : stdgo._internal.internal.cpu.Cpu_t_option.T_option);
        var _i_5:stdgo.GoInt = (0 : stdgo.GoInt);
        var _enable_4:Bool = false;
        var _key_2:stdgo.GoString = ("" : stdgo.GoString);
        var _i_1:stdgo.GoInt = (0 : stdgo.GoInt);
        var _field_0:stdgo.GoString = ("" : stdgo.GoString);
        var _fieldBreak = false;
        var _i_24428_7:stdgo.GoInt = (0 : stdgo.GoInt);
        var _i_6:stdgo.GoInt = (0 : stdgo.GoInt);
        var _value_3:stdgo.GoString = ("" : stdgo.GoString);
        var _gotoNext = 0i32;
        var __blank__ = _gotoNext == ((0i32 : stdgo.GoInt));
        while (_gotoNext != ((-1i32 : stdgo.GoInt))) {
            {
                final __value__ = _gotoNext;
                if (__value__ == (0i32)) {
                    _gotoNext = 23453i32;
                } else if (__value__ == (23453i32)) {
                    var __blank__ = 0i32;
                    _fieldBreak = false;
                    _gotoNext = 23461i32;
                } else if (__value__ == (23461i32)) {
                    if (!_fieldBreak && (_env != (stdgo.Go.str() : stdgo.GoString))) {
                        _gotoNext = 23475i32;
                    } else {
                        _gotoNext = 24416i32;
                    };
                } else if (__value__ == (23475i32)) {
                    _field_0 = (stdgo.Go.str() : stdgo.GoString)?.__copy__();
                    _i_1 = stdgo._internal.internal.cpu.Cpu__indexbyte._indexByte(_env?.__copy__(), (44 : stdgo.GoUInt8));
                    if ((_i_1 < (0 : stdgo.GoInt) : Bool)) {
                        _gotoNext = 23529i32;
                    } else {
                        _gotoNext = 23564i32;
                    };
                } else if (__value__ == (23529i32)) {
                    {
                        final __tmp__0 = _env?.__copy__();
                        final __tmp__1 = (stdgo.Go.str() : stdgo.GoString)?.__copy__();
                        _field_0 = @:binopAssign __tmp__0;
                        _env = @:binopAssign __tmp__1;
                    };
                    _gotoNext = 23607i32;
                } else if (__value__ == (23564i32)) {
                    _gotoNext = 23564i32;
                    {
                        final __tmp__0 = (_env.__slice__(0, _i_1) : stdgo.GoString)?.__copy__();
                        final __tmp__1 = (_env.__slice__((_i_1 + (1 : stdgo.GoInt) : stdgo.GoInt)) : stdgo.GoString)?.__copy__();
                        _field_0 = @:binopAssign __tmp__0;
                        _env = @:binopAssign __tmp__1;
                    };
                    var __blank__ = 0i32;
                    _gotoNext = 23607i32;
                } else if (__value__ == (23607i32)) {
                    if ((((_field_0.length) < (4 : stdgo.GoInt) : Bool) || ((_field_0.__slice__(0, (4 : stdgo.GoInt)) : stdgo.GoString) != ("cpu." : stdgo.GoString)) : Bool)) {
                        _gotoNext = 23648i32;
                    } else {
                        _gotoNext = 23668i32;
                    };
                } else if (__value__ == (23648i32)) {
                    var __blank__ = 0i32;
                    _gotoNext = 23461i32;
                } else if (__value__ == (23668i32)) {
                    _i_1 = stdgo._internal.internal.cpu.Cpu__indexbyte._indexByte(_field_0?.__copy__(), (61 : stdgo.GoUInt8));
                    if ((_i_1 < (0 : stdgo.GoInt) : Bool)) {
                        _gotoNext = 23705i32;
                    } else {
                        _gotoNext = 23787i32;
                    };
                } else if (__value__ == (23705i32)) {
                    stdgo.Go.print(("GODEBUG: no value specified for \"" : stdgo.GoString), _field_0, ("\"\n" : stdgo.GoString));
                    var __blank__ = 0i32;
                    _gotoNext = 23461i32;
                } else if (__value__ == (23787i32)) {
                    {
                        final __tmp__0 = (_field_0.__slice__((4 : stdgo.GoInt), _i_1) : stdgo.GoString)?.__copy__();
                        final __tmp__1 = (_field_0.__slice__((_i_1 + (1 : stdgo.GoInt) : stdgo.GoInt)) : stdgo.GoString)?.__copy__();
                        _key_2 = @:binopAssign __tmp__0;
                        _value_3 = @:binopAssign __tmp__1;
                    };
                    _gotoNext = 23867i32;
                } else if (__value__ == (23867i32)) {
                    {
                        final __value__ = _value_3;
                        if (__value__ == (("on" : stdgo.GoString))) {
                            _gotoNext = 23884i32;
                        } else if (__value__ == (("off" : stdgo.GoString))) {
                            _gotoNext = 23914i32;
                        } else {
                            _gotoNext = 23946i32;
                        };
                    };
                } else if (__value__ == (23884i32)) {
                    _enable_4 = true;
                    _gotoNext = 24068i32;
                } else if (__value__ == (23914i32)) {
                    _enable_4 = false;
                    _gotoNext = 24068i32;
                } else if (__value__ == (23946i32)) {
                    stdgo.Go.print(("GODEBUG: value \"" : stdgo.GoString), _value_3, ("\" not supported for cpu option \"" : stdgo.GoString), _key_2, ("\"\n" : stdgo.GoString));
                    _gotoNext = 23461i32;
                } else if (__value__ == (24068i32)) {
                    if (_key_2 == (("all" : stdgo.GoString))) {
                        _gotoNext = 24084i32;
                    } else {
                        _gotoNext = 24207i32;
                    };
                } else if (__value__ == (24084i32)) {
                    if ((0i32 : stdgo.GoInt) < (stdgo._internal.internal.cpu.Cpu__options._options.length)) {
                        _gotoNext = 24179i32;
                    } else {
                        _gotoNext = 24185i32;
                    };
                } else if (__value__ == (24112i32)) {
                    stdgo._internal.internal.cpu.Cpu__options._options[(_i_5 : stdgo.GoInt)].specified = true;
                    stdgo._internal.internal.cpu.Cpu__options._options[(_i_5 : stdgo.GoInt)].enable = _enable_4;
                    _i_5++;
                    _gotoNext = 24180i32;
                } else if (__value__ == (24179i32)) {
                    _i_5 = 0i32;
                    _gotoNext = 24180i32;
                } else if (__value__ == (24180i32)) {
                    if (_i_5 < (stdgo._internal.internal.cpu.Cpu__options._options.length)) {
                        _gotoNext = 24112i32;
                    } else {
                        _gotoNext = 24185i32;
                    };
                } else if (__value__ == (24185i32)) {
                    _gotoNext = 23461i32;
                } else if (__value__ == (24207i32)) {
                    if ((0i32 : stdgo.GoInt) < (stdgo._internal.internal.cpu.Cpu__options._options.length)) {
                        _gotoNext = 24351i32;
                    } else {
                        _gotoNext = 24357i32;
                    };
                } else if (__value__ == (24211i32)) {
                    _i_6++;
                    _gotoNext = 24352i32;
                } else if (__value__ == (24230i32)) {
                    if (stdgo._internal.internal.cpu.Cpu__options._options[(_i_6 : stdgo.GoInt)].name == (_key_2)) {
                        _gotoNext = 24261i32;
                    } else {
                        _gotoNext = 24211i32;
                    };
                } else if (__value__ == (24261i32)) {
                    stdgo._internal.internal.cpu.Cpu__options._options[(_i_6 : stdgo.GoInt)].specified = true;
                    stdgo._internal.internal.cpu.Cpu__options._options[(_i_6 : stdgo.GoInt)].enable = _enable_4;
                    _gotoNext = 23461i32;
                } else if (__value__ == (24351i32)) {
                    _i_6 = 0i32;
                    _gotoNext = 24352i32;
                } else if (__value__ == (24352i32)) {
                    if (_i_6 < (stdgo._internal.internal.cpu.Cpu__options._options.length)) {
                        _gotoNext = 24230i32;
                    } else {
                        _gotoNext = 24357i32;
                    };
                } else if (__value__ == (24357i32)) {
                    stdgo.Go.print(("GODEBUG: unknown cpu feature \"" : stdgo.GoString), _key_2, ("\"\n" : stdgo.GoString));
                    _gotoNext = 23461i32;
                } else if (__value__ == (24416i32)) {
                    if ((0i32 : stdgo.GoInt) < (stdgo._internal.internal.cpu.Cpu__options._options.length)) {
                        _gotoNext = 24629i32;
                    } else {
                        _gotoNext = 24632i32;
                    };
                } else if (__value__ == (24442i32)) {
                    _o_8 = stdgo._internal.internal.cpu.Cpu__options._options[(_i_24428_7 : stdgo.GoInt)];
                    if (!_o_8.specified) {
                        _gotoNext = 24462i32;
                    } else {
                        _gotoNext = 24483i32;
                    };
                } else if (__value__ == (24462i32)) {
                    _i_24428_7++;
                    _gotoNext = 24630i32;
                } else if (__value__ == (24483i32)) {
                    if ((_o_8.enable && !_o_8.feature.value : Bool)) {
                        _gotoNext = 24510i32;
                    } else {
                        _gotoNext = 24607i32;
                    };
                } else if (__value__ == (24510i32)) {
                    stdgo.Go.print(("GODEBUG: can not enable \"" : stdgo.GoString), _o_8.name, ("\", missing CPU support\n" : stdgo.GoString));
                    _i_24428_7++;
                    _gotoNext = 24630i32;
                } else if (__value__ == (24607i32)) {
                    _o_8.feature.value = _o_8.enable;
                    _i_24428_7++;
                    _gotoNext = 24630i32;
                } else if (__value__ == (24629i32)) {
                    {
                        final __tmp__0 = 0i32;
                        final __tmp__1 = stdgo._internal.internal.cpu.Cpu__options._options[(0i32 : stdgo.GoInt)];
                        _i_24428_7 = @:binopAssign __tmp__0;
                        _o_8 = @:binopAssign __tmp__1;
                    };
                    _gotoNext = 24630i32;
                } else if (__value__ == (24630i32)) {
                    if (_i_24428_7 < (stdgo._internal.internal.cpu.Cpu__options._options.length)) {
                        _gotoNext = 24442i32;
                    } else {
                        _gotoNext = 24632i32;
                    };
                } else if (__value__ == (24632i32)) {
                    _gotoNext = -1i32;
                };
            };
        };
    }
