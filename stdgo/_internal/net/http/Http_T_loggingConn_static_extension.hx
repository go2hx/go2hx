...
@:keep @:allow(stdgo._internal.net.http.Http.T_loggingConn_asInterface) class T_loggingConn_static_extension {
    ...
    static public function close( _c:stdgo.Ref<stdgo._internal.net.http.Http_T_loggingConn.T_loggingConn>):stdgo.Error {
        ...
        stdgo._internal.log.Log_printf.printf(("%s.Close() = ..." : stdgo.GoString), stdgo.Go.toInterface((@:checkr _c ?? throw "null pointer dereference")._name));
        _err = (@:checkr _c ?? throw "null pointer dereference").conn.close();
        stdgo._internal.log.Log_printf.printf(("%s.Close() = %v" : stdgo.GoString), stdgo.Go.toInterface((@:checkr _c ?? throw "null pointer dereference")._name), stdgo.Go.toInterface(_err));
        return _err;
    }
    @:keep
    @:tdfield
