package stdgo._internal.regexp;
function testReplaceAll(_t:stdgo.Ref<stdgo._internal.testing.Testing_t_.T_>):Void {
        //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L265"
        for (__0 => _tc in stdgo._internal.regexp.Regexp__replacetests._replaceTests) {
            var __tmp__ = stdgo._internal.regexp.Regexp_compile.compile(_tc._pattern?.__copy__()), _re:stdgo.Ref<stdgo._internal.regexp.Regexp_regexp.Regexp> = __tmp__._0, _err:stdgo.Error = __tmp__._1;
            //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L267"
            if (_err != null) {
                //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L268"
                _t.errorf(("Unexpected error compiling %q: %v" : stdgo.GoString), stdgo.Go.toInterface(_tc._pattern), stdgo.Go.toInterface(_err));
                //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L269"
                continue;
            };
            var _actual = (_re.replaceAllString(_tc._input?.__copy__(), _tc._replacement?.__copy__())?.__copy__() : stdgo.GoString);
            //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L272"
            if (_actual != (_tc._output)) {
                //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L273"
                _t.errorf(("%q.ReplaceAllString(%q,%q) = %q; want %q" : stdgo.GoString), stdgo.Go.toInterface(_tc._pattern), stdgo.Go.toInterface(_tc._input), stdgo.Go.toInterface(_tc._replacement), stdgo.Go.toInterface(_actual), stdgo.Go.toInterface(_tc._output));
            };
            _actual = (_re.replaceAll((_tc._input : stdgo.Slice<stdgo.GoUInt8>), (_tc._replacement : stdgo.Slice<stdgo.GoUInt8>)) : stdgo.GoString)?.__copy__();
            //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L278"
            if (_actual != (_tc._output)) {
                //"file:///home/runner/.go/go1.21.3/src/regexp/all_test.go#L279"
                _t.errorf(("%q.ReplaceAll(%q,%q) = %q; want %q" : stdgo.GoString), stdgo.Go.toInterface(_tc._pattern), stdgo.Go.toInterface(_tc._input), stdgo.Go.toInterface(_tc._replacement), stdgo.Go.toInterface(_actual), stdgo.Go.toInterface(_tc._output));
            };
        };
    }
