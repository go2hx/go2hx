...
function testNew(_t:stdgo.Ref<stdgo._internal.testing.Testing_T_.T_>):Void {
        ...
        if ((@:checkr _t1 ?? throw "null pointer dereference")._common != ((@:checkr _t2 ?? throw "null pointer dereference")._common)) {
            @:check2r _t.errorf(("t1 & t2 didn\'t share common struct; got %v != %v" : stdgo.GoString), stdgo.Go.toInterface((@:checkr _t1 ?? throw "null pointer dereference")._common), stdgo.Go.toInterface((@:checkr _t2 ?? throw "null pointer dereference")._common));
        };
