...
@:structInit class T_hmac {
    ...
    public function new(?_opad:Slice<GoUInt8>, ?_ipad:Slice<GoUInt8>, ?_outer:stdgo.hash.Hash.Hash, ?_inner:stdgo.hash.Hash.Hash, ?_marshaled:Bool) stdgo.internal.Macro.initLocals();
    public function toString() {
        return '{' + Go.string(_opad) + " " + Go.string(_ipad) + " " + Go.string(_outer) + " " + Go.string(_inner) + " " + Go.string(_marshaled) + "}";
    }
