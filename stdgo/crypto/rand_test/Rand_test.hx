...
@:structInit @:using(stdgo.crypto.rand_test.Rand_test.T_countingReader_static_extension) private class T_countingReader {
...
function exampleRead():Void {
	...
	var __tmp__ = stdgo.crypto.rand.Rand.read(_b),
		_0:GoInt = __tmp__._0,
	...
	stdgo.fmt.Fmt.println(Go.toInterface(stdgo.bytes.Bytes.equal(_b,
		new Slice<GoUInt8>((_c : GoInt).toBasic(), 0, ...[for (i in 0...(_c : GoInt).toBasic()) (0 : GoUInt8)]))));
}
...
function testPrimeSmall(_t:Ref<stdgo.testing.Testing.T>):Void {
	{
		...
		Go.cfor(_n < (10:GoInt), _n++, {
			var __tmp__ = stdgo.crypto.rand.Rand.prime(stdgo.crypto.rand.Rand.reader, _n),
				_p:Ref<stdgo.math.big.Big.Int_> = __tmp__._0,
				_err:Error = __tmp__._1;
			if (_err != null) {
...
function testPrimeBitsLt2(_t:Ref<stdgo.testing.Testing.T>):Void {
	{
		var __tmp__ = stdgo.crypto.rand.Rand.prime(stdgo.crypto.rand.Rand.reader, (1 : GoInt)),
			_p:Ref<stdgo.math.big.Big.Int_> = __tmp__._0,
			_err:Error = __tmp__._1;
...
function testPrimeNondeterministic(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	var __tmp__ = stdgo.crypto.rand.Rand.prime(Go.asInterface(_r), (32 : GoInt)),
		_p0:Ref<stdgo.math.big.Big.Int_> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
...
function testInt(_t:Ref<stdgo.testing.Testing.T>):Void {
	{
		...
		Go.cfor(_n < (140:GoInt), _n++, {
			...
			{
				var __tmp__ = stdgo.crypto.rand.Rand.int_(stdgo.crypto.rand.Rand.reader, _b),
					_i:Ref<stdgo.math.big.Big.Int_> = __tmp__._0,
					_err:Error = __tmp__._1;
				if (_err != null) {
...
function testIntReads(_t:Ref<stdgo.testing.Testing.T>):Void {
	{
		...
		Go.cfor(_i < (32:GoInt), _i++, {
			...
			_t.run(stdgo.fmt.Fmt.sprintf(("max=%d" : GoString), Go.toInterface(_max)), function(_t:Ref<stdgo.testing.Testing.T>):Void {
				...
				var __tmp__ = stdgo.crypto.rand.Rand.int_(Go.asInterface(_reader), stdgo.math.big.Big.newInt(_max)),
				...
				if (_reader._n != (_expected)) {
					_t.errorf(("Int(reader, %d) should read %d bytes, but it read: %d" : GoString), Go.toInterface(_max), Go.toInterface(_expected),
						Go.toInterface(_reader._n));
				};
			});
		});
	};
			...
			_t.run(stdgo.fmt.Fmt.sprintf(("max=%d" : GoString), Go.toInterface(_max)), function(_t:Ref<stdgo.testing.Testing.T>):Void {
				{
					...
					Go.cfor(_i < _max, _i++, {
						...
						var __tmp__ = stdgo.crypto.rand.Rand.int_(Go.asInterface((_b : Ref<stdgo.bytes.Bytes.Buffer>)),
							stdgo.math.big.Big.newInt((_max : GoInt64))),
							_n:Ref<stdgo.math.big.Big.Int_> = __tmp__._0,
							_err:Error = __tmp__._1;
						if (_err != null) {
							_t.fatalf(("Can\'t generate random value: %d, %v" : GoString), Go.toInterface(_max), Go.toInterface(_err));
						};
						if (_n.int64() != ((_i : GoInt64))) {
							_t.errorf(("Int(reader, %d) should have returned value of %d, but it returned: %v" : GoString), Go.toInterface(_max),
								Go.toInterface(_i), Go.toInterface(Go.asInterface(_n)));
						};
					});
				};
			});
