...
@:structInit class T_encoder {
	...
	public function new(?_err:stdgo.Error, ?_w:stdgo.io.Io.Writer, ?_buf:GoArray<GoUInt8>, ?_nbuf:GoInt, ?_out:GoArray<GoUInt8>)
...
@:structInit class T_decoder {
	public function read(_p:Slice<GoByte>):{var _0:GoInt; var _1:Error;} {
		...
		while (true) {
			...
			if (_d.value._nbuf > ((0 : GoInt))) {
				{
					var __tmp__ = decode(_d.value._outbuf.__slice__(((0 : GoInt))), _d.value._buf.__slice__(((0 : GoInt)), _d.value._nbuf),
						_d.value._readErr != null);
					_ndst = __tmp__._0;
					_nsrc = __tmp__._1;
	...
	public function new(?_err:stdgo.Error, ?_readErr:stdgo.Error, ?_r:stdgo.io.Io.Reader, ?_buf:GoArray<GoUInt8>, ?_nbuf:GoInt, ?_out:Slice<GoUInt8>,
			?_outbuf:GoArray<GoUInt8>)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(_err) + " " + Go.string(_readErr) + " " + Go.string(_r) + " " + Go.string(_buf) + " " + Go.string(_nbuf) + " "
...
function newDecoder(_r:stdgo.io.Io.Reader):stdgo.io.Io.Reader {
	return Go.pointer((({
		...
		_outbuf: new GoArray<GoUInt8>(...[for (i in 0...1024) ((0 : GoUInt8))])
	} : T_decoder))).value;
}
