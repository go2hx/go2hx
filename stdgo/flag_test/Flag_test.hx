...
function testChangingArgs(_t:stdgo.testing.Testing.T):Void {
        ...
        try {
            ...
            parse();
            var _args = args();
            if (((!_before.value || _cmd != (Go.str("subcmd") : GoString)) || !_after.value || (_args.length) != (1 : GoInt)) || (_args[(0 : GoInt)] != (Go.str("args") : GoString))) {
...
function testExitCode(_t:stdgo.testing.Testing.T):Void {
        ...
        for (_0 => _test in _tests) {
            ...
            _cmd.env = (stdgo.os.Os.environ().__append__((Go.str("GO_CHILD_FLAG=") : GoString) + _test._flag, (Go.str("GO_CHILD_FLAG_HANDLE=") : GoString) + _test._flagHandle));
            _cmd.run();
