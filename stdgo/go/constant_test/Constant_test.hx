...
function example_complexNumbers():Void {
	...
	var __tmp__ = stdgo.go.constant.Constant.float64Val(stdgo.go.constant.Constant.real(_c)),
		ar:GoFloat64 = __tmp__._0,
	...
	var __tmp__ = stdgo.go.constant.Constant.float64Val(stdgo.go.constant.Constant.imag(_c)),
		ai:GoFloat64 = __tmp__._0,
		_exact:Bool = __tmp__._1;
	if (!_exact) {
		stdgo.fmt.Fmt.printf(("Could not represent imaginary part %s as exactly as float64\n" : GoString),
			Go.toInterface(stdgo.go.constant.Constant.imag(_c)));
	};
...
function exampleUnaryOp():Void {
	var _vs = (new Slice<Value>(0, 0, stdgo.go.constant.Constant.makeBool(true), stdgo.go.constant.Constant.makeFloat64((2.7 : GoFloat64)),
		stdgo.go.constant.Constant.makeUint64(("42" : GoUInt64))) : Slice<Value>);
	for (_i => _v in _vs) {
		if (_v.kind() == ((1 : Kind))) {
...
function exampleCompare():Void {
	var _vs = (new Slice<Value>(0, 0, stdgo.go.constant.Constant.makeString(("Z" : GoString)), stdgo.go.constant.Constant.makeString(("bacon" : GoString)),
		stdgo.go.constant.Constant.makeString(("go" : GoString)), stdgo.go.constant.Constant.makeString(("Frame" : GoString)),
		stdgo.go.constant.Constant.makeString(("defer" : GoString)),
		stdgo.go.constant.Constant.makeFromLiteral(("\"a\"" : GoString), (9 : stdgo.go.token.Token.Token), ("0" : GoUInt))) : Slice<Value>);
...
function exampleSign():Void {
	...
	var _vs = (new Slice<Value>(0, 0, _negOne, _mkComplex(_zero, _negOne), _mkComplex(_one, _negOne), _mkComplex(_negOne, _one), _mkComplex(_negOne, _negOne),
		_zero, _mkComplex(_zero, _zero), _one, _mkComplex(_zero, _one), _mkComplex(_one, _one)) : Slice<Value>);
	for (_0 => _v in _vs) {
