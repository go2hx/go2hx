...
@:structInit @:using(stdgo.go.importer.Importer.T_gcimports_static_extension) private class T_gcimports {
	...
	public var _lookup:Lookup = (null : Lookup);

	public function new(?_fset:Ref<stdgo.go.token.Token.FileSet>, ?_packages:GoMap<GoString, Ref<stdgo.go.types.Types.Package>>, ?_lookup:Lookup) {
		if (_fset != null)
			this._fset = _fset;
		if (_packages != null)
...
@:structInit @:using(stdgo.go.importer.Importer.T_gccgoimports_static_extension) private class T_gccgoimports {
	public var _packages:GoMap<GoString, Ref<stdgo.go.types.Types.Package>> = (null : GoMap<GoString, Ref<stdgo.go.types.Types.Package>>);
	public var _importer:go.internal.gccgoimporter.Gccgoimporter.Importer = (null : go.internal.gccgoimporter.Gccgoimporter.Importer);
	public var _lookup:Lookup = (null : Lookup);

	public function new(?_packages:GoMap<GoString, Ref<stdgo.go.types.Types.Package>>, ?_importer:go.internal.gccgoimporter.Gccgoimporter.Importer,
			?_lookup:Lookup) {
		if (_packages != null)
			this._packages = _packages;
		if (_importer != null)
...
function forCompiler(_fset:Ref<stdgo.go.token.Token.FileSet>, _compiler:GoString, _lookup:Lookup):stdgo.go.types.Types.Importer {
	if (_compiler == (("gc" : GoString))) {
		return Go.asInterface((({_fset: _fset, _packages: (new GoObjectMap<GoString,
			...
			{get: () -> stdgo.internal.reflect.Reflect.GoType.refType({get: () -> stdgo.internal.reflect.Reflect.GoType.named("stdgo.go.types.Types.Package",
				[], null, false,
				{get: () -> null})})}))) : GoMap<GoString, Ref<stdgo.go.types.Types.Package>>), _lookup: _lookup} : T_gcimports) : Ref<T_gcimports>));
	} else if (_compiler == (("gccgo" : GoString))) {
		var _inst:go.internal.gccgoimporter.Gccgoimporter.GccgoInstallation = ({} : go.internal.gccgoimporter.Gccgoimporter.GccgoInstallation);
		{
		...
		return Go.asInterface((({_packages: (new GoObjectMap<GoString,
			Ref<stdgo.go.types.Types.Package>>(new stdgo.internal.reflect.Reflect._Type(stdgo.internal.reflect.Reflect.GoType.mapType({
			get: () -> stdgo.internal.reflect.Reflect.GoType.basic(string_kind)
		},
			{get: () -> stdgo.internal.reflect.Reflect.GoType.refType({get: () -> stdgo.internal.reflect.Reflect.GoType.named("stdgo.go.types.Types.Package",
				[], null, false,
				{get: () -> null})})}))) : GoMap<GoString, Ref<stdgo.go.types.Types.Package>>), _importer: _inst.getImporter((null : Slice<GoString>),
				(null : GoMap<Ref<stdgo.go.types.Types.Package>,
					go.internal.gccgoimporter.Gccgoimporter.InitData>)), _lookup: _lookup} : T_gccgoimports) : Ref<T_gccgoimports>));
	} else if (_compiler == (("source" : GoString))) {
		if (_lookup != null) {
			throw Go.toInterface(("source importer for custom import path lookup not supported (issue #13847)." : GoString));
		};
		return Go.asInterface(go.internal.srcimporter.Srcimporter.new_((stdgo.go.build.Build.default_ : Ref<stdgo.go.build.Build.Context>), _fset,
			(new GoObjectMap<GoString,
				Ref<stdgo.go.types.Types.Package>>(new stdgo.internal.reflect.Reflect._Type(stdgo.internal.reflect.Reflect.GoType.mapType({
				get: () -> stdgo.internal.reflect.Reflect.GoType.basic(string_kind)
			},
				{get: () -> stdgo.internal.reflect.Reflect.GoType.refType({get: () ->
						stdgo.internal.reflect.Reflect.GoType.named("stdgo.go.types.Types.Package", [], null, false, {get: () -> null})})}))) : GoMap<GoString,
...
function testForCompiler(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	{};
	var __tmp__ = stdgo.os.exec.Exec.command(stdgo.internal.testenv.Testenv.goToolPath(Go.asInterface(_t)), ("list" : GoString),
		("-f={{context.Compiler}}:{{.Target}}" : GoString), ("math/big" : GoString))
		.combinedOutput(),
	...
	var _target:GoString = stdgo.strings.Strings.trimSpace((_out : GoString));
	var __tmp__ = stdgo.strings.Strings.cut(_target, (":" : GoString)),
		_compiler:GoString = __tmp__._0,
		_target:GoString = __tmp__._1,
		_0:Bool = __tmp__._2;
	if (!stdgo.strings.Strings.hasSuffix(_target, (".a" : GoString))) {
		_t.fatalf(("unexpected package %s target %q (not *.a)" : GoString), Go.toInterface(("math/big" : GoString)), Go.toInterface(_target));
	};
	...
	_t.run(("LookupDefault" : GoString), function(_t:Ref<stdgo.testing.Testing.T>):Void {
		...
		var __tmp__ = _imp.import_(("math/big" : GoString)),
		...
		var _posn:stdgo.go.token.Token.Position = (_fset.position(_mathBigInt.pos()) == null ? null : _fset.position(_mathBigInt.pos()).__copy__());
		var _filename:GoString = stdgo.strings.Strings.replace(_posn.filename, ("$$GOROOT" : GoString),
			stdgo.internal.testenv.Testenv.goroot(Go.asInterface(_t)), (1 : GoInt));
		...
		if ((_posn.line > _lines.length)
			|| !stdgo.strings.Strings.hasPrefix(_lines[(_posn.line - (1 : GoInt):GoInt)], ("type Int" : GoString))) {
			_t.fatalf(("Object %v position %s does not contain its declaration" : GoString), Go.toInterface(_mathBigInt),
				Go.toInterface(Go.asInterface(_posn)));
		};
	});
	_t.run(("LookupCustom" : GoString), function(_t:Ref<stdgo.testing.Testing.T>):Void {
		if (internal.buildcfg.Buildcfg.experiment.flags.unified) {
			_t.skip(Go.toInterface(("not supported by GOEXPERIMENT=unified; see go.dev/cl/406319" : GoString)));
		};
		var _lookup:GoString -> {
			var _0:stdgo.io.Io.ReadCloser;
			var _1:Error;
		} = function(_path:GoString):{var _0:stdgo.io.Io.ReadCloser; var _1:Error;} {
		...
		var __tmp__ = _imp.import_(("math/bigger" : GoString)),
			_pkg:Ref<stdgo.go.types.Types.Package> = __tmp__._0,
			_err:Error = __tmp__._1;
		if (_err != null) {
			_t.fatal(Go.toInterface(_err));
...
class T_gcimports_asInterface {
	@:keep
	public function importFrom(_path:GoString, _srcDir:GoString,
			_mode:stdgo.go.types.Types.ImportMode):{var _0:Ref<stdgo.go.types.Types.Package>; var _1:Error;}
		return __self__.value.importFrom(_path, _srcDir, _mode);

	@:keep
	public function import_(_path:GoString):{var _0:Ref<stdgo.go.types.Types.Package>; var _1:Error;}
...
@:keep @:allow(stdgo.go.importer.Importer.T_gcimports_asInterface) class T_gcimports_static_extension {
	@:keep
	static public function importFrom(_m:Ref<T_gcimports>, _path:GoString, _srcDir:GoString,
			_mode:stdgo.go.types.Types.ImportMode):{var _0:Ref<stdgo.go.types.Types.Package>; var _1:Error;} {
		if (_mode != ((0 : stdgo.go.types.Types.ImportMode))) {
			throw Go.toInterface(("mode must be 0" : GoString));
		};
		return go.internal.gcimporter.Gcimporter.import_(_m._fset, _m._packages, _path, _srcDir, _m._lookup);
	}
...
@:keep @:allow(stdgo.go.importer.Importer.T_gccgoimports_asInterface) class T_gccgoimports_static_extension {
	@:keep
	static public function importFrom(_m:Ref<T_gccgoimports>, _path:GoString, _srcDir:GoString,
			_mode:stdgo.go.types.Types.ImportMode):{var _0:Ref<stdgo.go.types.Types.Package>; var _1:Error;} {
		if (_mode != ((0 : stdgo.go.types.Types.ImportMode))) {
			throw Go.toInterface(("mode must be 0" : GoString));
		};
