...
@:structInit @:using(go.internal.srcimporter.Srcimporter.Importer_static_extension) class Importer {
	...
	public function new(?_ctxt:Ref<stdgo.go.build.Build.Context>, ?_fset:Ref<stdgo.go.token.Token.FileSet>, ?_sizes:stdgo.go.types.Types.Sizes,
			?_packages:GoMap<GoString, Ref<stdgo.go.types.Types.Package>>) {
		if (_ctxt != null)
			this._ctxt = _ctxt;
		if (_fset != null)
			this._fset = _fset;
...
**/
function new_(_ctxt:Ref<stdgo.go.build.Build.Context>, _fset:Ref<stdgo.go.token.Token.FileSet>,
		_packages:GoMap<GoString, Ref<stdgo.go.types.Types.Package>>):Ref<Importer> {
	return (({
		_ctxt: _ctxt,
		_fset: _fset,
		_sizes: stdgo.go.types.Types.sizesFor(_ctxt.compiler, _ctxt.goarch),
		_packages: _packages
...
class Importer_asInterface {
	@:keep
	public function _joinPath(_elem:haxe.Rest<GoString>):GoString
		return __self__.value._joinPath(..._elem);

	@:keep
	public function _isAbsPath(_path:GoString):Bool
		return __self__.value._isAbsPath(_path);

	@:keep
	public function _absPath(_path:GoString):{var _0:GoString; var _1:Error;}
		return __self__.value._absPath(_path);

	@:keep
	public function _cgo(_bp:Ref<stdgo.go.build.Build.Package>):{var _0:Ref<stdgo.go.ast.Ast.File>; var _1:Error;}
		return __self__.value._cgo(_bp);

	@:keep
	public function _parseFiles(_dir:GoString, _filenames:Slice<GoString>):{var _0:Slice<Ref<stdgo.go.ast.Ast.File>>; var _1:Error;}
		return __self__.value._parseFiles(_dir, _filenames);
...
@:keep @:allow(go.internal.srcimporter.Srcimporter.Importer_asInterface) class Importer_static_extension {
	...
	static public function _cgo(_p:Ref<Importer>, _bp:Ref<stdgo.go.build.Build.Package>):{var _0:Ref<stdgo.go.ast.Ast.File>; var _1:Error;} {
		...
		var __tmp__ = stdgo.os.Os.mkdirTemp(Go.str(), ("srcimporter" : GoString)),
		...
		try {
			...
			if ((_bp.cgoPkgConfig.length) > (0 : GoInt)) {
				var _cmd = stdgo.os.exec.Exec.command(("pkg-config" : GoString),
					...((new Slice<GoString>(0, 0, ("--cflags" : GoString)) : Slice<GoString>).__append__(..._bp.cgoPkgConfig.__toArray__())).__toArray__());
			...
			};
			{
				...
				return stdgo.go.parser.Parser.parseFile(_p._fset, stdgo.path.filepath.Filepath.join(_tmpdir, ("_cgo_gotypes.go" : GoString)),
					(null : AnyInterface), (("0" : GoUInt) : stdgo.go.parser.Parser.Mode));
			};
			for (defer in __deferstack__) {
				defer();
			};
	...
	static public function _parseFiles(_p:Ref<Importer>, _dir:GoString, _filenames:Slice<GoString>):{var _0:Slice<Ref<stdgo.go.ast.Ast.File>>; var _1:Error;} {
		...
		var _files = new Slice<Ref<stdgo.go.ast.Ast.File>>((_filenames.length : GoInt).toBasic(), 0, ...[
			for (i in 0...(_filenames.length : GoInt).toBasic()) (null : Ref<stdgo.go.ast.Ast.File>)
		]);
			...
			Go.routine(() -> {
				var a = function(_i:GoInt, _filepath:GoString):Void {
					...
					try {
						...
						{
							var __tmp__ = stdgo.go.parser.Parser.parseFile(_p._fset, _filepath, Go.toInterface(_src),
	...
	**/
	@:keep
	static public function importFrom(_p:Ref<Importer>, _path:GoString, _srcDir:GoString,
			_mode:stdgo.go.types.Types.ImportMode):{var _0:Ref<stdgo.go.types.Types.Package>; var _1:Error;} {
		var __deferstack__:Array<Void->Void> = [];
		...
		{
			var __tmp__ = _p._absPath(_srcDir),
				_abs:GoString = __tmp__._0,
				_err:Error = __tmp__._1;
			if (_err == null) {
				_srcDir = _abs;
			};
		};
		var __tmp__ = _p._ctxt.import_(_path, _srcDir, (("0" : GoUInt) : stdgo.go.build.Build.ImportMode)),
			_bp:Ref<stdgo.go.build.Build.Package> = __tmp__._0,
			_err:Error = __tmp__._1;
		try {
			if (_err != null) {
				return {_0: null, _1: _err};
			...
			var __tmp__ = _p._parseFiles(_bp.dir, _filenames),
			...
			var _conf:stdgo.go.types.Types.Config = ({
				ignoreFuncBodies: true,
				error: function(_err:Error):Void {
					if ((_firstHardErr == null)
						&& !(Go.typeAssert((Go.toInterface(_err) : stdgo.go.types.Types.T_error)) : stdgo.go.types.Types.T_error).soft) {
						_firstHardErr = _err;
					};
				},
				importer: Go.asInterface(_p),
				sizes: _p._sizes
					...
					if (_err != null) {
						{
							...
							return {_0: null, _1: stdgo.fmt.Fmt.errorf(("error processing cgo for package %q: %w" : GoString), Go.toInterface(_bp.importPath),
								Go.toInterface(_err))};
						};
					};
			...
			if (_err != null) {
				...
				};
				{
					...
					return {_0: _pkg, _1: stdgo.fmt.Fmt.errorf(("type-checking package %q failed (%v)" : GoString), Go.toInterface(_bp.importPath),
						Go.toInterface(_err))};
				};
			};
			if (_firstHardErr != null) {
