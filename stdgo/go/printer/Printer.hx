...
private var _aNewline:Slice<GoUInt8> = (("\n" : GoString) : Slice<GoByte>);
private var _update:Pointer<Bool> = stdgo.flag.Flag.bool_(("update" : GoString), false, ("update golden files" : GoString));
private var _fset:Ref<stdgo.go.token.Token.FileSet> = stdgo.go.token.Token.newFileSet();

/**
	// Use go test -update to create/update the respective golden files.
**/
private var _data:Slice<T_entry> = (new Slice<T_entry>(0, 0,
...
private var _decls:Slice<GoString> = (new Slice<GoString>(0, 0, ("import \"fmt\"" : GoString),
	("const pi = 3.1415\nconst e = 2.71828\n\nvar x = pi" : GoString), ("func sum(x, y int) int\t{ return x + y }" : GoString)) : Slice<GoString>);

private var _stmts:Slice<GoString> = (new Slice<GoString>(0, 0, ("i := 0" : GoString), ("select {}\nvar a, b = 1, 2\nreturn a + b" : GoString),
...
**/
private final _noIndent:T_exprListMode = (("2" : GoUInt) : T_exprListMode);

private final _filteredMsg:GoString = ("contains filtered or unexported fields" : GoString);
private final _funcParam:T_paramMode = (2 : T_paramMode);
private final _funcTParam:T_paramMode = (2 : T_paramMode);
private final _typeTParam:T_paramMode = (2 : T_paramMode);
private var _testfile:Ref<stdgo.go.ast.Ast.File> = (null : Ref<stdgo.go.ast.Ast.File>);
private var _testsize:GoInt64 = (0 : GoInt64);

/**
	// max. number of newlines between source text
**/
private final _maxNewlines:GoUInt64 = ("2" : GoUInt64);
...
private final _infinity:GoUInt64 = ("1073741824" : GoUInt64);
private final _ignore:T_whiteSpace = (0 : T_whiteSpace);
private final _blank:T_whiteSpace = (32 : T_whiteSpace);
private final _vtab:T_whiteSpace = (11 : T_whiteSpace);
private final _newline:T_whiteSpace = (10 : T_whiteSpace);
private final _formfeed:T_whiteSpace = (12 : T_whiteSpace);
private final _indent:T_whiteSpace = (62 : T_whiteSpace);
private final _unindent:T_whiteSpace = (60 : T_whiteSpace);
...
**/
private final _noExtraLinebreak:T_pmode = (2 : T_pmode);

/**
	// inside space// trimmer is implemented as a state machine.
	// It can be in one of the following states:
**/
private final _inSpace:GoUInt64 = ("2" : GoUInt64);

/**
	// inside text bracketed by tabwriter.Escapes// trimmer is implemented as a state machine.
	// It can be in one of the following states:
**/
private final _inEscape:InvalidType = ("2" : GoUInt64);

/**
	// inside text// trimmer is implemented as a state machine.
	// It can be in one of the following states:
**/
private final _inText:InvalidType = ("2" : GoUInt64);
...
**/
final tabIndent:Mode = (("8" : GoUInt) : Mode);

/**
	// use spaces instead of tabs for alignment
**/
final useSpaces:Mode = (("8" : GoUInt) : Mode);

/**
	// emit //line directives to preserve original source positions
**/
final sourcePos:Mode = (("8" : GoUInt) : Mode);
...
private final _dataDir:GoString = ("testdata" : GoString);
private final _tabwidth:GoUInt64 = ("8" : GoUInt64);
private final _export:T_checkMode = (("16" : GoUInt) : T_checkMode);
private final _rawFormat:T_checkMode = (("16" : GoUInt) : T_checkMode);
private final _normNumber:T_checkMode = (("16" : GoUInt) : T_checkMode);
private final _idempotent:T_checkMode = (("16" : GoUInt) : T_checkMode);
private final _allowTypeParams:T_checkMode = (("16" : GoUInt) : T_checkMode);
...
@:structInit private class T_commentInfo {
	...
	public function new(?_cindex:GoInt, ?_comment:Ref<stdgo.go.ast.Ast.CommentGroup>, ?_commentOffset:GoInt, ?_commentNewline:Bool) {
		...
		if (_commentOffset != null)
			this._commentOffset = _commentOffset;
		if (_commentNewline != null)
			this._commentNewline = _commentNewline;
	}
...
@:structInit @:using(stdgo.go.printer.Printer.T_printer_static_extension) private class T_printer {
	/**
	...
	**/
	...
	public function new(?config:Config, ?_fset:Ref<stdgo.go.token.Token.FileSet>, ?_output:Slice<GoUInt8>, ?_indent:GoInt, ?_level:GoInt, ?_mode:T_pmode,
			?_endAlignment:Bool, ?_impliedSemi:Bool, ?_lastTok:stdgo.go.token.Token.Token, ?_prevOpen:stdgo.go.token.Token.Token, ?_wsbuf:Slice<T_whiteSpace>,
			?_goBuild:Slice<GoInt>, ?_plusBuild:Slice<GoInt>, ?_pos:stdgo.go.token.Token.Position, ?_out:stdgo.go.token.Token.Position,
			?_last:stdgo.go.token.Token.Position, ?_linePtr:Pointer<GoInt>, ?_comments:Slice<Ref<stdgo.go.ast.Ast.CommentGroup>>, ?_useNodeComments:Bool,
			?_commentInfo:T_commentInfo, ?_nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>, ?_cachedPos:stdgo.go.token.Token.Pos, ?_cachedLine:GoInt) {
		if (config != null)
			this.config = config;
		if (_fset != null)
			this._fset = _fset;
		...
		if (_mode != null)
			this._mode = _mode;
		if (_endAlignment != null)
			this._endAlignment = _endAlignment;
		if (_impliedSemi != null)
			this._impliedSemi = _impliedSemi;
		if (_lastTok != null)
			this._lastTok = _lastTok;
		if (_prevOpen != null)
			this._prevOpen = _prevOpen;
		if (_wsbuf != null)
			this._wsbuf = _wsbuf;
		if (_goBuild != null)
			this._goBuild = _goBuild;
		if (_plusBuild != null)
			this._plusBuild = _plusBuild;
		if (_pos != null)
			this._pos = _pos;
		if (_out != null)
			this._out = _out;
		if (_last != null)
			this._last = _last;
		if (_linePtr != null)
			this._linePtr = _linePtr;
		if (_comments != null)
			this._comments = _comments;
		if (_useNodeComments != null)
			this._useNodeComments = _useNodeComments;
		if (_commentInfo != null)
			this._commentInfo = _commentInfo;
		if (_nodeSizes != null)
			this._nodeSizes = _nodeSizes;
		if (_cachedPos != null)
			this._cachedPos = _cachedPos;
		if (_cachedLine != null)
	...
	@:embedded
	public function fprint(_output:stdgo.io.Io.Writer, _fset_:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface):Error
		return config.fprint(_output, _fset_, _node);

	@:embedded
	public function _fprint(_output:stdgo.io.Io.Writer, _fset_:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface,
			_nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>):Error
		return config._fprint(_output, _fset_, _node, _nodeSizes);

	public function __copy__() {
		return new T_printer(config, _fset, _output, _indent, _level, _mode, _endAlignment, _impliedSemi, _lastTok, _prevOpen, _wsbuf, _goBuild, _plusBuild,
			_pos, _out, _last, _linePtr, _comments, _useNodeComments, _commentInfo, _nodeSizes, _cachedPos, _cachedLine);
	}
}
...
/**
	...
	// through unchanged.
**/
@:structInit @:using(stdgo.go.printer.Printer.T_trimmer_static_extension) private class T_trimmer {
...
@:structInit @:using(stdgo.go.printer.Printer.T_limitWriter_static_extension) private class T_limitWriter {
...
private function _formatDocComment(_list:Slice<Ref<stdgo.go.ast.Ast.Comment>>):Slice<Ref<stdgo.go.ast.Ast.Comment>> {
	...
	if ((_list.length == (1 : GoInt)) && stdgo.strings.Strings.hasPrefix(_list[(0 : GoInt)].text, ("/*" : GoString))) {
		...
		for (_0 => _c in _list) {
			if (!stdgo.strings.Strings.hasPrefix(_c.text, ("//" : GoString))) {
				return _list;
			};
			if (_isDirective((_c.text.__slice__((2 : GoInt)) : GoString))) {
				_directives = _directives.__appendref__(_c);
				continue;
			};
			_b.writeString(stdgo.strings.Strings.trimPrefix((_c.text.__slice__((2 : GoInt)) : GoString), (" " : GoString)));
			_b.writeString(("\n" : GoString));
		};
...
private function _isDirective(_c:GoString):Bool {
	if ((stdgo.strings.Strings.hasPrefix(_c, ("line " : GoString)) || stdgo.strings.Strings.hasPrefix(_c, ("extern " : GoString)))
		|| stdgo.strings.Strings.hasPrefix(_c, ("export " : GoString))) {
		return true;
	};
...
private function _appendLines(_x:Slice<GoByte>, _y:Slice<GoByte>):Slice<GoByte> {
	if (((_y.length > (0 : GoInt)) && _isNL(_y[(0 : GoInt)]))
		&& ((_x.length == (0 : GoInt))
			|| (((_x.length >= (2 : GoInt)) && _isNL(_x[((_x.length) - (1 : GoInt) : GoInt)]))
				&& _isNL(_x[((_x.length) - (2 : GoInt) : GoInt)])))) {
...
private function _isTypeElem(_x:stdgo.go.ast.Ast.Expr):Bool {
	{
		final __type__ = _x;
		if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.ArrayType>))
...
private function _walkBinary(_e:Ref<stdgo.go.ast.Ast.BinaryExpr>):{var _0:Bool; var _1:Bool; var _2:GoInt;} {
	var _has4:Bool = false,
		_has5:Bool = false,
		_maxProblem:GoInt = (0 : GoInt);
		...
		final __type__ = _e.x;
		{
			...
			while (__bool__) {
				...
				if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.BinaryExpr>))) {
					var _l:Ref<stdgo.go.ast.Ast.BinaryExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BinaryExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.BinaryExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BinaryExpr>) : __type__.__underlying__()
						.value;
					if (_l.op.precedence() < _e.op.precedence()) {
						break;
					};
					var __tmp__ = _walkBinary(_l),
						_h4:Bool = __tmp__._0,
						_h5:Bool = __tmp__._1,
						_mp:GoInt = __tmp__._2;
					_has4 = _has4 || _h4;
					_has5 = _has5 || _h5;
					if (_maxProblem < _mp) {
						_maxProblem = _mp;
	...
	};
	{
		final __type__ = _e.y;
		if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.BinaryExpr>))) {
			var _r:Ref<stdgo.go.ast.Ast.BinaryExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BinaryExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.BinaryExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BinaryExpr>) : __type__.__underlying__()
				.value;
			if (_r.op.precedence() <= _e.op.precedence()) {
				break;
			};
			var __tmp__ = _walkBinary(_r),
				_h4:Bool = __tmp__._0,
				_h5:Bool = __tmp__._1,
				_mp:GoInt = __tmp__._2;
			_has4 = _has4 || _h4;
			...
			var _r:Ref<stdgo.go.ast.Ast.StarExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.StarExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.StarExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.StarExpr>) : __type__.__underlying__()
				.value;
			if (_e.op == ((15 : stdgo.go.token.Token.Token))) {
				_maxProblem = (5 : GoInt);
			};
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.UnaryExpr>))) {
			var _r:Ref<stdgo.go.ast.Ast.UnaryExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.UnaryExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.UnaryExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.UnaryExpr>) : __type__.__underlying__()
				.value;
			if ((_e.op.string() : GoString)
				+ (_r.op.string() : GoString) == (("/*" : GoString))
					|| (_e.op.string() : GoString)
						+ (_r.op.string() : GoString) == (("&&" : GoString))
							|| (_e.op.string() : GoString) + (_r.op.string() : GoString) == (("&^" : GoString))) {
				_maxProblem = (5 : GoInt);
			} else if ((_e.op.string() : GoString)
				+ (_r.op.string() : GoString) == (("++" : GoString))
					|| (_e.op.string() : GoString) + (_r.op.string() : GoString) == (("--" : GoString))) {
...
private function _cutoff(_e:Ref<stdgo.go.ast.Ast.BinaryExpr>, _depth:GoInt):GoInt {
	var __tmp__ = _walkBinary(_e),
		_has4:Bool = __tmp__._0,
		_has5:Bool = __tmp__._1,
		_maxProblem:GoInt = __tmp__._2;
...
private function _normalizedNumber(_lit:Ref<stdgo.go.ast.Ast.BasicLit>):Ref<stdgo.go.ast.Ast.BasicLit> {
	if (((_lit.kind != (5 : stdgo.go.token.Token.Token)) && (_lit.kind != (6 : stdgo.go.token.Token.Token)))
		&& (_lit.kind != (7 : stdgo.go.token.Token.Token))) {
		return _lit;
	};
...
private function _isTypeName(_x:stdgo.go.ast.Ast.Expr):Bool {
	{
		...
		if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.Ident>))) {
			var _t:Ref<stdgo.go.ast.Ast.Ident> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.Ident>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.Ident>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.Ident>) : __type__.__underlying__()
				.value;
			return true;
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.SelectorExpr>))) {
			var _t:Ref<stdgo.go.ast.Ast.SelectorExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.SelectorExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.SelectorExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.SelectorExpr>) : __type__.__underlying__()
				.value;
...
private function _sanitizeImportPath(_lit:Ref<stdgo.go.ast.Ast.BasicLit>):Ref<stdgo.go.ast.Ast.BasicLit> {
	...
	var __tmp__ = stdgo.strconv.Strconv.unquote(_lit.value),
...
private function _testprint(_out:stdgo.io.Io.Writer, _file:Ref<stdgo.go.ast.Ast.File>):Void {
	{
		var _err:Error = (((new Config((("1073741830" : GoUInt) : Mode), (8 : GoInt),
			(0 : GoInt)) : Config) : Ref<Config>)).fprint(_out, _fset, Go.toInterface(Go.asInterface(_file)));
		if (_err != null) {
...
private function _initialize():Void {
	{};
	var __tmp__ = stdgo.os.Os.readFile(("testdata/parser.go" : GoString)),
		_src:Slice<GoUInt8> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
		stdgo.log.Log.fatalf(("%s" : GoString), Go.toInterface(_err));
	};
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, ("testdata/parser.go" : GoString), Go.toInterface(_src),
	...
	_testprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _file);
	...
	_testfile = _file;
	_testsize = (_src.length : GoInt64);
}

function benchmarkPrint(_b:Ref<stdgo.testing.Testing.B>):Void {
	if (_testfile == null) {
		_initialize();
	};
	_b.reportAllocs();
	_b.setBytes(_testsize);
	{
		var _i:GoInt = (0 : GoInt);
		Go.cfor(_i < _b.n, _i++, {
			_testprint(stdgo.io.Io.discard, _testfile);
		});
	};
}
...
private function _commonPrefix(_a:GoString, _b:GoString):GoString {
	var _i:GoInt = (0 : GoInt);
	while ((((_i < _a.length) && (_i < _b.length)) && (_a[(_i : GoInt)] == _b[(_i : GoInt)]))
		&& ((_a[(_i : GoInt)] <= (32 : GoUInt8)) || (_a[(_i : GoInt)] == (42 : GoUInt8)))) {
		_i++;
	};
...
private function _stripCommonPrefix(_lines:Slice<GoString>):Void {
	...
	{
		var __tmp__ = stdgo.strings.Strings.cut(_prefix, ("*" : GoString)),
	...
	var __tmp__ = stdgo.strings.Strings.cut(_last, _closing),
		_before:GoString = __tmp__._0,
		_1:GoString = __tmp__._1,
		_2:Bool = __tmp__._2;
	if (_isBlank(_before)) {
		if (_lineOfStars) {
			_closing = (" */" : GoString);
...
private function _getDoc(_n:stdgo.go.ast.Ast.Node):Ref<stdgo.go.ast.Ast.CommentGroup> {
	{
		...
		if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.Field>))) {
			var _n:Ref<stdgo.go.ast.Ast.Field> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.Field>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.Field>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.Field>) : __type__.__underlying__()
				.value;
			return _n.doc;
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.ImportSpec>))) {
			var _n:Ref<stdgo.go.ast.Ast.ImportSpec> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.ImportSpec>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.ImportSpec>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.ImportSpec>) : __type__.__underlying__()
				.value;
			return _n.doc;
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.ValueSpec>))) {
			var _n:Ref<stdgo.go.ast.Ast.ValueSpec> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.ValueSpec>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.ValueSpec>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.ValueSpec>) : __type__.__underlying__()
				.value;
			return _n.doc;
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.TypeSpec>))) {
			var _n:Ref<stdgo.go.ast.Ast.TypeSpec> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.TypeSpec>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.TypeSpec>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.TypeSpec>) : __type__.__underlying__()
				.value;
			return _n.doc;
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.GenDecl>))) {
			var _n:Ref<stdgo.go.ast.Ast.GenDecl> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.GenDecl>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.GenDecl>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.GenDecl>) : __type__.__underlying__()
				.value;
			return _n.doc;
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.FuncDecl>))) {
			var _n:Ref<stdgo.go.ast.Ast.FuncDecl> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.FuncDecl>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.FuncDecl>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.FuncDecl>) : __type__.__underlying__()
				.value;
			return _n.doc;
		} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.File>))) {
			var _n:Ref<stdgo.go.ast.Ast.File> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.File>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.File>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.File>) : __type__.__underlying__()
				.value;
			return _n.doc;
		};
	};
...
private function _format(_src:Slice<GoByte>, _mode:T_checkMode):{var _0:Slice<GoByte>; var _1:Error;} {
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(), Go.toInterface(_src), (("4" : GoUInt) : stdgo.go.parser.Parser.Mode)),
	...
	var _res = _buf.bytes();
	{
		var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(), Go.toInterface(_res), (("4" : GoUInt) : stdgo.go.parser.Parser.Mode)),
			_0:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
			_err:Error = __tmp__._1;
		if (_err != null) {
			return {_0: (null : Slice<GoUInt8>), _1: stdgo.fmt.Fmt.errorf(("re-parse: %s\n%s" : GoString), Go.toInterface(_err),
				Go.toInterface(_buf.bytes()))};
		};
	};
...
private function _runcheck(_t:Ref<stdgo.testing.Testing.T>, _source:GoString, _golden:GoString, _mode:T_checkMode):Void {
	var __tmp__ = stdgo.os.Os.readFile(_source),
		_src:Slice<GoUInt8> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
	...
	var __tmp__ = _format(_src, _mode),
		_res:Slice<GoUInt8> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
		_t.error(Go.toInterface(_err));
		return;
	...
	var __tmp__ = stdgo.os.Os.readFile(_golden),
		_gld:Slice<GoUInt8> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
		_t.error(Go.toInterface(_err));
		return;
...
function testLineComments(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(),
		Go.toInterface(("// comment 1\n\t// comment 2\n\t// comment 3\n\tpackage main\n\t" : GoString)), (("4" : GoUInt) : stdgo.go.parser.Parser.Mode)),
		_f:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
...
private function _testComment(_t:Ref<stdgo.testing.Testing.T>, _f:Ref<stdgo.go.ast.Ast.File>, _srclen:GoInt, _comment:Ref<stdgo.go.ast.Ast.Comment>):Void {
	...
	var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
	{
		...
		Go.cfor(_offs <= _srclen, _offs++, {
			...
			{
				var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(), Go.toInterface(_buf.bytes()), (("0" : GoUInt) : stdgo.go.parser.Parser.Mode)),
					_0:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
					_err:Error = __tmp__._1;
				if (_err != null) {
					_t.fatalf(("incorrect program for pos = %d:\n%s" : GoString), Go.toInterface(Go.asInterface(_comment.slash)),
						Go.toInterface((_buf.string() : GoString)));
				};
			};
			_comment.slash++;
		});
...
function testBadComments(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	_testComment(_t, _f,
		...
		(({
			slash: _pos,
			text: ("//-style comment" : GoString)
		} : stdgo.go.ast.Ast.Comment) : Ref<stdgo.go.ast.Ast.Comment>));
	_testComment(_t, _f,
		(("\n// first comment - text and position changed by test\npackage p\nimport \"fmt\"\nconst pi = 3.14 // rough circle\nvar (\n\tx, y, z int = 1, 2, 3\n\tu, v float64\n)\nfunc fibo(n int) {\n\tif n < 2 {\n\t\treturn n /* seed values */\n\t}\n\treturn fibo(n-1) + fibo(n-2)\n}\n" : GoString)
			.length),
		(({
...
function testSourcePos(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, ("src" : GoString),
		...
		(("4" : GoUInt) : stdgo.go.parser.Parser.Mode)),
		_f1:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
		_t.fatal(Go.toInterface(_err));
	};
	var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
	_err = ((({mode: (("12" : GoUInt) : Mode), tabwidth: (8 : GoInt)} : Config) : Ref<Config>)).fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)),
		_fset, Go.toInterface(Go.asInterface(_f1)));
	if (_err != null) {
		_t.fatal(Go.toInterface(_err));
	};
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(), Go.toInterface(_buf.bytes()), (("0" : GoUInt) : stdgo.go.parser.Parser.Mode)),
		_f2:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
...
function testIssue5945(_t:Ref<stdgo.testing.Testing.T>):Void {
	{};
	{};
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, ("src.go" : GoString),
		Go.toInterface(("\npackage p   // line 2\nfunc f() {} // line 3\n\nvar x, y, z int\n\n\nfunc g() { // line 8\n}\n" : GoString)),
	...
	if (_got != (("//line src.go:2\npackage p\n\n//line src.go:3\nfunc f() {}\n\nvar x, y, z int\n\n//line src.go:8\nfunc g() {\n}\n" : GoString))) {
		_t.errorf(("got:\n%s\nwant:\n%s\n" : GoString), Go.toInterface(_got),
			Go.toInterface(("//line src.go:2\npackage p\n\n//line src.go:3\nfunc f() {}\n\nvar x, y, z int\n\n//line src.go:8\nfunc g() {\n}\n" : GoString)));
	};
}

function testDeclLists(_t:Ref<stdgo.testing.Testing.T>):Void {
	for (_0 => _src in _decls) {
		var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(), Go.toInterface(("package p;" : GoString) + _src),
			(("4" : GoUInt) : stdgo.go.parser.Parser.Mode)),
			_file:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
			_err:Error = __tmp__._1;
		if (_err != null) {
...
function testStmtLists(_t:Ref<stdgo.testing.Testing.T>):Void {
	for (_0 => _src in _stmts) {
		var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(), Go.toInterface((("package p; func _() {" : GoString) + _src) + ("}" : GoString)),
			(("4" : GoUInt) : stdgo.go.parser.Parser.Mode)),
			_file:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
			_err:Error = __tmp__._1;
		if (_err != null) {
		...
		var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
		_err = fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _fset,
			Go.toInterface((Go.typeAssert((Go.toInterface(_file.decls[(0 : GoInt)]) : Ref<stdgo.go.ast.Ast.FuncDecl>)) : Ref<stdgo.go.ast.Ast.FuncDecl>)
				.body.list));
		if (_err != null) {
			throw Go.toInterface(_err);
		};
...
function testBaseIndent(_t:Ref<stdgo.testing.Testing.T>):Void {
	_t.parallel();
	{};
	var __tmp__ = stdgo.os.Os.readFile(("printer.go" : GoString)),
		_src:Slice<GoUInt8> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
		throw Go.toInterface(_err);
	};
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, ("printer.go" : GoString), Go.toInterface(_src), (("0" : GoUInt) : stdgo.go.parser.Parser.Mode)),
		_file:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
		throw Go.toInterface(_err);
	};
	{
		...
		Go.cfor(_indent < (4:GoInt), _indent++, {
			...
			_t.run(stdgo.fmt.Fmt.sprint(Go.toInterface(_indent)), function(_t:Ref<stdgo.testing.Testing.T>):Void {
				_t.parallel();
				var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
				((({tabwidth: (8 : GoInt), indent: _indent} : Config) : Ref<Config>)).fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _fset,
					Go.toInterface(Go.asInterface(_file)));
				...
				for (_i => _line in _lines) {
					...
					if (_n < _indent) {
						_t.errorf(("line %d: got only %d tabs; want at least %d: %q" : GoString), Go.toInterface(_i), Go.toInterface(_n),
							Go.toInterface(_indent), Go.toInterface(_line));
					};
				};
			});
		});
...
function testFuncType(_t:Ref<stdgo.testing.Testing.T>):Void {
	var _src = (({name: (({name: ("p" : GoString)} : stdgo.go.ast.Ast.Ident) : Ref<stdgo.go.ast.Ast.Ident>), decls: (new Slice<stdgo.go.ast.Ast.Decl>(0, 0,
		Go.asInterface((({name: (({name: ("f" : GoString)} : stdgo.go.ast.Ast.Ident) : Ref<stdgo.go.ast.Ast.Ident>),
			type: ((new stdgo.go.ast.Ast.FuncType() : stdgo.go.ast.Ast.FuncType) : Ref<stdgo.go.ast.Ast.FuncType>)} : stdgo.go.ast.Ast.FuncDecl) : Ref<stdgo.go.ast.Ast.FuncDecl>))) : Slice<stdgo.go.ast.Ast.Decl>)} : stdgo.go.ast.Ast.File) : Ref<stdgo.go.ast.Ast.File>);
	var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
	{
...
function testWriteErrors(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	};
	{
		...
		Go.cfor(_i < (20:GoInt), _i++, {
			...
			var _err:Error = ((({mode: (("1" : GoUInt) : Mode)} : Config) : Ref<Config>)).fprint(Go.asInterface(_lw), _fset,
				Go.toInterface(Go.asInterface(_file)));
...
function testX(_t:Ref<stdgo.testing.Testing.T>):Void {
	{};
	var __tmp__ = _format((("\npackage p\nfunc _() {}\n" : GoString) : Slice<GoByte>), (("0" : GoUInt) : T_checkMode)),
...
function testCommentedNode(_t:Ref<stdgo.testing.Testing.T>):Void {
	{};
	var _fset = stdgo.go.token.Token.newFileSet();
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, ("input.go" : GoString),
		Go.toInterface(("package main\n\nfunc foo() {\n\t// comment inside func\n}\n\n// leading comment\ntype bar int // comment2\n\n" : GoString)),
		(("4" : GoUInt) : stdgo.go.parser.Parser.Mode)),
		_f:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
	...
	var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
	_err = fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _fset,
		Go.toInterface((({node: Go.toInterface(_f.decls[(0 : GoInt)]), comments: _f.comments} : CommentedNode) : Ref<CommentedNode>)));
	if (_err != null) {
		_t.fatal(Go.toInterface(_err));
	};
	if ((_buf.string() : GoString) != (("func foo() {\n\t// comment inside func\n}" : GoString))) {
		_t.errorf(("got %q, want %q" : GoString), Go.toInterface((_buf.string() : GoString)),
			Go.toInterface(("func foo() {\n\t// comment inside func\n}" : GoString)));
	};
	_buf.reset();
	_err = fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _fset,
		Go.toInterface((({node: Go.toInterface(_f.decls[(1 : GoInt)]), comments: _f.comments} : CommentedNode) : Ref<CommentedNode>)));
	if (_err != null) {
		_t.fatal(Go.toInterface(_err));
	};
	if ((_buf.string() : GoString) != (("// leading comment\ntype bar int\t// comment2\n" : GoString))) {
		_t.errorf(("got %q, want %q" : GoString), Go.toInterface((_buf.string() : GoString)),
			Go.toInterface(("// leading comment\ntype bar int\t// comment2\n" : GoString)));
	};
}
...
function testIssue11151(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	if (_err != null) {
		_t.errorf(("%v\norig: %q\ngot : %q" : GoString), Go.toInterface(_err),
			Go.toInterface(("package p\t/*\r/1\r*\r/2*\r\r\r\r/3*\r\r+\r\r/4*/\n" : GoString)), Go.toInterface(_got));
	};
}
...
function testParenthesizedDecl(_t:Ref<stdgo.testing.Testing.T>):Void {
	{};
	var _fset = stdgo.go.token.Token.newFileSet();
	var __tmp__ = stdgo.go.parser.Parser.parseFile(_fset, Go.str(), Go.toInterface(("package p; var ( a float64; b int )" : GoString)),
		(("0" : GoUInt) : stdgo.go.parser.Parser.Mode)),
		_f:Ref<stdgo.go.ast.Ast.File> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err != null) {
	...
	var _original:GoString = (_buf.string() : GoString);
	{
		...
		Go.cfor(_i != ((_f.decls.length)), _i++, {
			(Go.typeAssert((Go.toInterface(_f.decls[(_i : GoInt)]) : Ref<stdgo.go.ast.Ast.GenDecl>)) : Ref<stdgo.go.ast.Ast.GenDecl>)
				.lparen = (0 : stdgo.go.token.Token.Pos);
		});
	};
	_buf.reset();
	_err = fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _fset, Go.toInterface(Go.asInterface(_f)));
...
function testIssue32854(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	_ret.results[(0 : GoInt)] = (Go.typeAssert((Go.toInterface(_ret.results[(0 : GoInt)]) : Ref<stdgo.go.ast.Ast.CompositeLit>)) : Ref<stdgo.go.ast.Ast.CompositeLit>)
		.elts[(0 : GoInt)];
	var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
	{
...
@:keep var _ = {
	try {
		...
		var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
		{
			var _err:Error = fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _fset,
				Go.toInterface(Go.asInterface((({name: ("foobar" : GoString)} : stdgo.go.ast.Ast.Ident) : Ref<stdgo.go.ast.Ast.Ident>))));
			if (_err != null) {
				throw Go.toInterface(_err);
			};
		};
...
class T_printer_asInterface {
	@:keep
	public function _printNode(_node:AnyInterface):Error
		return __self__.value._printNode(_node);
	...
	**/
	@:keep
	public function _flush(_next:stdgo.go.token.Token.Position, _tok:stdgo.go.token.Token.Token):{var _0:Bool; var _1:Bool;}
		return __self__.value._flush(_next, _tok);
	...
	**/
	@:keep
	public function _print(_args:haxe.Rest<AnyInterface>):Void
		__self__.value._print(..._args);

	/**
		// whiteWhitespace writes the first n whitespace entries.
	**/
	@:keep
	public function _writeWhitespace(_n:GoInt):Void
		__self__.value._writeWhitespace(_n);
	...
	**/
	@:keep
	public function _intersperseComments(_next:stdgo.go.token.Token.Position, _tok:stdgo.go.token.Token.Token):{var _0:Bool; var _1:Bool;}
		return __self__.value._intersperseComments(_next, _tok);

	/**
		// containsLinebreak reports whether the whitespace buffer contains any line breaks.
	**/
	@:keep
	public function _containsLinebreak():Bool
		return __self__.value._containsLinebreak();
	...
	**/
	@:keep
	public function _writeCommentSuffix(_needsLinebreak:Bool):{var _0:Bool; var _1:Bool;}
		return __self__.value._writeCommentSuffix(_needsLinebreak);

	@:keep
	public function _writeComment(_comment:Ref<stdgo.go.ast.Ast.Comment>):Void
		__self__.value._writeComment(_comment);
	...
	**/
	@:keep
	public function _writeCommentPrefix(_pos:stdgo.go.token.Token.Position, _next:stdgo.go.token.Token.Position, _prev:Ref<stdgo.go.ast.Ast.Comment>,
			_tok:stdgo.go.token.Token.Token):Void
		__self__.value._writeCommentPrefix(_pos, _next, _prev, _tok);
	...
	**/
	@:keep
	public function _writeString(_pos:stdgo.go.token.Token.Position, _s:GoString, _isLit:Bool):Void
		__self__.value._writeString(_pos, _s, _isLit);
	...
	**/
	@:keep
	public function _writeByte(_ch:GoByte, _n:GoInt):Void
		__self__.value._writeByte(_ch, _n);

	/**
		// writeIndent writes indentation.
	**/
	@:keep
	public function _writeIndent():Void
		__self__.value._writeIndent();

	/**
		// writeLineDirective writes a //line directive if necessary.
	**/
	@:keep
	public function _writeLineDirective(_pos:stdgo.go.token.Token.Position):Void
		__self__.value._writeLineDirective(_pos);

	@:keep
	public function _lineFor(_pos:stdgo.go.token.Token.Pos):GoInt
		return __self__.value._lineFor(_pos);

	@:keep
	public function _posFor(_pos:stdgo.go.token.Token.Pos):stdgo.go.token.Token.Position
		return __self__.value._posFor(_pos);
	...
	**/
	@:keep
	public function _linesFrom(_line:GoInt):GoInt
		return __self__.value._linesFrom(_line);
	...
	**/
	@:keep
	public function _recordLine(_linePtr:Pointer<GoInt>):Void
		__self__.value._recordLine(_linePtr);
	...
	**/
	@:keep
	public function _commentSizeBefore(_next:stdgo.go.token.Token.Position):GoInt
		return __self__.value._commentSizeBefore(_next);
	...
	**/
	@:keep
	public function _commentBefore(_next:stdgo.go.token.Token.Position):Bool
		return __self__.value._commentBefore(_next);

	@:keep
	public function _nextComment():Void
		__self__.value._nextComment();
	...
	**/
	@:keep
	public function _commentsHaveNewline(_list:Slice<Ref<stdgo.go.ast.Ast.Comment>>):Bool
		return __self__.value._commentsHaveNewline(_list);

	@:keep
	public function _internalError(_msg:haxe.Rest<AnyInterface>):Void
		__self__.value._internalError(..._msg);

	@:keep
	public function _init(_cfg:Ref<Config>, _fset:Ref<stdgo.go.token.Token.FileSet>, _nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>):Void
		__self__.value._init(_cfg, _fset, _nodeSizes);

	@:keep
	public function _file(_src:Ref<stdgo.go.ast.Ast.File>):Void
		__self__.value._file(_src);

	@:keep
	public function _declList(_list:Slice<stdgo.go.ast.Ast.Decl>):Void
		__self__.value._declList(_list);

	@:keep
	public function _decl(_decl:stdgo.go.ast.Ast.Decl):Void
		__self__.value._decl(_decl);

	@:keep
	public function _funcDecl(_d:Ref<stdgo.go.ast.Ast.FuncDecl>):Void
		__self__.value._funcDecl(_d);
	...
	**/
	@:keep
	public function _distanceFrom(_startPos:stdgo.go.token.Token.Pos, _startOutCol:GoInt):GoInt
		return __self__.value._distanceFrom(_startPos, _startOutCol);
	...
	**/
	@:keep
	public function _funcBody(_headerSize:GoInt, _sep:T_whiteSpace, _b:Ref<stdgo.go.ast.Ast.BlockStmt>):Void
		__self__.value._funcBody(_headerSize, _sep, _b);

	/**
		// bodySize is like nodeSize but it is specialized for *ast.BlockStmt's.
	**/
	@:keep
	public function _bodySize(_b:Ref<stdgo.go.ast.Ast.BlockStmt>, _maxSize:GoInt):GoInt
		return __self__.value._bodySize(_b, _maxSize);

	/**
		// numLines returns the number of lines spanned by node n in the original source.
	**/
	@:keep
	public function _numLines(_n:stdgo.go.ast.Ast.Node):GoInt
		return __self__.value._numLines(_n);
	...
	**/
	@:keep
	public function _nodeSize(_n:stdgo.go.ast.Ast.Node, _maxSize:GoInt):GoInt
		return __self__.value._nodeSize(_n, _maxSize);

	@:keep
	public function _genDecl(_d:Ref<stdgo.go.ast.Ast.GenDecl>):Void
		__self__.value._genDecl(_d);
	...
	**/
	@:keep
	public function _spec(_spec:stdgo.go.ast.Ast.Spec, _n:GoInt, _doIndent:Bool):Void
		__self__.value._spec(_spec, _n, _doIndent);

	@:keep
	public function _valueSpec(_s:Ref<stdgo.go.ast.Ast.ValueSpec>, _keepType:Bool):Void
		__self__.value._valueSpec(_s, _keepType);

	@:keep
	public function _stmt(_stmt:stdgo.go.ast.Ast.Stmt, _nextIsRBrace:Bool):Void
		__self__.value._stmt(_stmt, _nextIsRBrace);
	...
	**/
	@:keep
	public function _indentList(_list:Slice<stdgo.go.ast.Ast.Expr>):Bool
		return __self__.value._indentList(_list);

	@:keep
	public function _controlClause(_isForStmt:Bool, _init:stdgo.go.ast.Ast.Stmt, _expr:stdgo.go.ast.Ast.Expr, _post:stdgo.go.ast.Ast.Stmt):Void
		__self__.value._controlClause(_isForStmt, _init, _expr, _post);

	/**
		// block prints an *ast.BlockStmt; it always spans at least two lines.
	**/
	@:keep
	public function _block(_b:Ref<stdgo.go.ast.Ast.BlockStmt>, _nindent:GoInt):Void
		__self__.value._block(_b, _nindent);
	...
	**/
	@:keep
	public function _stmtList(_list:Slice<stdgo.go.ast.Ast.Stmt>, _nindent:GoInt, _nextIsRBrace:Bool):Void
		__self__.value._stmtList(_list, _nindent, _nextIsRBrace);

	@:keep
	public function _expr(_x:stdgo.go.ast.Ast.Expr):Void
		__self__.value._expr(_x);

	@:keep
	public function _expr0(_x:stdgo.go.ast.Ast.Expr, _depth:GoInt):Void
		__self__.value._expr0(_x, _depth);
	...
	**/
	@:keep
	public function _selectorExpr(_x:Ref<stdgo.go.ast.Ast.SelectorExpr>, _depth:GoInt, _isMethod:Bool):Bool
		return __self__.value._selectorExpr(_x, _depth, _isMethod);

	@:keep
	public function _possibleSelectorExpr(_expr:stdgo.go.ast.Ast.Expr, _prec1:GoInt, _depth:GoInt):Bool
		return __self__.value._possibleSelectorExpr(_expr, _prec1, _depth);

	@:keep
	public function _expr1(_expr:stdgo.go.ast.Ast.Expr, _prec1:GoInt, _depth:GoInt):Void
		__self__.value._expr1(_expr, _prec1, _depth);
	...
	**/
	@:keep
	public function _binaryExpr(_x:Ref<stdgo.go.ast.Ast.BinaryExpr>, _prec1:GoInt, _cutoff:GoInt, _depth:GoInt):Void
		__self__.value._binaryExpr(_x, _prec1, _cutoff, _depth);

	@:keep
	public function _fieldList(_fields:Ref<stdgo.go.ast.Ast.FieldList>, _isStruct:Bool, _isIncomplete:Bool):Void
		__self__.value._fieldList(_fields, _isStruct, _isIncomplete);

	@:keep
	public function _setLineComment(_text:GoString):Void
		__self__.value._setLineComment(_text);

	@:keep
	public function _isOneLineFieldList(_list:Slice<Ref<stdgo.go.ast.Ast.Field>>):Bool
		return __self__.value._isOneLineFieldList(_list);

	@:keep
	public function _signature(_sig:Ref<stdgo.go.ast.Ast.FuncType>):Void
		__self__.value._signature(_sig);

	@:keep
	public function _parameters(_fields:Ref<stdgo.go.ast.Ast.FieldList>, _mode:T_paramMode):Void
		__self__.value._parameters(_fields, _mode);
	...
	**/
	@:keep
	public function _exprList(_prev0:stdgo.go.token.Token.Pos, _list:Slice<stdgo.go.ast.Ast.Expr>, _depth:GoInt, _mode:T_exprListMode,
			_next0:stdgo.go.token.Token.Pos, _isIncomplete:Bool):Void
	...
	**/
	@:keep
	public function _identList(_list:Slice<Ref<stdgo.go.ast.Ast.Ident>>, _indent:Bool):Void
		__self__.value._identList(_list, _indent);
	...
	**/
	@:keep
	public function _setComment(_g:Ref<stdgo.go.ast.Ast.CommentGroup>):Void
		__self__.value._setComment(_g);
	...
	**/
	@:keep
	public function _linebreak(_line:GoInt, _min:GoInt, _ws:T_whiteSpace, _newSection:Bool):GoInt
		return __self__.value._linebreak(_line, _min, _ws, _newSection);

	@:keep
	public function _commentTextAt(_start:GoInt):GoString
		return __self__.value._commentTextAt(_start);

	@:keep
	public function _lineAt(_start:GoInt):Slice<GoByte>
		return __self__.value._lineAt(_start);

	@:keep
	public function _fixGoBuildLines():Void
		__self__.value._fixGoBuildLines();

	@:embedded
	public function _fprint(_output:stdgo.io.Io.Writer, _fset_:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface,
			_nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>):Error
		return __self__.value._fprint(_output, _fset_, _node, _nodeSizes);

	@:embedded
	public function fprint(_output:stdgo.io.Io.Writer, _fset_:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface):Error
		return __self__.value.fprint(_output, _fset_, _node);
...
@:keep @:allow(stdgo.go.printer.Printer.T_printer_asInterface) class T_printer_static_extension {
		...
		return stdgo.internal.Macro.controlFlow({
			...
			_p._print(Go.toInterface((0 : T_pmode)));
			{
				...
				if (Go.typeEquals((__type__ : stdgo.go.ast.Ast.Expr))) {
					var _n:stdgo.go.ast.Ast.Expr = __type__ == null ? (null : stdgo.go.ast.Ast.Expr) : __type__.__underlying__() == null ? (null : stdgo.go.ast.Ast.Expr) : __type__ == null ? (null : stdgo.go.ast.Ast.Expr) : __type__.__underlying__()
						.value;
					_p._expr(_n);
				} else if (Go.typeEquals((__type__ : stdgo.go.ast.Ast.Stmt))) {
					var _n:stdgo.go.ast.Ast.Stmt = __type__ == null ? (null : stdgo.go.ast.Ast.Stmt) : __type__.__underlying__() == null ? (null : stdgo.go.ast.Ast.Stmt) : __type__ == null ? (null : stdgo.go.ast.Ast.Stmt) : __type__.__underlying__()
						.value;
					{
						var __tmp__ = try {
					...
					var _n:stdgo.go.ast.Ast.Decl = __type__ == null ? (null : stdgo.go.ast.Ast.Decl) : __type__.__underlying__() == null ? (null : stdgo.go.ast.Ast.Decl) : __type__ == null ? (null : stdgo.go.ast.Ast.Decl) : __type__.__underlying__()
						.value;
					_p._decl(_n);
				} else if (Go.typeEquals((__type__ : stdgo.go.ast.Ast.Spec))) {
					var _n:stdgo.go.ast.Ast.Spec = __type__ == null ? (null : stdgo.go.ast.Ast.Spec) : __type__.__underlying__() == null ? (null : stdgo.go.ast.Ast.Spec) : __type__ == null ? (null : stdgo.go.ast.Ast.Spec) : __type__.__underlying__()
						.value;
					_p._spec(_n, (1 : GoInt), false);
				} else if (Go.typeEquals((__type__ : Slice<stdgo.go.ast.Ast.Stmt>))) {
					var _n:Slice<stdgo.go.ast.Ast.Stmt> = __type__ == null ? (null : Slice<stdgo.go.ast.Ast.Stmt>) : __type__.__underlying__() == null ? (null : Slice<stdgo.go.ast.Ast.Stmt>) : __type__ == null ? (null : Slice<stdgo.go.ast.Ast.Stmt>) : __type__.__underlying__()
						.value;
					for (_11 => _s in _n) {
						{
							var __tmp__ = try {
								{value: (Go.typeAssert((Go.toInterface(_s) : Ref<stdgo.go.ast.Ast.LabeledStmt>)) : Ref<stdgo.go.ast.Ast.LabeledStmt>),
									ok: true};
							} catch (_) {
								{value: (null : Ref<stdgo.go.ast.Ast.LabeledStmt>), ok: false};
							}, _12 = __tmp__.value, _ok = __tmp__.ok;
							if (_ok) {
								_p._indent = (1 : GoInt);
							};
						};
					...
					var _n:Slice<stdgo.go.ast.Ast.Decl> = __type__ == null ? (null : Slice<stdgo.go.ast.Ast.Decl>) : __type__.__underlying__() == null ? (null : Slice<stdgo.go.ast.Ast.Decl>) : __type__ == null ? (null : Slice<stdgo.go.ast.Ast.Decl>) : __type__.__underlying__()
						.value;
					_p._declList(_n);
				} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.File>))) {
					var _n:Ref<stdgo.go.ast.Ast.File> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.File>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.File>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.File>) : __type__.__underlying__()
	...
	**/
	...
	static public function _print(_p:Ref<T_printer>, _args:haxe.Rest<AnyInterface>):Void {
		...
		for (_0 => _arg in _args) {
			...
			if (_p._lastTok == ((0 : stdgo.go.token.Token.Token))) {} else if (_p._lastTok == ((49 : stdgo.go.token.Token.Token))
				|| _p._lastTok == ((50 : stdgo.go.token.Token.Token))) {
			...
			};
			{
				...
				if (Go.typeEquals((__type__ : T_pmode))) {
					var _x:T_pmode = __type__ == null ? ((0 : GoInt) : T_pmode) : __type__.__underlying__() == null ? ((0 : GoInt) : T_pmode) : __type__ == null ? ((0 : GoInt) : T_pmode) : __type__.__underlying__()
						.value;
					_p._mode = _p._mode ^ (_x);
					continue;
				} else if (Go.typeEquals((__type__ : T_whiteSpace))) {
					var _x:T_whiteSpace = __type__ == null ? ((0 : GoUInt8) : T_whiteSpace) : __type__.__underlying__() == null ? ((0 : GoUInt8) : T_whiteSpace) : __type__ == null ? ((0 : GoUInt8) : T_whiteSpace) : __type__.__underlying__()
						.value;
					...
					var _x:stdgo.go.token.Token.Token = __type__ == null ? ((0 : GoInt) : stdgo.go.token.Token.Token) : __type__.__underlying__() == null ? ((0 : GoInt) : stdgo.go.token.Token.Token) : __type__ == null ? ((0 : GoInt) : stdgo.go.token.Token.Token) : __type__.__underlying__()
						.value;
					...
					_data = _s;
					if (_x == ((61 : stdgo.go.token.Token.Token))
						|| _x == ((65 : stdgo.go.token.Token.Token))
						|| _x == ((69 : stdgo.go.token.Token.Token))
						|| _x == ((80 : stdgo.go.token.Token.Token))
						|| _x == ((37 : stdgo.go.token.Token.Token))
						|| _x == ((38 : stdgo.go.token.Token.Token))
						|| _x == ((54 : stdgo.go.token.Token.Token))
						|| _x == ((55 : stdgo.go.token.Token.Token))
						|| _x == ((56 : stdgo.go.token.Token.Token))) {
						_impliedSemi = true;
					};
					_p._lastTok = _x;
				} else if (Go.typeEquals((__type__ : stdgo.go.token.Token.Pos))) {
					var _x:stdgo.go.token.Token.Pos = __type__ == null ? ((0 : GoInt) : stdgo.go.token.Token.Pos) : __type__.__underlying__() == null ? ((0 : GoInt) : stdgo.go.token.Token.Pos) : __type__ == null ? ((0 : GoInt) : stdgo.go.token.Token.Pos) : __type__.__underlying__()
						.value;
					if (_x.isValid()) {
						_p._pos = (_p._posFor(_x) == null ? null : _p._posFor(_x).__copy__());
					};
					continue;
			...
			var __tmp__ = _p._flush((_next == null ? null : _next.__copy__()), _p._lastTok),
				_wroteNewline:Bool = __tmp__._0,
				_droppedFF:Bool = __tmp__._1;
			if (!_p._impliedSemi) {
					...
					var _ch:T_whiteSpace = _p._wsbuf[(_i : GoInt)];
					{
						...
						while (true) {
							if (__switchIndex__ == 0 || (__switchIndex__ == -1 && (_ch == (0 : T_whiteSpace)))) {
								...
								|| (__switchIndex__ == -1 && (_ch == ((10 : T_whiteSpace)) || _ch == ((12 : T_whiteSpace))))) {
	...
	**/
	@:keep
	static public function _intersperseComments(_p:Ref<T_printer>, _next:stdgo.go.token.Token.Position,
			_tok:stdgo.go.token.Token.Token):{var _0:Bool; var _1:Bool;} {
		var _wroteNewline:Bool = false, _droppedFF:Bool = false;
		...
		while (_p._commentBefore((_next == null ? null : _next.__copy__()))) {
			...
			if (((_p._lastTok != (75 : stdgo.go.token.Token.Token)) && (_p._posFor(_p._commentInfo._comment.pos()).column == (1 : GoInt)))
				&& (Go.toInterface(Go.asInterface(_p._posFor(_p._commentInfo._comment.end() +
					(1 : stdgo.go.token.Token.Pos)))) == Go.toInterface(Go.asInterface(_next)))) {
				_list = _formatDocComment(_list);
				_changed = true;
				if ((_p._commentInfo._comment.list.length > (0 : GoInt)) && (_list.length == (0 : GoInt))) {
					_p._writeCommentPrefix((_p._posFor(_p._commentInfo._comment.pos()) == null ? null : _p._posFor(_p._commentInfo._comment.pos()).__copy__()),
						(_next == null ? null : _next.__copy__()), _last, _tok);
			...
			for (_16 => _c in _list) {
				_p._writeCommentPrefix((_p._posFor(_c.pos()) == null ? null : _p._posFor(_c.pos()).__copy__()), (_next == null ? null : _next.__copy__()),
					_last, _tok);
				_p._writeComment(_c);
				_last = _c;
			};
		...
		if (_last != null) {
			...
			if (((((((_p._mode & (1 : T_pmode)) == (0 : T_pmode)) && (_last.text[(1 : GoInt)] == (42 : GoUInt8)))
				&& (_p._lineFor(_last.pos()) == _next.line))
				&& (_tok != (52 : stdgo.go.token.Token.Token)))
				&& ((_tok != (54 : stdgo.go.token.Token.Token)) || (_p._prevOpen == (49 : stdgo.go.token.Token.Token))))
				&& ((_tok != (55 : stdgo.go.token.Token.Token)) || (_p._prevOpen == (50 : stdgo.go.token.Token.Token)))) {
			...
			if (((_last.text[(1 : GoInt)] == (47 : GoUInt8)) || (_tok == (1 : stdgo.go.token.Token.Token)))
				|| ((_tok == (56 : stdgo.go.token.Token.Token)) && ((_p._mode & (2 : T_pmode)) == (0 : T_pmode)))) {
				_needsLinebreak = true;
			};
	...
	**/
	@:keep
	static public function _writeCommentPrefix(_p:Ref<T_printer>, _pos:stdgo.go.token.Token.Position, _next:stdgo.go.token.Token.Position,
			_prev:Ref<stdgo.go.ast.Ast.Comment>, _tok:stdgo.go.token.Token.Token):Void {
	...
	**/
	...
	static public function _commentSizeBefore(_p:Ref<T_printer>, _next:stdgo.go.token.Token.Position):GoInt {
		...
		try {
			while (_p._commentBefore((_next == null ? null : _next.__copy__()))) {
				for (_2 => _c in _p._commentInfo._comment.list) {
	...
	**/
	...
	static public function _commentsHaveNewline(_p:Ref<T_printer>, _list:Slice<Ref<stdgo.go.ast.Ast.Comment>>):Bool {
		...
		for (_i => _c in _list) {
			...
			};
			{
				...
				if ((_t.length >= (2 : GoInt))
					&& ((_t[(1 : GoInt)] == (47 : GoUInt8)) || stdgo.strings.Strings.contains(_t, ("\n" : GoString)))) {
					return true;
				};
			};
		};
	...
	@:keep
	static public function _init(_p:Ref<T_printer>, _cfg:Ref<Config>, _fset:Ref<stdgo.go.token.Token.FileSet>,
			_nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>):Void {
		_p.config = (_cfg == null ? null : _cfg.__copy__());
		_p._fset = _fset;
		_p._pos = ({line: (1 : GoInt), column: (1 : GoInt)} : stdgo.go.token.Token.Position);
		_p._out = ({line: (1 : GoInt), column: (1 : GoInt)} : stdgo.go.token.Token.Position);
		_p._wsbuf = new Slice<T_whiteSpace>((0 : GoInt).toBasic(), (16 : GoInt), ...[for (i in 0...(0 : GoInt).toBasic()) ((0 : GoUInt8) : T_whiteSpace)]);
		_p._nodeSizes = _nodeSizes;
		_p._cachedPos = (-1 : stdgo.go.token.Token.Pos);
	}

	@:keep
	static public function _file(_p:Ref<T_printer>, _src:Ref<stdgo.go.ast.Ast.File>):Void {
		_p._setComment(_src.doc);
		_p._print(Go.toInterface(Go.asInterface(_src.pos())), Go.toInterface(Go.asInterface((78 : stdgo.go.token.Token.Token))),
			Go.toInterface((32 : T_whiteSpace)));
	...
	static public function _decl(_p:Ref<T_printer>, _decl:stdgo.go.ast.Ast.Decl):Void {
		{
			...
			if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.BadDecl>))) {
				var _d:Ref<stdgo.go.ast.Ast.BadDecl> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BadDecl>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.BadDecl>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BadDecl>) : __type__.__underlying__()
					.value;
				_p._print(Go.toInterface(Go.asInterface(_d.pos())), Go.toInterface(("BadDecl" : GoString)));
			} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.GenDecl>))) {
				var _d:Ref<stdgo.go.ast.Ast.GenDecl> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.GenDecl>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.GenDecl>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.GenDecl>) : __type__.__underlying__()
					.value;
				_p._genDecl(_d);
	...
	static public function _funcDecl(_p:Ref<T_printer>, _d:Ref<stdgo.go.ast.Ast.FuncDecl>):Void {
		...
		_p._print(Go.toInterface(Go.asInterface(_d.pos())), Go.toInterface(Go.asInterface((71 : stdgo.go.token.Token.Token))),
			Go.toInterface((32 : T_whiteSpace)));
	...
	static public function _funcBody(_p:Ref<T_printer>, _headerSize:GoInt, _sep:T_whiteSpace, _b:Ref<stdgo.go.ast.Ast.BlockStmt>):Void {
		...
		try {
			...
			if ((_headerSize + _p._bodySize(_b, (100 : GoInt))) <= (100 : GoInt)) {
				_p._print(Go.toInterface(_sep), Go.toInterface(Go.asInterface(_b.lbrace)), Go.toInterface(Go.asInterface((51 : stdgo.go.token.Token.Token))));
				...
				_p._print(Go.toInterface((2 : T_pmode)), Go.toInterface(Go.asInterface(_b.rbrace)),
					Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))), Go.toInterface((2 : T_pmode)));
				{
					for (defer in __deferstack__) {
						defer();
					};
	...
	**/
	...
	static public function _nodeSize(_p:Ref<T_printer>, _n:stdgo.go.ast.Ast.Node, _maxSize:GoInt):GoInt {
		var _size:GoInt = (0 : GoInt);
		{
			var __tmp__ = (_p._nodeSizes != null
				&& _p._nodeSizes.__exists__(_n) ? {value: _p._nodeSizes[_n], ok: true} : {value: (0 : GoInt), ok: false}),
				_size:GoInt = __tmp__.value,
		...
		var _buf:stdgo.bytes.Bytes.Buffer = ({} : stdgo.bytes.Bytes.Buffer);
		{
			var _err:Error = _cfg._fprint(Go.asInterface((_buf : Ref<stdgo.bytes.Bytes.Buffer>)), _p._fset, Go.toInterface(_n), _p._nodeSizes);
			if (_err != null) {
				return _size;
			};
		};
		if (_buf.len() <= _maxSize) {
			for (_10 => _ch in _buf.bytes()) {
				if (_ch == ((10 : GoUInt8)) || _ch == ((12 : GoUInt8))) {
					return _size;
				};
			};
	...
	static public function _genDecl(_p:Ref<T_printer>, _d:Ref<stdgo.go.ast.Ast.GenDecl>):Void {
		_p._setComment(_d.doc);
		_p._print(Go.toInterface(Go.asInterface(_d.pos())), Go.toInterface(Go.asInterface(_d.tok)), Go.toInterface((32 : T_whiteSpace)));
		if (_d.lparen.isValid() || (_d.specs.length > (1 : GoInt))) {
			_p._print(Go.toInterface(Go.asInterface(_d.lparen)), Go.toInterface(Go.asInterface((49 : stdgo.go.token.Token.Token))));
			{
				var _n:GoInt = (_d.specs.length);
				if (_n > (0 : GoInt)) {
					_p._print(Go.toInterface((62 : T_whiteSpace)), Go.toInterface((12 : T_whiteSpace)));
					if ((_n > (1 : GoInt))
						&& ((_d.tok == (64 : stdgo.go.token.Token.Token)) || (_d.tok == (85 : stdgo.go.token.Token.Token)))) {
						...
						for (_i => _s in _d.specs) {
							...
							_p._valueSpec((Go.typeAssert((Go.toInterface(_s) : Ref<stdgo.go.ast.Ast.ValueSpec>)) : Ref<stdgo.go.ast.Ast.ValueSpec>),
								_keepType[(_i : GoInt)]);
						};
					} else {
						var _line:GoInt = (0 : GoInt);
			...
			_p._print(Go.toInterface(Go.asInterface(_d.rparen)), Go.toInterface(Go.asInterface((54 : stdgo.go.token.Token.Token))));
	...
	**/
	...
	static public function _spec(_p:Ref<T_printer>, _spec:stdgo.go.ast.Ast.Spec, _n:GoInt, _doIndent:Bool):Void {
		{
			...
			if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.ImportSpec>))) {
				...
				_p._print(Go.toInterface(Go.asInterface(_s.endPos)));
			} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.ValueSpec>))) {
				...
				if (_s.values != null) {
					_p._print(Go.toInterface((32 : T_whiteSpace)), Go.toInterface(Go.asInterface((42 : stdgo.go.token.Token.Token))),
						Go.toInterface((32 : T_whiteSpace)));
					_p._exprList((0 : stdgo.go.token.Token.Pos), _s.values, (1 : GoInt), (("0" : GoUInt) : T_exprListMode), (0 : stdgo.go.token.Token.Pos),
						false);
				};
				_p._setComment(_s.comment);
			} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.TypeSpec>))) {
	...
	static public function _valueSpec(_p:Ref<T_printer>, _s:Ref<stdgo.go.ast.Ast.ValueSpec>, _keepType:Bool):Void {
		...
		if (_s.values != null) {
			_p._print(Go.toInterface((11 : T_whiteSpace)), Go.toInterface(Go.asInterface((42 : stdgo.go.token.Token.Token))),
				Go.toInterface((32 : T_whiteSpace)));
	...
	static public function _stmt(_p:Ref<T_printer>, _stmt:stdgo.go.ast.Ast.Stmt, _nextIsRBrace:Bool):Void {
		_p._print(Go.toInterface(Go.asInterface(_stmt.pos())));
		{
			final __type__ = _stmt;
			if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.BadStmt>))) {
				var _s:Ref<stdgo.go.ast.Ast.BadStmt> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BadStmt>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.BadStmt>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BadStmt>) : __type__.__underlying__()
					.value;
				...
				var _s:Ref<stdgo.go.ast.Ast.EmptyStmt> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.EmptyStmt>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.EmptyStmt>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.EmptyStmt>) : __type__.__underlying__()
					.value;
				...
				_p._print(Go.toInterface(Go.asInterface(_s.colon)), Go.toInterface(Go.asInterface((58 : stdgo.go.token.Token.Token))),
					Go.toInterface((62 : T_whiteSpace)));
				{
					var __tmp__ = try {
					...
					if (_isEmpty) {
						if (!_nextIsRBrace) {
							_p._print(Go.toInterface((10 : T_whiteSpace)), Go.toInterface(Go.asInterface(_e.pos())),
								Go.toInterface(Go.asInterface((57 : stdgo.go.token.Token.Token))));
							break;
						};
					} else {
				...
				_p._print(Go.toInterface((32 : T_whiteSpace)), Go.toInterface(Go.asInterface(_s.arrow)),
					Go.toInterface(Go.asInterface((36 : stdgo.go.token.Token.Token))), Go.toInterface((32 : T_whiteSpace)));
				...
				_p._print(Go.toInterface(Go.asInterface(_s.tokPos)), Go.toInterface(Go.asInterface(_s.tok)));
				...
				_p._print(Go.toInterface((32 : T_whiteSpace)), Go.toInterface(Go.asInterface(_s.tokPos)), Go.toInterface(Go.asInterface(_s.tok)),
					Go.toInterface((32 : T_whiteSpace)));
				...
				if (_s.results != null) {
					...
					if (_p._indentList(_s.results)) {
						...
						_p._exprList((0 : stdgo.go.token.Token.Pos), _s.results, (1 : GoInt), (("2" : GoUInt) : T_exprListMode),
							(0 : stdgo.go.token.Token.Pos), false);
						_p._print(Go.toInterface((60 : T_whiteSpace)));
					} else {
						_p._exprList((0 : stdgo.go.token.Token.Pos), _s.results, (1 : GoInt), (("0" : GoUInt) : T_exprListMode),
							(0 : stdgo.go.token.Token.Pos), false);
					};
				};
				...
				if (_s.else_ != null) {
					_p._print(Go.toInterface((32 : T_whiteSpace)), Go.toInterface(Go.asInterface((68 : stdgo.go.token.Token.Token))),
						Go.toInterface((32 : T_whiteSpace)));
					{
						final __type__ = _s.else_;
						if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.BlockStmt>))
							|| Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.IfStmt>))) {
							...
							_p._print(Go.toInterface(Go.asInterface((51 : stdgo.go.token.Token.Token))), Go.toInterface((62 : T_whiteSpace)),
								Go.toInterface((12 : T_whiteSpace)));
							_p._stmt(_s.else_, true);
							_p._print(Go.toInterface((60 : T_whiteSpace)), Go.toInterface((12 : T_whiteSpace)),
								Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))));
						};
					};
				};
				...
				var _body = _s.body;
				if ((_body.list.length == (0 : GoInt))
					&& !_p._commentBefore((_p._posFor(_body.rbrace) == null ? null : _p._posFor(_body.rbrace).__copy__()))) {
					_p._print(Go.toInterface(Go.asInterface(_body.lbrace)), Go.toInterface(Go.asInterface((51 : stdgo.go.token.Token.Token))),
						Go.toInterface(Go.asInterface(_body.rbrace)), Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))));
				} else {
				...
				if (_s.key != null) {
					_p._expr(_s.key);
					if (_s.value != null) {
						_p._print(Go.toInterface(Go.asInterface(_s.value.pos())), Go.toInterface(Go.asInterface((52 : stdgo.go.token.Token.Token))),
							Go.toInterface((32 : T_whiteSpace)));
						_p._expr(_s.value);
					};
					_p._print(Go.toInterface((32 : T_whiteSpace)), Go.toInterface(Go.asInterface(_s.tokPos)), Go.toInterface(Go.asInterface(_s.tok)),
	...
	**/
	...
	@:keep
	static public function _controlClause(_p:Ref<T_printer>, _isForStmt:Bool, _init:stdgo.go.ast.Ast.Stmt, _expr:stdgo.go.ast.Ast.Expr,
			_post:stdgo.go.ast.Ast.Stmt):Void {
		_p._print(Go.toInterface((32 : T_whiteSpace)));
	...
	**/
	...
	static public function _block(_p:Ref<T_printer>, _b:Ref<stdgo.go.ast.Ast.BlockStmt>, _nindent:GoInt):Void {
		_p._print(Go.toInterface(Go.asInterface(_b.lbrace)), Go.toInterface(Go.asInterface((51 : stdgo.go.token.Token.Token))));
		_p._stmtList(_b.list, _nindent, true);
		_p._linebreak(_p._lineFor(_b.rbrace), (1 : GoInt), (0 : T_whiteSpace), true);
		_p._print(Go.toInterface(Go.asInterface(_b.rbrace)), Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))));
	}
		...
		for (_0 => _s in _list) {
			{
				...
				if (!_isEmpty) {
					if ((_p._output.length) > (0 : GoInt)) {
						_p._linebreak(_p._lineFor(_s.pos()), (1 : GoInt), (0 : T_whiteSpace), ((_i == (0 : GoInt)) || (_nindent == (0 : GoInt)))
							|| (_p._linesFrom(_line) > (0 : GoInt)));
					};
					_p._recordLine(Go.pointer(_line));
					_p._stmt(_s, _nextIsRBrace && (_i == (_list.length - (1 : GoInt))));
					{
						...
						while (true) {
							var __tmp__ = try {
								{value: (Go.typeAssert((Go.toInterface(_t) : Ref<stdgo.go.ast.Ast.LabeledStmt>)) : Ref<stdgo.go.ast.Ast.LabeledStmt>),
									ok: true};
							} catch (_) {
								{value: (null : Ref<stdgo.go.ast.Ast.LabeledStmt>), ok: false};
							}, _lt = __tmp__.value, _4 = __tmp__.ok;
							if (_lt == null) {
								break;
							};
							_line++;
	...
	**/
	...
	static public function _selectorExpr(_p:Ref<T_printer>, _x:Ref<stdgo.go.ast.Ast.SelectorExpr>, _depth:GoInt, _isMethod:Bool):Bool {
		...
		_p._print(Go.toInterface(Go.asInterface((53 : stdgo.go.token.Token.Token))));
		{
			...
			if (_p._pos.isValid() && (_p._pos.line < _line)) {
				_p._print(Go.toInterface((62 : T_whiteSpace)), Go.toInterface((10 : T_whiteSpace)), Go.toInterface(Go.asInterface(_x.sel.pos())),
					Go.toInterface(Go.asInterface(_x.sel)));
				if (!_isMethod) {
					_p._print(Go.toInterface((60 : T_whiteSpace)));
				};
				return true;
		...
		_p._print(Go.toInterface(Go.asInterface(_x.sel.pos())), Go.toInterface(Go.asInterface(_x.sel)));
		return false;
	}
	...
	static public function _expr1(_p:Ref<T_printer>, _expr:stdgo.go.ast.Ast.Expr, _prec1:GoInt, _depth:GoInt):Void {
		_p._print(Go.toInterface(Go.asInterface(_expr.pos())));
		{
			final __type__ = _expr;
			if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.BadExpr>))) {
				var _x:Ref<stdgo.go.ast.Ast.BadExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BadExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.BadExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.BadExpr>) : __type__.__underlying__()
					.value;
				...
				var _x:Ref<stdgo.go.ast.Ast.KeyValueExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.KeyValueExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.KeyValueExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.KeyValueExpr>) : __type__.__underlying__()
					.value;
				_p._expr(_x.key);
				_p._print(Go.toInterface(Go.asInterface(_x.colon)), Go.toInterface(Go.asInterface((58 : stdgo.go.token.Token.Token))),
					Go.toInterface((32 : T_whiteSpace)));
				_p._expr(_x.value);
			} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.StarExpr>))) {
				var _x:Ref<stdgo.go.ast.Ast.StarExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.StarExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.StarExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.StarExpr>) : __type__.__underlying__()
				...
				var _x:Ref<stdgo.go.ast.Ast.FuncLit> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.FuncLit>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.FuncLit>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.FuncLit>) : __type__.__underlying__()
					.value;
				_p._print(Go.toInterface(Go.asInterface(_x.type.pos())), Go.toInterface(Go.asInterface((71 : stdgo.go.token.Token.Token))));
				var _startCol:GoInt = _p._out.column - (("func" : GoString).length);
					...
					.value;
				{
					var __tmp__ = try {
						...
						{value: (null : Ref<stdgo.go.ast.Ast.ParenExpr>), ok: false};
					}, _4 = __tmp__.value, _hasParens = __tmp__.ok;
					if (_hasParens) {
						_p._expr0(_x.x, _depth);
					} else {
						_p._print(Go.toInterface(Go.asInterface((49 : stdgo.go.token.Token.Token))));
						_p._expr0(_x.x, _reduceDepth(_depth));
						_p._print(Go.toInterface(Go.asInterface(_x.rparen)), Go.toInterface(Go.asInterface((54 : stdgo.go.token.Token.Token))));
					};
				};
			} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.SelectorExpr>))) {
				...
				var _x:Ref<stdgo.go.ast.Ast.TypeAssertExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.TypeAssertExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.TypeAssertExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.TypeAssertExpr>) : __type__.__underlying__()
					.value;
				_p._expr1(_x.x, (7 : GoInt), _depth);
				_p._print(Go.toInterface(Go.asInterface((53 : stdgo.go.token.Token.Token))), Go.toInterface(Go.asInterface(_x.lparen)),
					Go.toInterface(Go.asInterface((49 : stdgo.go.token.Token.Token))));
				if (_x.type != null) {
					_p._expr(_x.type);
				} else {
				...
				var _x:Ref<stdgo.go.ast.Ast.IndexExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.IndexExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.IndexExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.IndexExpr>) : __type__.__underlying__()
					.value;
				_p._expr1(_x.x, (7 : GoInt), (1 : GoInt));
				_p._print(Go.toInterface(Go.asInterface(_x.lbrack)), Go.toInterface(Go.asInterface((50 : stdgo.go.token.Token.Token))));
				_p._expr0(_x.index, _depth + (1 : GoInt));
				_p._print(Go.toInterface(Go.asInterface(_x.rbrack)), Go.toInterface(Go.asInterface((55 : stdgo.go.token.Token.Token))));
			} else if (Go.typeEquals((__type__ : Ref<stdgo.go.ast.Ast.IndexListExpr>))) {
				var _x:Ref<stdgo.go.ast.Ast.IndexListExpr> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.IndexListExpr>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.IndexListExpr>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.IndexListExpr>) : __type__.__underlying__()
					.value;
				_p._expr1(_x.x, (7 : GoInt), (1 : GoInt));
				_p._print(Go.toInterface(Go.asInterface(_x.lbrack)), Go.toInterface(Go.asInterface((50 : stdgo.go.token.Token.Token))));
				...
				var _wasIndented:Bool = false;
				{
					...
					}, _6 = __tmp__.value, _ok = __tmp__.ok;
					if (_ok) {
						_p._print(Go.toInterface(Go.asInterface((49 : stdgo.go.token.Token.Token))));
				...
				_p._print(Go.toInterface(Go.asInterface(_x.lparen)), Go.toInterface(Go.asInterface((49 : stdgo.go.token.Token.Token))));
				if (_x.ellipsis.isValid()) {
					_p._exprList(_x.lparen, _x.args, _depth, (("0" : GoUInt) : T_exprListMode), _x.ellipsis, false);
					_p._print(Go.toInterface(Go.asInterface(_x.ellipsis)), Go.toInterface(Go.asInterface((48 : stdgo.go.token.Token.Token))));
				...
				_p._level++;
				_p._print(Go.toInterface(Go.asInterface(_x.lbrace)), Go.toInterface(Go.asInterface((51 : stdgo.go.token.Token.Token))));
				...
				_p._print(Go.toInterface((62 : T_whiteSpace)), Go.toInterface((60 : T_whiteSpace)), Go.toInterface(_mode),
					Go.toInterface(Go.asInterface(_x.rbrace)), Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))), Go.toInterface(_mode));
				_p._level--;
				...
				var _x:Ref<stdgo.go.ast.Ast.MapType> = __type__ == null ? (null : Ref<stdgo.go.ast.Ast.MapType>) : __type__.__underlying__() == null ? (null : Ref<stdgo.go.ast.Ast.MapType>) : __type__ == null ? (null : Ref<stdgo.go.ast.Ast.MapType>) : __type__.__underlying__()
					.value;
				_p._print(Go.toInterface(Go.asInterface((77 : stdgo.go.token.Token.Token))),
					Go.toInterface(Go.asInterface((50 : stdgo.go.token.Token.Token))));
				_p._expr(_x.key);
				_p._print(Go.toInterface(Go.asInterface((55 : stdgo.go.token.Token.Token))));
				_p._expr(_x.value);
				...
				if (_x.dir == ((3 : stdgo.go.ast.Ast.ChanDir))) {
					...
					_p._print(Go.toInterface(Go.asInterface((36 : stdgo.go.token.Token.Token))),
						Go.toInterface(Go.asInterface((63 : stdgo.go.token.Token.Token))));
				} else if (_x.dir == ((1 : stdgo.go.ast.Ast.ChanDir))) {
					_p._print(Go.toInterface(Go.asInterface((63 : stdgo.go.token.Token.Token))), Go.toInterface(Go.asInterface(_x.arrow)),
						Go.toInterface(Go.asInterface((36 : stdgo.go.token.Token.Token))));
				};
				_p._print(Go.toInterface((32 : T_whiteSpace)));
				_p._expr(_x.value);
			} else {
	...
	**/
	...
	static public function _binaryExpr(_p:Ref<T_printer>, _x:Ref<stdgo.go.ast.Ast.BinaryExpr>, _prec1:GoInt, _cutoff:GoInt, _depth:GoInt):Void {
		...
		_p._print(Go.toInterface(Go.asInterface(_x.opPos)), Go.toInterface(Go.asInterface(_x.op)));
	...
	static public function _fieldList(_p:Ref<T_printer>, _fields:Ref<stdgo.go.ast.Ast.FieldList>, _isStruct:Bool, _isIncomplete:Bool):Void {
		...
		if (!_hasComments && _srcIsOneLine) {
			if ((_list.length) == ((0 : GoInt))) {
				_p._print(Go.toInterface(Go.asInterface(_lbrace)), Go.toInterface(Go.asInterface((51 : stdgo.go.token.Token.Token))),
					Go.toInterface(Go.asInterface(_rbrace)), Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))));
				return;
				...
				_p._print(Go.toInterface((32 : T_whiteSpace)), Go.toInterface(Go.asInterface(_rbrace)),
					Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))));
				return;
			};
		};
		_p._print(Go.toInterface((32 : T_whiteSpace)), Go.toInterface(Go.asInterface(_lbrace)),
			Go.toInterface(Go.asInterface((51 : stdgo.go.token.Token.Token))), Go.toInterface((62 : T_whiteSpace)));
		...
		_p._print(Go.toInterface((60 : T_whiteSpace)), Go.toInterface((12 : T_whiteSpace)), Go.toInterface(Go.asInterface(_rbrace)),
			Go.toInterface(Go.asInterface((56 : stdgo.go.token.Token.Token))));
	}

	@:keep
	static public function _setLineComment(_p:Ref<T_printer>, _text:GoString):Void {
		_p._setComment((({list: (new Slice<Ref<stdgo.go.ast.Ast.Comment>>(0, 0,
			({slash: (0 : stdgo.go.token.Token.Pos),
				text: _text} : stdgo.go.ast.Ast.Comment)) : Slice<Ref<stdgo.go.ast.Ast.Comment>>)} : stdgo.go.ast.Ast.CommentGroup) : Ref<stdgo.go.ast.Ast.CommentGroup>));
	}
	...
	static public function _parameters(_p:Ref<T_printer>, _fields:Ref<stdgo.go.ast.Ast.FieldList>, _mode:T_paramMode):Void {
		var _0:stdgo.go.token.Token.Token = (49 : stdgo.go.token.Token.Token),
			_1:stdgo.go.token.Token.Token = (54 : stdgo.go.token.Token.Token),
			_closeTok:stdgo.go.token.Token.Token = _1,
			_openTok:stdgo.go.token.Token.Token = _0;
		...
		_p._print(Go.toInterface(Go.asInterface(_fields.opening)), Go.toInterface(Go.asInterface(_openTok)));
		if ((_fields.list.length) > (0 : GoInt)) {
			...
			for (_i => _par in _fields.list) {
				...
				if (_i > (0 : GoInt)) {
					if (!_needsLinebreak) {
						_p._print(Go.toInterface(Go.asInterface(_par.pos())));
					};
			...
			};
			{
				...
				} else if (((_mode == (2 : T_paramMode)) && (_fields.numFields() == (1 : GoInt)))
					&& _combinesWithName(_fields.list[(0 : GoInt)].type)) {
					_p._print(Go.toInterface(Go.asInterface((52 : stdgo.go.token.Token.Token))));
				};
			};
			if (_ws == ((0 : T_whiteSpace))) {
		...
		_p._print(Go.toInterface(Go.asInterface(_fields.closing)), Go.toInterface(Go.asInterface(_closeTok)));
	}
	...
	**/
	@:keep
	static public function _exprList(_p:Ref<T_printer>, _prev0:stdgo.go.token.Token.Pos, _list:Slice<stdgo.go.ast.Ast.Expr>, _depth:GoInt,
			_mode:T_exprListMode, _next0:stdgo.go.token.Token.Pos, _isIncomplete:Bool):Void {
		if ((_list.length) == ((0 : GoInt))) {
			if (_isIncomplete) {
				...
				if (_prev.isValid() && (_prev.line == _next.line)) {
					...
					_p._print(Go.toInterface((10 : T_whiteSpace)));
					_p._print(Go.toInterface((62 : T_whiteSpace)), Go.toInterface(("// contains filtered or unexported fields" : GoString)),
						Go.toInterface((60 : T_whiteSpace)), Go.toInterface((10 : T_whiteSpace)));
				};
			};
			return;
		};
		...
		if ((_prev.isValid() && (_prev.line == _line)) && (_line == _endLine)) {
			for (_i => _x in _list) {
				if (_i > (0 : GoInt)) {
					_p._print(Go.toInterface(Go.asInterface(_x.pos())), Go.toInterface(Go.asInterface((52 : stdgo.go.token.Token.Token))),
						Go.toInterface((32 : T_whiteSpace)));
				};
				_p._expr0(_x, _depth);
			};
			if (_isIncomplete) {
				_p._print(Go.toInterface(Go.asInterface((52 : stdgo.go.token.Token.Token))), Go.toInterface((32 : T_whiteSpace)),
					Go.toInterface(("/* contains filtered or unexported fields */" : GoString)));
			};
			return;
		};
		...
		for (_i => _x in _list) {
			...
			if (_i > (0 : GoInt)) {
				if (!_needsLinebreak) {
					_p._print(Go.toInterface(Go.asInterface(_x.pos())));
				};
				_p._print(Go.toInterface(Go.asInterface((52 : stdgo.go.token.Token.Token))));
			...
			if ((((_list.length > (1 : GoInt)) && _isPair) && (_size > (0 : GoInt))) && _needsLinebreak) {
				_p._expr(_pair.key);
				_p._print(Go.toInterface(Go.asInterface(_pair.colon)), Go.toInterface(Go.asInterface((58 : stdgo.go.token.Token.Token))),
					Go.toInterface((11 : T_whiteSpace)));
		...
		if ((((_mode & (("1" : GoUInt) : T_exprListMode)) != (("0" : GoUInt) : T_exprListMode)) && _next.isValid())
			&& (_p._pos.line < _next.line)) {
			_p._print(Go.toInterface(Go.asInterface((52 : stdgo.go.token.Token.Token))));
			if (_isIncomplete) {
				_p._print(Go.toInterface((10 : T_whiteSpace)));
				_p._print(Go.toInterface(("// contains filtered or unexported fields" : GoString)));
	...
	**/
	...
	static public function _identList(_p:Ref<T_printer>, _list:Slice<Ref<stdgo.go.ast.Ast.Ident>>, _indent:Bool):Void {
		var _xlist = new Slice<stdgo.go.ast.Ast.Expr>((_list.length : GoInt).toBasic(), 0,
			...[for (i in 0...(_list.length : GoInt).toBasic()) (null : stdgo.go.ast.Ast.Expr)]);
		for (_i => _x in _list) {
	...
	**/
	...
	static public function _setComment(_p:Ref<T_printer>, _g:Ref<stdgo.go.ast.Ast.CommentGroup>):Void {
		...
		if (_p._comments == null) {
			_p._comments = new Slice<Ref<stdgo.go.ast.Ast.CommentGroup>>((1 : GoInt).toBasic(), 0,
				...[for (i in 0...(1 : GoInt).toBasic()) (null : Ref<stdgo.go.ast.Ast.CommentGroup>)]);
		} else if (_p._commentInfo._cindex < (_p._comments.length)) {
			_p._flush((_p._posFor(_g.list[(0 : GoInt)].pos()) == null ? null : _p._posFor(_g.list[(0 : GoInt)].pos()).__copy__()),
				(0 : stdgo.go.token.Token.Token));
	...
	**/
	...
	static public function _fixGoBuildLines(_p:Ref<T_printer>):Void {
		...
		var _insert:GoInt = (0 : GoInt);
		{
			...
			while (true) {
				...
				while ((_pos < _p._output.length)
					&& ((_p._output[(_pos : GoInt)] == (32 : GoUInt8)) || (_p._output[(_pos : GoInt)] == (9 : GoUInt8)))) {
					_pos++;
				};
				if (((((_pos + (3 : GoInt)) < _p._output.length) && (_p._output[(_pos : GoInt)] == (255 : GoUInt8)))
					&& (_p._output[(_pos + (1 : GoInt) : GoInt)] == (47 : GoUInt8)))
					&& (_p._output[(_pos + (2 : GoInt) : GoInt)] == (47 : GoUInt8))) {
					_blank = false;
					while ((_pos < _p._output.length) && !_isNL(_p._output[(_pos : GoInt)])) {
						_pos++;
					};
		...
		var _x:stdgo.go.build.constraint.Constraint.Expr = (null : stdgo.go.build.constraint.Constraint.Expr);
		{
			...
			while (true) {
				if ((_p._goBuild.length) == ((0 : GoInt))) {
					for (_40 => _pos in _p._plusBuild) {
						var __tmp__ = stdgo.go.build.constraint.Constraint.parse(_p._commentTextAt(_pos)),
							_y:stdgo.go.build.constraint.Constraint.Expr = __tmp__._0,
							_err:Error = __tmp__._1;
						if (_err != null) {
						...
						if (_x == null) {
							...
							_x = Go.asInterface((({x: _x,
								y: _y} : stdgo.go.build.constraint.Constraint.AndExpr) : Ref<stdgo.go.build.constraint.Constraint.AndExpr>));
						};
					};
					break;
		...
		if (_x == null) {
			for (_45 => _pos in _p._goBuild) {
				_block = _block.__appendref__(..._p._lineAt(_pos).__toArray__());
			};
			for (_48 => _pos in _p._plusBuild) {
				_block = _block.__appendref__(..._p._lineAt(_pos).__toArray__());
			};
		} else {
			...
			if ((_p._plusBuild.length) > (0 : GoInt)) {
				var __tmp__ = stdgo.go.build.constraint.Constraint.plusBuildLines(_x),
					_lines:Slice<GoString> = __tmp__._0,
					_err:Error = __tmp__._1;
				if (_err != null) {
					_lines = (new Slice<GoString>(0, 0, ("// +build error: " : GoString) + _err.error()) : Slice<GoString>);
				};
				for (_53 => _line in _lines) {
					_block = _block.__appendref__((255 : GoUInt8));
	...
	@:embedded
	public static function _fprint(__self__:T_printer, _output:stdgo.io.Io.Writer, _fset_:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface,
			_nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>):Error
		return __self__._fprint(_output, _fset_, _node, _nodeSizes);

	@:embedded
	public static function fprint(__self__:T_printer, _output:stdgo.io.Io.Writer, _fset_:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface):Error
		return __self__.fprint(_output, _fset_, _node);
}

class T_trimmer_asInterface {
	@:keep
	public function write(_data:Slice<GoByte>):{var _0:GoInt; var _1:Error;}
		return __self__.value.write(_data);

	@:keep
	public function _resetSpace():Void
		__self__.value._resetSpace();
...
class Config_asInterface {
	...
	**/
	@:keep
	public function fprint(_output:stdgo.io.Io.Writer, _fset:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface):Error
		return __self__.value.fprint(_output, _fset, _node);

	/**
		// fprint implements Fprint and takes a nodesSizes map for setting up the printer state.
	**/
	@:keep
	public function _fprint(_output:stdgo.io.Io.Writer, _fset:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface,
			_nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>):Error
		return __self__.value._fprint(_output, _fset, _node, _nodeSizes);
...
@:keep @:allow(stdgo.go.printer.Printer.Config_asInterface) class Config_static_extension {
	...
	**/
	...
	static public function fprint(_cfg:Ref<Config>, _output:stdgo.io.Io.Writer, _fset:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface):Error {
		return _cfg._fprint(_output, _fset, _node,
			(new GoObjectMap<stdgo.go.ast.Ast.Node, GoInt>(new stdgo.internal.reflect.Reflect._Type(stdgo.internal.reflect.Reflect.GoType.mapType({
				get: () -> stdgo.internal.reflect.Reflect.GoType.named("stdgo.go.ast.Ast.Node", [], null, false, {get: () -> null})
			},
				{get: () -> stdgo.internal.reflect.Reflect.GoType.basic(int_kind)}))) : GoMap<stdgo.go.ast.Ast.Node, GoInt>));
	}
	...
	**/
	@:keep
	static public function _fprint(_cfg:Ref<Config>, _output:stdgo.io.Io.Writer, _fset:Ref<stdgo.go.token.Token.FileSet>, _node:AnyInterface,
			_nodeSizes:GoMap<stdgo.go.ast.Ast.Node, GoInt>):Error {
		var _err:Error = (null : Error);
		var _p:T_printer = ({} : T_printer);
		_p._init(_cfg, _fset, _nodeSizes);
		{
		...
		{
			var __tmp__ = try {
				{value: (Go.typeAssert((Go.toInterface(_output) : Ref<stdgo.text.tabwriter.Tabwriter.Writer>)) : Ref<stdgo.text.tabwriter.Tabwriter.Writer>),
					ok: true};
			} catch (_) {
				{value: (null : Ref<stdgo.text.tabwriter.Tabwriter.Writer>), ok: false};
			}, _tw = __tmp__.value, _4 = __tmp__.ok;
			if (_tw != null) {
				_err = _tw.flush();
			};
		};
...
class T_limitWriter_asInterface {
	...
	public function write(_buf:Slice<GoByte>):{var _0:GoInt; var _1:Error;}
		return __self__.value.write(_buf);
...
class T_visitor_asInterface {
	@:keep
	public function visit(_n:stdgo.go.ast.Ast.Node):stdgo.go.ast.Ast.Visitor
		return __self__.value.visit(_n);
