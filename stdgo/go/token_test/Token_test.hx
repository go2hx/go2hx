...
function example_retrievePositionInfo():Void {
        ...
        for (__0 => _decl in _f.decls) {
            ...
            var _kind:stdgo.GoString = ("func" : stdgo.GoString)?.__copy__();
            {
                var __tmp__ = try {
            ...
            stdgo.fmt.Fmt.printf(("%s: %s\n" : stdgo.GoString)?.__copy__(), stdgo.Go.toInterface(_fmtPosition), stdgo.Go.toInterface(_kind));
        };
    }
