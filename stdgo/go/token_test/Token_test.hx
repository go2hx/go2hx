...
function example_retrievePositionInfo():Void {
        ...
        for (__0 => _decl in _f.decls) {
            ...
            var _kind:stdgo.GoString = ("func" : stdgo.GoString);
            {
                ...
                if (_ok) {
                    _kind = (_gen.tok.string() : stdgo.GoString);
                };
            };
            var _fmtPosition:stdgo.GoString = (_relPosition.string() : stdgo.GoString);
            if (stdgo.Go.toInterface(_relPosition) != stdgo.Go.toInterface(_absPosition)) {
                _fmtPosition = _fmtPosition + ((("[" : stdgo.GoString) + (_absPosition.string() : stdgo.GoString)) + ("]" : stdgo.GoString));
            };
