...
@:structInit class T_decoder {
	public function _readBlock():{var _0:GoInt; var _1:Error;} {
		...
		var __tmp__ = _readByte(_d.value._r),
	...
	public function _newImageFromDescriptor():{var _0:Pointer<stdgo.image.Image.Paletted>; var _1:Error;} {
		var _d = new Pointer(() -> this, __tmp__ -> this.__set__(__tmp__));
		{
			...
			if (_err != null) {
				return {_0: new Pointer<stdgo.image.Image.Paletted>().nil(), _1: stdgo.fmt.Fmt.errorf("gif: can\'t read image descriptor: %s",
					Go.toInterface(_err))};
			};
		};
		...
		return {_0: stdgo.image.Image.newPaletted((({min: new stdgo.image.Image.Point(_left,
			_top).__copy__(), max: new stdgo.image.Image.Point(new stdgo.image.Image.Point(_left + _width,
				_top + _height).__copy__().__copy__())} : stdgo.image.Image.Rectangle)).__copy__(),
			new stdgo.image.color.Color.Palette()), _1: ((null : stdgo.Error))};
	}
	...
	public function _readImageDescriptor(_keepAllFrames:Bool):Error {
		...
		var __tmp__ = _d.value._newImageFromDescriptor(),
			_m:Pointer<stdgo.image.Image.Paletted> = __tmp__._0,
			_err:stdgo.Error = __tmp__._1;
		try {
			if (_err != null) {
				return _err;
			...
			if (_d.value._hasTransparentIndex) {
				if (!_useLocalColorTable) {
					_m.value.palette = ((new stdgo.image.color.Color.Palette() : stdgo.image.color.Color.Palette))
						.__append__(..._d.value._globalColorTable.__t__.toArray())
						.__copy__();
				};
				{
					...
					if (_ti < _m.value.palette.__t__.length) {
						...
						var _p:stdgo.image.color.Color.Palette = new stdgo.image.color.Color.Palette(new Slice<stdgo.image.color.Color.Color>(...[
							for (i in 0...((_ti + ((1 : GoInt)) : GoInt)).toBasic()) ((null : stdgo.image.color.Color.Color))
						])).__copy__();
			...
			var _br:Pointer<T_blockReader> = Go.pointer((({
				_d: _d,
				_i: 0,
				_j: 0,
			...
			{
				var __tmp__ = _lzwr.read(_d.value._tmp.__slice__(((256 : GoInt)), ((257 : GoInt)))),
					_n:GoInt = __tmp__._0,
					_err:stdgo.Error = __tmp__._1;
				if (_n != ((0 : GoInt))
	...
	public function _readExtension():Error {
		...
		if (_extension == _eApplication && ((_d.value._tmp.__slice__(0, _size) : GoString)) == (("NETSCAPE2.0" : GoString))) {
			var __tmp__ = _d.value._readBlock(),
				_n:GoInt = __tmp__._0,
				_err:stdgo.Error = __tmp__._1;
			if (_err != null) {
				return stdgo.fmt.Fmt.errorf("gif: reading extension: %v", Go.toInterface(_err));
	...
	public function _readColorTable(_fields:GoByte):{var _0:stdgo.image.color.Color.Palette; var _1:Error;} {
		...
		var _j:GoInt = ((0 : GoInt)),
			_p:stdgo.image.color.Color.Palette = new stdgo.image.color.Color.Palette(new Slice<stdgo.image.color.Color.Color>(...[for (i in 0...((_n : GoInt))
				.toBasic()) ((null : stdgo.image.color.Color.Color))])).__copy__();
		{
			var _i;
			for (_obj in _p.__t__.keyValueIterator()) {
				_i = _obj.key;
				_p.__t__[_i] = new stdgo.image.color.Color.RGBA(_d.value._tmp[_j + ((0 : GoInt))], _d.value._tmp[_j + ((1 : GoInt))],
					_d.value._tmp[_j + ((2 : GoInt))], ((255 : GoUInt8))).__copy__();
				_j = _j + (((3 : GoInt)));
			};
		};
	...
	public function new(?_r:T_reader, ?_vers:GoString, ?_width:GoInt, ?_height:GoInt, ?_loopCount:GoInt, ?_delayTime:GoInt, ?_backgroundIndex:GoUInt8,
			?_disposalMethod:GoUInt8, ?_imageFields:GoUInt8, ?_transparentIndex:GoUInt8, ?_hasTransparentIndex:Bool,
			?_globalColorTable:stdgo.image.color.Color.Palette, ?_delay:Slice<GoInt>, ?_disposal:Slice<GoUInt8>,
			?_image:Slice<Pointer<stdgo.image.Image.Paletted>>, ?_tmp:GoArray<GoUInt8>)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(_r) + " " + Go.string(_vers) + " " + Go.string(_width) + " " + Go.string(_height) + " " + Go.string(_loopCount) + " "
			+ Go.string(_delayTime) + " " + Go.string(_backgroundIndex) + " " + Go.string(_disposalMethod) + " " + Go.string(_imageFields) + " "
			+ Go.string(_transparentIndex) + " " + Go.string(_hasTransparentIndex) + " " + Go.string(_globalColorTable) + " " + Go.string(_delay) + " "
			+ Go.string(_disposal) + " " + Go.string(_image) + " " + Go.string(_tmp) + "}";
	}
	...
	public function __copy__() {
		return new T_decoder(_r, _vers, _width, _height, _loopCount, _delayTime, _backgroundIndex, _disposalMethod, _imageFields, _transparentIndex,
			_hasTransparentIndex, _globalColorTable, _delay, _disposal, _image, _tmp);
	}
...
@:structInit class T_blockReader {
	...
	public function new(?_d:Pointer<T_decoder>, ?_i:GoUInt8, ?_j:GoUInt8, ?_err:stdgo.Error)
		stdgo.internal.Macro.initLocals();
...
@:structInit class T_interlaceScan {
	...
	public function new(?_skip:GoInt, ?_start:GoInt)
		stdgo.internal.Macro.initLocals();
...
@:structInit class GIF {
	...
	public function new(?image:Slice<Pointer<stdgo.image.Image.Paletted>>, ?delay:Slice<GoInt>, ?loopCount:GoInt, ?disposal:Slice<GoUInt8>,
			?config:stdgo.image.Image.Config, ?backgroundIndex:GoUInt8)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(image) + " " + Go.string(delay) + " " + Go.string(loopCount) + " " + Go.string(disposal) + " " + Go.string(config) + " "
			+ Go.string(backgroundIndex) + "}";
	}
...
@:structInit class T_encoder {
	public function _writeImageBlock(_pm:Pointer<stdgo.image.Image.Paletted>, _delay:GoInt, _disposal:GoByte):Void {
		...
		if (_b.min.x < ((0 : GoInt))
			|| _b.max.x >= (((1 : GoUnTypedInt)) << ((16 : GoUnTypedInt)))
			|| _b.min.y < ((0 : GoInt))
			|| _b.max.y >= (((1 : GoUnTypedInt)) << ((16 : GoUnTypedInt)))) {
			_e.value._err = stdgo.errors.Errors.new_("gif: image block is too large to encode");
			return;
		};
		if (!_b.in_((({max: new stdgo.image.Image.Point(new stdgo.image.Image.Point(_e.value._g.config.width,
			_e.value._g.config.height).__copy__().__copy__()), min: new stdgo.image.Image.Point()} : stdgo.image.Image.Rectangle)).__copy__())) {
		...
		var _paddedSize:GoInt = _log2(_pm.value.palette.__t__.length);
		{
			...
			if (_ok
				&& _pm.value.palette.__t__.length <= _gp.__t__.length
				&& Go.pointer(_gp.__t__[((0 : GoInt))]) == Go.pointer(_pm.value.palette.__t__[((0 : GoInt))])) {
				_e.value._writeByte(((0 : GoUInt8)));
			} else {
				var __tmp__ = _encodeColorTable(_e.value._localColorTable.__slice__(0), _pm.value.palette.__copy__(), _paddedSize),
					_ct:GoInt = __tmp__._0,
					_err:stdgo.Error = __tmp__._1;
				if (_err != null) {
					if (_e.value._err == null) {
						_e.value._err = _err;
	...
	public function _colorTablesMatch(_localLen:GoInt, _transparentIndex:GoInt):Bool {
		...
		if (_transparentIndex >= ((0 : GoInt))) {
			...
			return stdgo.bytes.Bytes.equal(_e.value._globalColorTable.__slice__(0, _trOff), _e.value._localColorTable.__slice__(0, _trOff))
				&& stdgo.bytes.Bytes.equal(_e.value._globalColorTable.__slice__(_trOff + ((3 : GoInt)), _localSize),
					_e.value._localColorTable.__slice__(_trOff + ((3 : GoInt)), _localSize));
		};
	...
	public function _writeHeader():Void {
		...
		if (_e.value._g.image.length > ((1 : GoInt)) && _e.value._g.loopCount >= ((0 : GoInt))) {
			...
			_e.value._write(_e.value._buf.__slice__(0, ((3 : GoInt))));
			var __tmp__ = stdgo.io.Io.writeString(_e.value._w, "NETSCAPE2.0"),
				_:GoInt = __tmp__._0,
				_err:stdgo.Error = __tmp__._1;
			if (_err != null && _e.value._err == null) {
				_e.value._err = _err;
	...
	public function new(?_w:T_writer, ?_err:stdgo.Error, ?_g:GIF, ?_globalCT:GoInt, ?_buf:GoArray<GoUInt8>, ?_globalColorTable:GoArray<GoUInt8>,
			?_localColorTable:GoArray<GoUInt8>)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(_w) + " " + Go.string(_err) + " " + Go.string(_g) + " " + Go.string(_globalCT) + " " + Go.string(_buf) + " "
			+ Go.string(_globalColorTable) + " " + Go.string(_localColorTable) + "}";
	}
...
@:structInit class T_blockWriter {
	...
	public function new(?_e:Pointer<T_encoder>)
		stdgo.internal.Macro.initLocals();
...
@:structInit class Options {
	...
	public function new(?numColors:GoInt, ?quantizer:stdgo.image.draw.Draw.Quantizer, ?drawer:stdgo.image.draw.Draw.Drawer)
		stdgo.internal.Macro.initLocals();
...
var _interlacing:Slice<T_interlaceScan> = new Slice<T_interlaceScan>(new T_interlaceScan(((8 : GoInt)), ((0 : GoInt))).__copy__(),
	new T_interlaceScan(((8 : GoInt)), ((4 : GoInt))).__copy__(), new T_interlaceScan(((4 : GoInt)), ((2 : GoInt))).__copy__(),
	new T_interlaceScan(((2 : GoInt)), ((1 : GoInt))).__copy__());
...
var _log2Lookup:GoArray<GoInt> = new GoArray<GoInt>(((2 : GoInt)), ((4 : GoInt)), ((8 : GoInt)), ((16 : GoInt)), ((32 : GoInt)), ((64 : GoInt)),
	((128 : GoInt)), ((256 : GoInt))).copy();
...
function _readFull(_r:stdgo.io.Io.Reader, _b:Slice<GoByte>):Error {
	var __tmp__ = stdgo.io.Io.readFull(_r, _b),
		_:GoInt = __tmp__._0,
		_err:stdgo.Error = __tmp__._1;
...
function decodeAll(_r:stdgo.io.Io.Reader):{var _0:Pointer<GIF>; var _1:Error;} {
	...
	var _gif:Pointer<GIF> = Go.pointer((({
		image: _d._image,
		loopCount: _d._loopCount,
		delay: _d._delay,
		disposal: _d._disposal,
		config: (({colorModel: _d._globalColorTable.__copy__(), width: _d._width, height: _d._height} : stdgo.image.Image.Config)).__copy__(),
		backgroundIndex: _d._backgroundIndex
	} : GIF)));
...
function decodeConfig(_r:stdgo.io.Io.Reader):{var _0:stdgo.image.Image.Config; var _1:Error;} {
	...
	return {_0: (({colorModel: _d._globalColorTable.__copy__(), width: _d._width, height: _d._height} : stdgo.image.Image.Config)).__copy__(),
		_1: ((null : stdgo.Error))};
}
...
function _encodeColorTable(_dst:Slice<GoByte>, _p:stdgo.image.color.Color.Palette, _size:GoInt):{var _0:GoInt; var _1:Error;} {
	...
	};
	{
		...
		for (_obj in _p.__t__.keyValueIterator()) {
			...
			var _r:GoUInt8 = ((0 : GoUInt8)),
				_g:GoUInt8 = ((0 : GoUInt8)),
				_b:GoUInt8 = ((0 : GoUInt8));
			{
				var __tmp__ = try {
				...
				if (_ok) {
					...
					var __tmp__ = _c.rgba(),
						_rr:GoUInt32 = __tmp__._0,
						_gg:GoUInt32 = __tmp__._1,
						_bb:GoUInt32 = __tmp__._2,
...
function encodeAll(_w:stdgo.io.Io.Writer, _g:Pointer<GIF>):Error {
	...
	var _e:T_encoder = (({
		_g: _g.value.__copy__(),
		_w: ((null : T_writer)),
		_err: ((null : stdgo.Error)),
		_globalCT: 0,
		_buf: new GoArray<GoUInt8>(...[for (i in 0...256) ((0 : GoUInt8))]),
		_globalColorTable: new GoArray<GoUInt8>(...[for (i in 0...768) ((0 : GoUInt8))]),
		_localColorTable: new GoArray<GoUInt8>(...[for (i in 0...768) ((0 : GoUInt8))])
	} : T_encoder)).__copy__();
...
function encode(_w:stdgo.io.Io.Writer, _m:stdgo.image.Image.Image, _o:Pointer<Options>):Error {
	...
	if (_pm == null || _pm.isNil() || _pm.value.palette.__t__.length > _opts.numColors) {
		_pm = stdgo.image.Image.newPaletted(_b.__copy__(),
			new stdgo.image.color.Color.Palette(stdgo.image.color.palette.Palette.plan9.__slice__(0, _opts.numColors)));
		if (_opts.quantizer != null) {
			_pm.value.palette = _opts.quantizer.quantize(new stdgo.image.color.Color.Palette(new Slice<stdgo.image.color.Color.Color>(...[
				for (i in 0...((((0 : GoInt)) : GoInt)).toBasic()) ((null : stdgo.image.color.Color.Color))
			]).setCap(((_opts.numColors : GoInt)).toBasic())).__copy__(), _m).__copy__();
		};
	...
	return encodeAll(_w, Go.pointer((({
		image: new Slice<Pointer<stdgo.image.Image.Paletted>>(_pm),
		delay: new Slice<GoInt>(((0 : GoInt))),
		config: (({colorModel: _pm.value.palette.__copy__(), width: _b.dx(), height: _b.dy()} : stdgo.image.Image.Config)).__copy__(),
		loopCount: 0,
		disposal: new Slice<GoUInt8>().nil(),
		backgroundIndex: 0
	} : GIF))));
}
...
class T_decoder_extension_fields {
	public function _decode(__tmp__, _r:stdgo.io.Io.Reader, _configOnly:Bool, _keepAllFrames:Bool):Error
		return __tmp__._decode(_r, _configOnly, _keepAllFrames);

	public function _readHeaderAndScreenDescriptor(__tmp__):Error
		return __tmp__._readHeaderAndScreenDescriptor();

	public function _readColorTable(__tmp__, _fields:GoByte):{var _0:stdgo.image.color.Color.Palette; var _1:Error;}
		return __tmp__._readColorTable(_fields);

	public function _readExtension(__tmp__):Error
		return __tmp__._readExtension();

	public function _readGraphicControl(__tmp__):Error
		return __tmp__._readGraphicControl();

	public function _readImageDescriptor(__tmp__, _keepAllFrames:Bool):Error
		return __tmp__._readImageDescriptor(_keepAllFrames);

	public function _newImageFromDescriptor(__tmp__):{var _0:Pointer<stdgo.image.Image.Paletted>; var _1:Error;}
		return __tmp__._newImageFromDescriptor();

	public function _readBlock(__tmp__):{var _0:GoInt; var _1:Error;}
		return __tmp__._readBlock();
}

class T_blockReader_extension_fields {
	public function _fill(__tmp__):Void
		__tmp__._fill();
...
class T_encoder_extension_fields {
	...
	public function _write(__tmp__, _p:Slice<GoByte>):Void
		__tmp__._write(_p);

	public function _writeByte(__tmp__, _b:GoByte):Void
		__tmp__._writeByte(_b);

	public function _writeHeader(__tmp__):Void
		__tmp__._writeHeader();

	public function _colorTablesMatch(__tmp__, _localLen:GoInt, _transparentIndex:GoInt):Bool
		return __tmp__._colorTablesMatch(_localLen, _transparentIndex);

	public function _writeImageBlock(__tmp__, _pm:Pointer<stdgo.image.Image.Paletted>, _delay:GoInt, _disposal:GoByte):Void
		__tmp__._writeImageBlock(_pm, _delay, _disposal);
}

class T_blockWriter_extension_fields {
	public function _setup(__tmp__):Void
		__tmp__._setup();
