...
@:named @:structInit class Error extends stdgo.os.exec.Exec.Error {
    ...
    public override function __copy__() return new Error(name, err);
}
...
function _relError(_file:GoString, _path:GoString):Error {
        return stdgo.fmt.Fmt.errorf("%s resolves to executable relative to current directory (.%c%s)", Go.toInterface(_file), Go.toInterface(stdgo.path.filepath.Filepath.separator), Go.toInterface(_path));
    }
...
function _fixCmd(_name:GoString, _cmd:Pointer<stdgo.os.exec.Exec.Cmd>):Void {
        if (stdgo.path.filepath.Filepath.base(_name) == _name && !stdgo.path.filepath.Filepath.isAbs(_cmd.value.path)) {
            var _lookPathErr:Pointer<stdgo.Error> = ((((stdgo.reflect.Reflect.valueOf(Go.toInterface(_cmd)).elem().fieldByName("lookPathErr").addr().pointer() : stdgo.unsafe.Unsafe.Pointer_)) : Pointer<Error>));
