...
@:named @:structInit class Cmd extends stdgo.os.exec.Exec.Cmd {
	public function new(?path, ?args, ?env, ?dir, ?stdin, ?stdout, ?stderr, ?extraFiles, ?sysProcAttr, ?process, ?processState, ?_ctx, ?_lookPathErr,
			?_finished, ?_childFiles, ?_closeAfterStart, ?_closeAfterWait, ?_goroutine, ?_errch, ?_waitDone)
		super(path, args, env, dir, stdin, stdout, stderr, extraFiles, sysProcAttr, process, processState, _ctx, _lookPathErr, _finished, _childFiles,
			_closeAfterStart, _closeAfterWait, _goroutine, _errch, _waitDone);

	public override function __copy__()
		return new Cmd(path, args, env, dir, stdin, stdout, stderr, extraFiles, sysProcAttr, process, processState, _ctx, _lookPathErr, _finished,
			_childFiles, _closeAfterStart, _closeAfterWait, _goroutine, _errch, _waitDone);
}

@:named @:structInit class Error extends stdgo.os.exec.Exec.Error {
	public function new(?name, ?err)
		super(name, err);
...
@:named @:structInit class ExitError extends stdgo.os.exec.Exec.ExitError {
	public function new(?processState, ?stderr)
		super(processState, stderr);
...
function _relError(_file:GoString, _path:GoString):Error {
	return stdgo.fmt.Fmt.errorf("%s resolves to executable relative to current directory (.%c%s)", Go.toInterface(_file),
		Go.toInterface(stdgo.path.filepath.Filepath.separator), Go.toInterface(_path));
}

function lookPath(_file:GoString):{var _0:GoString; var _1:Error;} {
	var __tmp__ = stdgo.os.exec.Exec.lookPath(_file),
...
function _fixCmd(_name:GoString, _cmd:Pointer<stdgo.os.exec.Exec.Cmd>):Void {
	if (stdgo.path.filepath.Filepath.base(_name) == _name && !stdgo.path.filepath.Filepath.isAbs(_cmd.value.path)) {
		var _lookPathErr:Pointer<stdgo.Error> = ((((stdgo.reflect.Reflect.valueOf(Go.toInterface(_cmd))
			.elem()
			.fieldByName("lookPathErr")
			.addr()
			.pointer() : stdgo.unsafe.Unsafe.Pointer_)) : Pointer<Error>));
		if (_lookPathErr.value == null) {
