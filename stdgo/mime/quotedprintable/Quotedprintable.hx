...
@:structInit class Reader {
		...
		while (_p.length > ((0 : GoInt))) {
			if (_r.value._line.length == ((0 : GoInt))) {
				...
				if (stdgo.bytes.Bytes.hasSuffix(_r.value._line, _softSuffix)) {
					...
					if (!stdgo.bytes.Bytes.hasPrefix(_rightStripped, _lf)
						&& !stdgo.bytes.Bytes.hasPrefix(_rightStripped, _crlf)
						&& !(_rightStripped.length == ((0 : GoInt))
							&& _r.value._line.length > ((0 : GoInt))
							&& Go.toInterface(_r.value._rerr) == Go.toInterface(stdgo.io.Io.eof))) {
			...
			var _b:GoUInt8 = _r.value._line[((0 : GoInt))];
			{
				...
				while (true) {
					if (_b == (("=".code : GoRune))) {
						...
						if (_err != null) {
							if (_r.value._line.length >= ((2 : GoInt))
								&& _r.value._line[((1 : GoInt))] != (("\r".code : GoRune))
								&& _r.value._line[((1 : GoInt))] != (("\n".code : GoRune))) {
								_b = (("=".code : GoRune));
								break;
							};
	...
	public function new(?_br:Pointer<bufio.Bufio.Reader>, ?_rerr:stdgo.Error, ?_line:Slice<GoUInt8>)
...
@:structInit class Writer {
	public function _flush():Error {
		...
		{
			var __tmp__ = _w.value._w.write(_w.value._line.__slice__(0, _w.value._i)),
				_:GoInt = __tmp__._0,
	...
	public function new(?binary:Bool, ?_w:stdgo.io.Io.Writer, ?_i:GoInt, ?_line:GoArray<GoUInt8>, ?_cr:Bool)
		stdgo.internal.Macro.initLocals();
...
function newWriter(_w:stdgo.io.Io.Writer):Pointer<Writer> {
	return Go.pointer((({
		...
		binary: false,
		_i: 0,
		_line: new GoArray<GoUInt8>(...[for (i in 0...78) ((0 : GoUInt8))]),
		_cr: false
	} : Writer)));
}
...
class Writer_extension_fields {
	...
	public function _write(__tmp__, _p:Slice<GoByte>):Error
		return __tmp__._write(_p);

	public function _encode(__tmp__, _b:GoByte):Error
		return __tmp__._encode(_b);

	public function _checkLastByte(__tmp__):Error
		return __tmp__._checkLastByte();

	public function _insertSoftLineBreak(__tmp__):Error
		return __tmp__._insertSoftLineBreak();

	public function _insertCRLF(__tmp__):Error
		return __tmp__._insertCRLF();
