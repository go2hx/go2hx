...
@:structInit class T_request {
	public function _parseParams():Void {
		...
		while (_text.length > ((0 : GoInt))) {
			var __tmp__ = _readSize(_text),
				_keyLen:GoUInt32 = __tmp__._0,
				_n:GoInt = __tmp__._1;
			if (_n == ((0 : GoInt))) {
				return;
			};
	...
	public var _params:GoMap<GoString, GoString> = new GoMap<GoString,
	...
	public function new(?_pw:Pointer<stdgo.io.Io.PipeWriter>, ?_reqId:GoUInt16, ?_params:GoMap<GoString, GoString>, ?_buf:GoArray<GoUInt8>,
			?_rawParams:Slice<GoUInt8>, ?_keepConn:Bool)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(_pw) + " " + Go.string(_reqId) + " " + Go.string(_params) + " " + Go.string(_buf) + " " + Go.string(_rawParams) + " "
			+ Go.string(_keepConn) + "}";
	}
...
@:structInit class T_response {
	...
	public function _writeCGIHeader(_p:Slice<GoByte>):Void {
		...
		stdgo.fmt.Fmt.fprintf(_r.value._w.value, "Status: %d %s\r\n", Go.toInterface(_r.value._code),
			Go.toInterface(stdgo.net.http.Http.statusText(_r.value._code)));
		{
			var __tmp__ = _r.value._header.exists("Content-Type") ? {value: _r.value._header["Content-Type"],
				ok: true} : {value: _r.value._header.defaultValue(), ok: false},
				_:Slice<GoString> = __tmp__.value,
				_hasType:Bool = __tmp__.ok;
	...
	public function new(?_req:Pointer<T_request>, ?_header:stdgo.net.http.Http.Header, ?_code:GoInt, ?_wroteHeader:Bool, ?_wroteCGIHeader:Bool,
			?_w:Pointer<T_bufWriter>)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(_req) + " " + Go.string(_header) + " " + Go.string(_code) + " " + Go.string(_wroteHeader) + " " + Go.string(_wroteCGIHeader)
			+ " " + Go.string(_w) + "}";
	}
...
@:structInit class T_child {
	...
	public function _serveRequest(_req:Pointer<T_request>, _body:stdgo.io.Io.ReadCloser):Void {
		...
		var __tmp__ = stdgo.net.http.cgi.Cgi.requestFromMap(_req.value._params),
			_httpReq:Pointer<stdgo.net.http.Http.Request> = __tmp__._0,
		...
		if (_err != null) {
			...
			var _envVarCtx:stdgo.context.Context.Context = stdgo.context.Context.withValue(_httpReq.value.context(),
				Go.toInterface(new T_envVarsContextKey().__copy__()), Go.toInterface(_withoutUsedEnvVars));
	...
	public function _handleRecord(_rec:Pointer<T_record>):Error {
		var _c = new Pointer(() -> this, __tmp__ -> this.__set__(__tmp__));
		var __tmp__ = _c.value._requests.exists(_rec.value._h.id) ? {value: _c.value._requests[_rec.value._h.id],
			ok: true} : {value: _c.value._requests.defaultValue(), ok: false},
			_req:Pointer<T_request> = __tmp__.value,
			_ok:Bool = __tmp__.ok;
		...
		if (_rec.value._h.type.__t__ == _typeBeginRequest.__t__) {
			...
			var _values:GoMap<GoString, GoString> = new GoMap<GoString,
				GoString>(new stdgo.reflect.Reflect._Type(stdgo.reflect.Reflect.GoType.mapType(stdgo.reflect.Reflect.GoType.basic(string_kind),
	...
	public var _requests:GoMap<GoUInt16, Pointer<T_request>> = new GoMap<GoUInt16,
		Pointer<T_request>>(new stdgo.reflect.Reflect._Type(stdgo.reflect.Reflect.GoType.mapType(stdgo.reflect.Reflect.GoType.basic(uint16_kind),
		stdgo.reflect.Reflect.GoType.pointer(stdgo.reflect.Reflect.GoType.named("net/http/fcgi.request", [],
			stdgo.reflect.Reflect.GoType.named("net/http/fcgi.request", [], stdgo.reflect.Reflect.GoType.structType([
				{
					name: "_pw",
					embedded: false,
					tag: "",
					type: stdgo.reflect.Reflect.GoType.pointer(stdgo.reflect.Reflect.GoType.named("io.PipeWriter", [],
						stdgo.reflect.Reflect.GoType.structType([
							{
								name: "_p",
								embedded: false,
								tag: "",
								type: stdgo.reflect.Reflect.GoType.pointer(stdgo.reflect.Reflect.GoType.named("io.pipe", [],
									stdgo.reflect.Reflect.GoType.structType([
										{
											name: "_wrMu",
											embedded: false,
										...
										},
										{
											...
											embedded: false,
											tag: "",
											type: stdgo.reflect.Reflect.GoType.chanType(0, stdgo.reflect.Reflect.GoType.basic(int_kind))
										},
										{
											name: "_once",
											embedded: false,
											tag: "",
											type: stdgo.reflect.Reflect.GoType.named("sync.Once", [], stdgo.reflect.Reflect.GoType.structType([
												{
													name: "_done",
													embedded: false,
													tag: "",
										...
										},
										{
											...
											embedded: false,
											tag: "",
											type: stdgo.reflect.Reflect.GoType.named("io.onceError", [], stdgo.reflect.Reflect.GoType.structType([
												{
													name: "mutex",
													embedded: true,
													tag: "",
										...
										},
										{
											...
											embedded: false,
											tag: "",
											type: stdgo.reflect.Reflect.GoType.named("io.onceError", [], stdgo.reflect.Reflect.GoType.invalidType)
										}
									])))
							}
						])))
				...
				},
				{
					...
					embedded: false,
					tag: "",
					type: stdgo.reflect.Reflect.GoType.arrayType(stdgo.reflect.Reflect.GoType.basic(uint8_kind), 1024)
				},
				{
					name: "_rawParams",
					embedded: false,
	...
	public function new(?_conn:Pointer<T_conn>, ?_handler:stdgo.net.http.Http.Handler, ?_requests:GoMap<GoUInt16, Pointer<T_request>>)
		stdgo.internal.Macro.initLocals();
...
@:structInit class T_header {
	...
	public function new(?version:GoUInt8, ?type:T_recType, ?id:GoUInt16, ?contentLength:GoUInt16, ?paddingLength:GoUInt8, ?reserved:GoUInt8)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(version) + " " + Go.string(type) + " " + Go.string(id) + " " + Go.string(contentLength) + " " + Go.string(paddingLength)
			+ " " + Go.string(reserved) + "}";
	}
...
@:structInit class T_beginRequest {
	...
	public function new(?_role:GoUInt16, ?_flags:GoUInt8, ?_reserved:GoArray<GoUInt8>)
		stdgo.internal.Macro.initLocals();
...
@:structInit class T_conn {
	public function _writePairs(_recType:T_recType, _reqId:GoUInt16, _pairs:GoMap<GoString, GoString>):Error {
		...
		var _b:Slice<GoUInt8> = new Slice<GoUInt8>(...[for (i in 0...((((8 : GoInt)) : GoInt)).toBasic()) ((0 : GoUInt8))]);
		{
			...
			for (_obj in _pairs.keyValueIterator()) {
				...
				{
					var __tmp__ = _w.value.write(_b.__slice__(0, _n)),
						_:GoInt = __tmp__._0,
						_err:stdgo.Error = __tmp__._1;
					if (_err != null) {
						return _err;
				...
				{
					var __tmp__ = _w.value.writeString(_k),
						_:GoInt = __tmp__._0,
						_err:stdgo.Error = __tmp__._1;
					if (_err != null) {
						return _err;
				...
				{
					var __tmp__ = _w.value.writeString(_v),
						_:GoInt = __tmp__._0,
						_err:stdgo.Error = __tmp__._1;
					if (_err != null) {
						return _err;
	...
	public function _writeRecord(_recType:T_recType, _reqId:GoUInt16, _b:Slice<GoByte>):Error {
		...
		try {
			...
			{
				var _err:stdgo.Error = stdgo.encoding.binary.Binary.write(Go.pointer(_c.value._buf).value, stdgo.encoding.binary.Binary.bigEndian.__copy__(),
					Go.toInterface(_c.value._h.__copy__()));
				if (_err != null) {
					{
						for (defer in deferstack) {
							defer();
			...
			{
				var __tmp__ = _c.value._buf.write(_b),
					_:GoInt = __tmp__._0,
					_err:stdgo.Error = __tmp__._1;
				if (_err != null) {
					{
			...
			{
				var __tmp__ = _c.value._buf.write(_pad.__slice__(0, _c.value._h.paddingLength)),
					_:GoInt = __tmp__._0,
					_err:stdgo.Error = __tmp__._1;
				if (_err != null) {
					{
			...
			var __tmp__ = _c.value._rwc.write(_c.value._buf.bytes()),
				_:GoInt = __tmp__._0,
				_err:stdgo.Error = __tmp__._1;
			{
				for (defer in deferstack) {
	...
	public function new(?_mutex:stdgo.sync.Sync.Mutex, ?_rwc:stdgo.io.Io.ReadWriteCloser, ?_buf:stdgo.bytes.Bytes.Buffer, ?_h:T_header)
		stdgo.internal.Macro.initLocals();
...
@:structInit class T_record {
	...
	public function new(?_h:T_header, ?_buf:GoArray<GoUInt8>)
		stdgo.internal.Macro.initLocals();
...
@:structInit class T_bufWriter {
	...
	public function new(?_closer:stdgo.io.Io.Closer, ?writer:Pointer<bufio.Bufio.Writer>)
		stdgo.internal.Macro.initLocals();
	...
	public function readFrom(_r:stdgo.io.Io.Reader):{var _0:GoInt64; var _1:stdgo.Error;}
		return writer.readFrom(_0);

	public function reset(_w:stdgo.io.Io.Writer)
		writer.reset(_0);
	...
	public function writeByte(_c:GoUInt8):stdgo.Error
		return writer.writeByte(_0);

	public function writeRune(_r:GoInt32):{var _0:GoInt; var _1:stdgo.Error;}
		return writer.writeRune(_0);

	public function writeString(_s:GoString):{var _0:GoString -> {
		var _0:GoInt;
		var _1:stdgo.Error;
	}; var _1:GoString -> {
		var _0:GoInt;
...
@:structInit class T_streamWriter {
	...
	public function new(?_c:Pointer<T_conn>, ?_recType:T_recType, ?_reqId:GoUInt16)
		stdgo.internal.Macro.initLocals();
...
function _newRequest(_reqId:GoUInt16, _flags:GoUInt8):Pointer<T_request> {
	var _r:Pointer<T_request> = Go.pointer((({
		_reqId: _reqId,
		_params: new GoMap<GoString,
			GoString>(new stdgo.reflect.Reflect._Type(stdgo.reflect.Reflect.GoType.mapType(stdgo.reflect.Reflect.GoType.basic(string_kind),
				stdgo.reflect.Reflect.GoType.basic(string_kind)))),
		_keepConn: _flags & _flagKeepConn != ((0 : GoUInt8)),
		_pw: new Pointer<stdgo.io.Io.PipeWriter>().nil(),
		_buf: new GoArray<GoUInt8>(...[for (i in 0...1024) ((0 : GoUInt8))]),
		_rawParams: new Slice<GoUInt8>().nil()
	} : T_request)));
...
function _newResponse(_c:Pointer<T_child>, _req:Pointer<T_request>):Pointer<T_response> {
	return Go.pointer((({
		_req: _req,
		_header: new stdgo.net.http.Http.Header(new GoMap<GoString,
		...
		_w: _newWriter(_c.value._conn, _typeStdout, _req.value._reqId),
		_code: 0,
		_wroteHeader: false,
		_wroteCGIHeader: false
	} : T_response)));
}

function _newChild(_rwc:stdgo.io.Io.ReadWriteCloser, _handler:stdgo.net.http.Http.Handler):Pointer<T_child> {
	return Go.pointer((({_conn: _newConn(_rwc), _handler: _handler, _requests: new GoMap<GoUInt16,
		Pointer<T_request>>(new stdgo.reflect.Reflect._Type(stdgo.reflect.Reflect.GoType.mapType(stdgo.reflect.Reflect.GoType.basic(uint16_kind),
		stdgo.reflect.Reflect.GoType.pointer(stdgo.reflect.Reflect.GoType.named("net/http/fcgi.request", [],
			stdgo.reflect.Reflect.GoType.named("net/http/fcgi.request", [], stdgo.reflect.Reflect.GoType.structType([
				{
...
function _filterOutUsedEnvVars(_envVars:GoMap<GoString, GoString>):GoMap<GoString, GoString> {
	var _withoutUsedEnvVars:GoMap<GoString, GoString> = new GoMap<GoString,
		GoString>(new stdgo.reflect.Reflect._Type(stdgo.reflect.Reflect.GoType.mapType(stdgo.reflect.Reflect.GoType.basic(string_kind),
			stdgo.reflect.Reflect.GoType.basic(string_kind))));
	{
		var _k;
...
function serve(_l:stdgo.net.Net.Listener, _handler:stdgo.net.http.Http.Handler):Error {
	...
	try {
		...
		while (true) {
			var __tmp__ = _l.accept(),
				_rw:stdgo.net.Net.Conn = __tmp__._0,
				_err:stdgo.Error = __tmp__._1;
			if (_err != null) {
				{
					for (defer in deferstack) {
...
function processEnv(_r:Pointer<stdgo.net.http.Http.Request>):GoMap<GoString, GoString> {
	var __tmp__ = try {
		...
		{value: new GoMap<GoString,
			GoString>(new stdgo.reflect.Reflect._Type(stdgo.reflect.Reflect.GoType.mapType(stdgo.reflect.Reflect.GoType.basic(string_kind),
				stdgo.reflect.Reflect.GoType.basic(string_kind)))).nil(), ok: false};
...
function _addFastCGIEnvToContext(_s:GoString):Bool {
	if (_s == (("CONTENT_LENGTH" : GoString))
		|| _s == (("CONTENT_TYPE" : GoString))
		|| _s == (("HTTPS" : GoString))
		|| _s == (("PATH_INFO" : GoString))
		|| _s == (("QUERY_STRING" : GoString))
		|| _s == (("REMOTE_ADDR" : GoString))
		|| _s == (("REMOTE_HOST" : GoString))
		|| _s == (("REMOTE_PORT" : GoString))
		|| _s == (("REQUEST_METHOD" : GoString))
		|| _s == (("REQUEST_URI" : GoString))
		|| _s == (("SCRIPT_NAME" : GoString))
		|| _s == (("SERVER_PROTOCOL" : GoString))) {
		return false;
	};
...
function _newConn(_rwc:stdgo.io.Io.ReadWriteCloser):Pointer<T_conn> {
	return Go.pointer((({
		...
		_buf: new stdgo.bytes.Bytes.Buffer(),
		_h: new T_header()
	} : T_conn)));
}
...
function _readSize(_s:Slice<GoByte>):{var _0:GoUInt32; var _1:GoInt;} {
	...
	var _size:GoUInt32 = ((_s[((0 : GoInt))] : GoUInt32)),
		_n:GoInt = ((1 : GoInt));
...
class T_request_extension_fields {
	public function _parseParams(__tmp__):Void
		__tmp__._parseParams();
}

class T_response_extension_fields {
	public function header(__tmp__):stdgo.net.http.Http.Header
		return __tmp__.header();

	public function write(__tmp__, _p:Slice<GoByte>):{var _0:GoInt; var _1:Error;}
		return __tmp__.write(_p);

	public function writeHeader(__tmp__, _code:GoInt):Void
		__tmp__.writeHeader(_code);

	public function _writeCGIHeader(__tmp__, _p:Slice<GoByte>):Void
		__tmp__._writeCGIHeader(_p);

	public function flush(__tmp__):Void
		__tmp__.flush();
...
class T_child_extension_fields {
	public function _serve(__tmp__):Void
		__tmp__._serve();

	public function _handleRecord(__tmp__, _rec:Pointer<T_record>):Error
		return __tmp__._handleRecord(_rec);

	public function _serveRequest(__tmp__, _req:Pointer<T_request>, _body:stdgo.io.Io.ReadCloser):Void
		__tmp__._serveRequest(_req, _body);

	public function _cleanUp(__tmp__):Void
		__tmp__._cleanUp();
}

class T_header_extension_fields {
	public function _init(__tmp__, _recType:T_recType, _reqId:GoUInt16, _contentLength:GoInt):Void
		__tmp__._init(_recType, _reqId, _contentLength);
}

class T_beginRequest_extension_fields {
	public function _read(__tmp__, _content:Slice<GoByte>):Error
		return __tmp__._read(_content);
}
...
class T_conn_extension_fields {
	...
	public function _writeRecord(__tmp__, _recType:T_recType, _reqId:GoUInt16, _b:Slice<GoByte>):Error
		return __tmp__._writeRecord(_recType, _reqId, _b);

	public function _writeEndRequest(__tmp__, _reqId:GoUInt16, _appStatus:GoInt, _protocolStatus:GoUInt8):Error
		return __tmp__._writeEndRequest(_reqId, _appStatus, _protocolStatus);

	public function _writePairs(__tmp__, _recType:T_recType, _reqId:GoUInt16, _pairs:GoMap<GoString, GoString>):Error
		return __tmp__._writePairs(_recType, _reqId, _pairs);
}

class T_record_extension_fields {
	public function _read(__tmp__, _r:stdgo.io.Io.Reader):Error
		return __tmp__._read(_r);

	public function _content(__tmp__):Slice<GoByte>
		return __tmp__._content();
}
