...
@:structInit class ClientTrace {
	...
	public function _compose(_old:Pointer<ClientTrace>):Void {
		...
		var _structType:stdgo.reflect.Reflect.Type = _tv.type();
		{
			...
			Go.cfor(_i < _structType.numField(), _i++, {
				...
				var _newFunc:stdgo.reflect.Reflect.Value = stdgo.reflect.Reflect.makeFunc(_hookType,
					function(_args:Slice<stdgo.reflect.Reflect.Value>):Slice<stdgo.reflect.Reflect.Value> {
						_tfCopy.call(_args);
	...
	public function new(?getConn:GoString->Void, ?gotConn:GotConnInfo->Void, ?putIdleConn:stdgo.Error->Void, ?gotFirstResponseByte:() -> Void,
			?got100Continue:() -> Void, ?got1xxResponse:(GoInt, stdgo.net.textproto.Textproto.MIMEHeader) -> stdgo.Error, ?dnsstart:DNSStartInfo->Void,
			?dnsdone:DNSDoneInfo->Void, ?connectStart:(GoString, GoString) -> Void, ?connectDone:(GoString, GoString, stdgo.Error) -> Void,
			?tlshandshakeStart:() -> Void, ?tlshandshakeDone:(stdgo.crypto.tls.Tls.ConnectionState, stdgo.Error) -> Void,
			?wroteHeaderField:(GoString, Slice<GoString>) -> Void, ?wroteHeaders:() -> Void, ?wait100Continue:() -> Void, ?wroteRequest:WroteRequestInfo->Void)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(getConn) + " " + Go.string(gotConn) + " " + Go.string(putIdleConn) + " " + Go.string(gotFirstResponseByte) + " "
			+ Go.string(got100Continue) + " " + Go.string(got1xxResponse) + " " + Go.string(dnsstart) + " " + Go.string(dnsdone) + " "
			+ Go.string(connectStart) + " " + Go.string(connectDone) + " " + Go.string(tlshandshakeStart) + " " + Go.string(tlshandshakeDone) + " "
			+ Go.string(wroteHeaderField) + " " + Go.string(wroteHeaders) + " " + Go.string(wait100Continue) + " " + Go.string(wroteRequest) + "}";
	}
	...
	public function __copy__() {
		return new ClientTrace(getConn, gotConn, putIdleConn, gotFirstResponseByte, got100Continue, got1xxResponse, dnsstart, dnsdone, connectStart,
			connectDone, tlshandshakeStart, tlshandshakeDone, wroteHeaderField, wroteHeaders, wait100Continue, wroteRequest);
	}
...
@:structInit class DNSDoneInfo {
	...
	public function new(?addrs:Slice<stdgo.net.Net.IPAddr>, ?err:stdgo.Error, ?coalesced:Bool)
		stdgo.internal.Macro.initLocals();
...
@:structInit class GotConnInfo {
	...
	public function new(?conn:stdgo.net.Net.Conn, ?reused:Bool, ?wasIdle:Bool, ?idleTime:stdgo.time.Time.Duration)
		stdgo.internal.Macro.initLocals();
...
function withClientTrace(_ctx:stdgo.context.Context.Context, _trace:Pointer<ClientTrace>):stdgo.context.Context.Context {
	...
	if (_trace.value._hasNetHooks()) {
		var _nt:Pointer<internal.nettrace.Nettrace.Trace> = Go.pointer((({
			connectStart: _trace.value.connectStart,
			connectDone: _trace.value.connectDone,
			dnsstart: null,
			dnsdone: null
		} : internal.nettrace.Nettrace.Trace)));
		...
		if (_trace.value.dnsdone != null) {
			_nt.value.dnsdone = function(_netIPs:Slice<AnyInterface>, _coalesced:Bool, _err:Error):Void {
				var _addrs:Slice<stdgo.net.Net.IPAddr> = new Slice<stdgo.net.Net.IPAddr>(...[for (i in 0...((_netIPs.length : GoInt))
					.toBasic()) new stdgo.net.Net.IPAddr()]);
				{
					var _i;
					var _ip;
...
class ClientTrace_extension_fields {
	public function _compose(__tmp__, _old:Pointer<ClientTrace>):Void
		__tmp__._compose(_old);

	public function _hasNetHooks(__tmp__):Bool
		return __tmp__._hasNetHooks();
}
