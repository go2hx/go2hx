...
function _testSetGetenv(_t:Ref<stdgo.testing.Testing.T>, _key:GoString, _value:GoString):Void {
	...
	if (_err != null) {
		_t.fatalf(Go.str("Setenv failed to set %q: %v"), Go.toInterface(_value), Go.toInterface(_err));
	};
...
function testEnv(_t:Ref<stdgo.testing.Testing.T>):Void {
	_testSetGetenv(_t, Go.str("TESTENV"), Go.str("AVALUE"));
	_testSetGetenv(_t, Go.str("TESTENV"), Go.str());
}
...
function testExecErrPermutedFds(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	var __tmp__ = stdgo.os.Os.startProcess(Go.str("/"), (new Slice<GoString>(0, 0, Go.str("/")) : Slice<GoString>), _attr),
		_0:Ref<stdgo.os.Os.Process> = __tmp__._0,
		_err:Error = __tmp__._1;
	if (_err == null) {
		_t.fatalf(Go.str("StartProcess of invalid program returned err = nil"));
	};
}

function testGettimeofday(_t:Ref<stdgo.testing.Testing.T>):Void {
	if (true) {
		_t.skip(Go.toInterface(Go.str("not implemented on js")));
	};
