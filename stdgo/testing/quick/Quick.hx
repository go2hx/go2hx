...
private var _defaultMaxCount:Pointer<GoInt> = stdgo.flag.Flag.int_(("quickchecks" : GoString), (100 : GoInt),
	("The default number of iterations for each check" : GoString));

/**
	// complexSize is the maximum length of arbitrary values that contain other
	// values.
**/
private final _complexSize:GoUInt64 = ("50" : GoUInt64);
...
@:structInit @:using(stdgo.testing.quick.Quick.Config_static_extension) class Config {
	...
	**/
	...
	public function new(?maxCount:GoInt, ?maxCountScale:GoFloat64, ?rand:Ref<stdgo.math.rand.Rand.Rand>,
			?values:(Slice<stdgo.reflect.Reflect.Value>, Ref<stdgo.math.rand.Rand.Rand>) -> Void) {
		if (maxCount != null)
			this.maxCount = maxCount;
		if (maxCountScale != null)
...
**/
@:structInit @:using(stdgo.testing.quick.Quick.T_myStruct_static_extension) private class T_myStruct {
...
**/
private function _sizedValue(_t:stdgo.reflect.Reflect.Type, _rand:Ref<stdgo.math.rand.Rand.Rand>,
		_size:GoInt):{var _0:stdgo.reflect.Reflect.Value; var _1:Bool;} {
	var _value:stdgo.reflect.Reflect.Value = ({} : stdgo.reflect.Reflect.Value),
		_ok:Bool = false;
	{
		var __tmp__ = try {
	...
	var _v:stdgo.reflect.Reflect.Value = (stdgo.reflect.Reflect.new_(_t)
		.elem() == null ? null : stdgo.reflect.Reflect.new_(_t)
		.elem()
		.__copy__());
	{
		...
		if (_concrete.kind() == ((("1" : GoUInt) : stdgo.reflect.Reflect.Kind))) {
			...
			_v.set((stdgo.reflect.Reflect.makeMap(_concrete) == null ? null : stdgo.reflect.Reflect.makeMap(_concrete).__copy__()));
			{
				...
				Go.cfor(_i < _numElems, _i++, {
					var __tmp__ = _sizedValue(_concrete.key(), _rand, _size),
						_key:stdgo.reflect.Reflect.Value = __tmp__._0,
						_ok1:Bool = __tmp__._1;
					var __tmp__ = _sizedValue(_concrete.elem(), _rand, _size),
						_value:stdgo.reflect.Reflect.Value = __tmp__._0,
			...
			if (_rand.intn(_size) == ((0 : GoInt))) {
				...
				var __tmp__ = _sizedValue(_concrete.elem(), _rand, _size),
					_elem:stdgo.reflect.Reflect.Value = __tmp__._0,
					_ok:Bool = __tmp__._1;
				if (!_ok) {
					return {_0: (new stdgo.reflect.Reflect.Value() : stdgo.reflect.Reflect.Value), _1: false};
				};
			...
			_v.set((stdgo.reflect.Reflect.makeSlice(_concrete, _numElems,
				_numElems) == null ? null : stdgo.reflect.Reflect.makeSlice(_concrete, _numElems, _numElems).__copy__()));
			{
				var _i:GoInt = (0 : GoInt);
				Go.cfor(_i < _numElems, _i++, {
					var __tmp__ = _sizedValue(_concrete.elem(), _rand, _sizeLeft),
						_elem:stdgo.reflect.Reflect.Value = __tmp__._0,
						_ok:Bool = __tmp__._1;
					if (!_ok) {
						return {_0: (new stdgo.reflect.Reflect.Value() : stdgo.reflect.Reflect.Value), _1: false};
			...
			};
			{
				var _i:GoInt = (0 : GoInt);
				Go.cfor(_i < _n, _i++, {
					var __tmp__ = _sizedValue(_concrete.field(_i).type, _rand, _sizeLeft),
						_elem:stdgo.reflect.Reflect.Value = __tmp__._0,
						_ok:Bool = __tmp__._1;
					if (!_ok) {
						return {_0: (new stdgo.reflect.Reflect.Value() : stdgo.reflect.Reflect.Value), _1: false};
...
function check(_f:AnyInterface, _config:Ref<Config>):Error {
	...
	var __tmp__ = _functionAndType(_f),
		_fVal:stdgo.reflect.Reflect.Value = __tmp__._0,
		_fType:stdgo.reflect.Reflect.Type = __tmp__._1,
		_ok:Bool = __tmp__._2;
	if (!_ok) {
	...
	var _arguments = new Slice<stdgo.reflect.Reflect.Value>((_fType.numIn() : GoInt).toBasic(), 0, ...[
		for (i in 0...(_fType.numIn() : GoInt).toBasic()) ({} : stdgo.reflect.Reflect.Value)
	]);
...
function checkEqual(_f:AnyInterface, _g:AnyInterface, _config:Ref<Config>):Error {
	...
	var __tmp__ = _functionAndType(_f),
		_x:stdgo.reflect.Reflect.Value = __tmp__._0,
		_xType:stdgo.reflect.Reflect.Type = __tmp__._1,
		_ok:Bool = __tmp__._2;
	if (!_ok) {
		return Go.asInterface((("f is not a function" : GoString) : SetupError));
	};
	var __tmp__ = _functionAndType(_g),
		_y:stdgo.reflect.Reflect.Value = __tmp__._0,
		_yType:stdgo.reflect.Reflect.Type = __tmp__._1,
		_ok:Bool = __tmp__._2;
	if (!_ok) {
	...
	var _arguments = new Slice<stdgo.reflect.Reflect.Value>((_xType.numIn() : GoInt).toBasic(), 0, ...[
		for (i in 0...(_xType.numIn() : GoInt).toBasic()) ({} : stdgo.reflect.Reflect.Value)
	]);
	var _rand = _config._getRand();
	var _maxCount:GoInt = _config._getMaxCount();
	{
		...
		Go.cfor(_i < _maxCount, _i++, {
			...
			if (!stdgo.reflect.Reflect.deepEqual(Go.toInterface(_xOut), Go.toInterface(_yOut))) {
				return Go.asInterface(((new CheckEqualError((new CheckError(_i + (1 : GoInt), _toInterfaces(_arguments)) : CheckError), _xOut,
					_yOut) : CheckEqualError) : Ref<CheckEqualError>));
			};
		});
	};
	return (null : Error);
...
**/
private function _arbitraryValues(_args:Slice<stdgo.reflect.Reflect.Value>, _f:stdgo.reflect.Reflect.Type, _config:Ref<Config>,
		_rand:Ref<stdgo.math.rand.Rand.Rand>):Error {
	...
	};
	{
		...
		Go.cfor(_j < (_args.length), _j++, {
			...
			if (!_ok) {
				_err = Go.asInterface((stdgo.fmt.Fmt.sprintf(("cannot create arbitrary value of type %s for argument %d" : GoString),
					Go.toInterface(_f.in_(_j)), Go.toInterface(_j)) : SetupError));
				return _err;
			};
		});
	};
...
private function _functionAndType(_f:AnyInterface):{var _0:stdgo.reflect.Reflect.Value; var _1:stdgo.reflect.Reflect.Type; var _2:Bool;} {
	var _v:stdgo.reflect.Reflect.Value = ({} : stdgo.reflect.Reflect.Value),
		_t:stdgo.reflect.Reflect.Type = (null : stdgo.reflect.Reflect.Type),
		_ok:Bool = false;
...
@:structInit class R_testRecursive_0 {
	...
	public function new(?ptr:Ref<R_testRecursive_0>, ?sliceP:Slice<Ref<R_testRecursive_0>>, ?slice:Slice<R_testRecursive_0>,
			?map:GoMap<GoInt, R_testRecursive_0>, ?mapP:GoMap<GoInt, Ref<R_testRecursive_0>>, ?mapR:GoMap<Ref<R_testRecursive_0>, Ref<R_testRecursive_0>>,
			?sliceMap:Slice<GoMap<GoInt, R_testRecursive_0>>) {
		if (ptr != null)
			this.ptr = ptr;
		if (sliceP != null)
			this.sliceP = sliceP;
...
function testInt64(_t:Ref<stdgo.testing.Testing.T>):Void {
	...
	if ((((_lo : GoUInt64) >> (("62" : GoUInt64) : GoUInt64)) == ("0" : GoUInt64))
		|| (((_hi : GoUInt64) >> (("62" : GoUInt64) : GoUInt64)) == ("0" : GoUInt64))) {
...
class Config_asInterface {
	...
	**/
	@:keep
	public function _getMaxCount():GoInt
		return __self__.value._getMaxCount();

	/**
		// getRand returns the *rand.Rand to use for a given Config.
	**/
	@:keep
	public function _getRand():Ref<stdgo.math.rand.Rand.Rand>
		return __self__.value._getRand();
...
@:keep @:allow(stdgo.testing.quick.Quick.CheckEqualError_asInterface) class CheckEqualError_static_extension {
	...
	static public function error(_s:Ref<CheckEqualError>):GoString {
		return stdgo.fmt.Fmt.sprintf(("#%d: failed on input %s. Output 1: %s. Output 2: %s" : GoString), Go.toInterface(_s.checkError.count),
			Go.toInterface(_toString(_s.checkError.in_)), Go.toInterface(_toString(_s.out1)), Go.toInterface(_toString(_s.out2)));
	}
}

class T_myStruct_asInterface {
	@:keep
	public function generate(_r:Ref<stdgo.math.rand.Rand.Rand>, _0:GoInt):stdgo.reflect.Reflect.Value
		return __self__.value.generate(_r, _0);
...
@:keep @:allow(stdgo.testing.quick.Quick.T_myStruct_asInterface) class T_myStruct_static_extension {
	...
	static public function generate(_m:T_myStruct, _r:Ref<stdgo.math.rand.Rand.Rand>, _0:GoInt):stdgo.reflect.Reflect.Value {
		return
			(stdgo.reflect.Reflect.valueOf(Go.toInterface(Go.asInterface(({_x: (42 : GoInt)} : T_myStruct)))) == null ? null : stdgo.reflect.Reflect.valueOf(Go.toInterface(Go.asInterface(({_x: (42 : GoInt)} : T_myStruct))))
			.__copy__());
	}
}
