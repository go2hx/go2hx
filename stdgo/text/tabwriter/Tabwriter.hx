...
@:structInit class T_cell {
	...
	public function new(?_size:GoInt, ?_width:GoInt, ?_htab:Bool)
...
@:structInit class Writer {
	...
	public function _write0(_buf:Slice<GoByte>):Void {
		var _b = new Pointer(() -> this, __tmp__ -> this.__set__(__tmp__));
		var __tmp__ = _b.value._output.write(_buf),
			_n:GoInt = __tmp__._0,
				...
				if (_n >= ((2 : GoInt))) {
					{
						...
						if (_prev > _b.value._lines[_n - ((1 : GoInt))].cap()) {
							_b.value._lines[_n - ((1 : GoInt))] = new Slice<T_cell>(...[for (i in 0...((((0 : GoInt)) : GoInt)).toBasic()) new T_cell()])
								.setCap(((_prev : GoInt)).toBasic());
						};
					};
				};
			};
	...
	public function new(?_output:stdgo.io.Io.Writer, ?_minwidth:GoInt, ?_tabwidth:GoInt, ?_padding:GoInt, ?_padbytes:GoArray<GoUInt8>, ?_flags:GoUInt,
			?_buf:Slice<GoUInt8>, ?_pos:GoInt, ?_cell:T_cell, ?_endChar:GoUInt8, ?_lines:Slice<Slice<T_cell>>, ?_widths:Slice<GoInt>)
		stdgo.internal.Macro.initLocals();

	public function toString() {
		return '{' + Go.string(_output) + " " + Go.string(_minwidth) + " " + Go.string(_tabwidth) + " " + Go.string(_padding) + " " + Go.string(_padbytes)
			+ " " + Go.string(_flags) + " " + Go.string(_buf) + " " + Go.string(_pos) + " " + Go.string(_cell) + " " + Go.string(_endChar) + " "
			+ Go.string(_lines) + " " + Go.string(_widths) + "}";
	}
	...
	public function __copy__() {
		return new Writer(_output, _minwidth, _tabwidth, _padding, _padbytes, _flags, _buf, _pos, _cell, _endChar, _lines, _widths);
	}
...
class Writer_extension_fields {
	public function _addLine(__tmp__, _flushed:Bool):Void
		__tmp__._addLine(_flushed);

	public function _reset(__tmp__):Void
		__tmp__._reset();

	public function init(__tmp__, _output:stdgo.io.Io.Writer, _minwidth:GoInt, _tabwidth:GoInt, _padding:GoInt, _padchar:GoByte, _flags:GoUInt):Pointer<Writer>
		return __tmp__.init(_output, _minwidth, _tabwidth, _padding, _padchar, _flags);

	public function _dump(__tmp__):Void
		__tmp__._dump();

	public function _write0(__tmp__, _buf:Slice<GoByte>):Void
		__tmp__._write0(_buf);

	public function _writeN(__tmp__, _src:Slice<GoByte>, _n:GoInt):Void
		__tmp__._writeN(_src, _n);

	public function _writePadding(__tmp__, _textw:GoInt, _cellw:GoInt, _useTabs:Bool):Void
		__tmp__._writePadding(_textw, _cellw, _useTabs);

	public function _writeLines(__tmp__, _pos0:GoInt, _line0:GoInt, _line1:GoInt):GoInt
		return __tmp__._writeLines(_pos0, _line0, _line1);

	public function _format(__tmp__, _pos0:GoInt, _line0:GoInt, _line1:GoInt):GoInt
		return __tmp__._format(_pos0, _line0, _line1);

	public function _append(__tmp__, _text:Slice<GoByte>):Void
		__tmp__._append(_text);

	public function _updateWidth(__tmp__):Void
		__tmp__._updateWidth();

	public function _startEscape(__tmp__, _ch:GoByte):Void
		__tmp__._startEscape(_ch);

	public function _endEscape(__tmp__):Void
		__tmp__._endEscape();

	public function _terminateCell(__tmp__, _htab:Bool):GoInt
		return __tmp__._terminateCell(_htab);

	public function _handlePanic(__tmp__, _err:Pointer<Error>, _op:GoString):Void
		__tmp__._handlePanic(_err, _op);

	public function flush(__tmp__):Error
		return __tmp__.flush();

	public function _flush(__tmp__):Error
		return __tmp__._flush();

	public function _flushNoDefers(__tmp__):Void
		__tmp__._flushNoDefers();

	public function write(__tmp__, _buf:Slice<GoByte>):{var _0:GoInt; var _1:Error;}
		return __tmp__.write(_buf);
}
